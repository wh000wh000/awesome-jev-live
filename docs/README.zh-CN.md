...
> [!NOTE]
> **实时索引** · 上次同步: `2026-09-19T20:16:12+08:00` (UTC+8)
> · 条目: **532** · 本轮新增: **55** · 实现语言: **27**
...
- [社区客户端、SDK 与适配器](#社区客户端sdk-与适配器) — **84**
- [Agent 工具链：MCP、钩子、门禁与编码 Agent](#agent-工具链mcp钩子门禁与编码-agent) — **171**
- [路由、护栏与审批](#路由护栏与审批) — **63**
- [评估、校准与基准测试](#评估校准与基准测试) — **50**
- [开放复现、权重与架构研究](#开放复现权重与架构研究) — **20**
- [应用、游戏、机器人与交互演示](#应用游戏机器人与交互演示) — **46**
- [文章、讨论与同类列表](#文章讨论与同类列表) — **41**
- [其他项目](#其他项目) — **50**
...
拉取请求中的每个代码文件，先由 TypeSafe 的 Jev 按 Uncle Bob 的 Clean Code 标准评判，再由 Luna 审查。基于 eve 和 Next.js 构建。
...
- [vayungodara/jev-lint](https://github.com/vayungodara/jev-lint) - 使用 TypeSafe Jev 检查 Markdown 知识库（Obsidian vault 或 LLM wiki）中的矛盾、过时声明、未解决标记和缺失页面。
...
在软件开发流程中使用 Jev（system one 模型）的 Opencode 插件。与 Opencode 团队无关联。
...
面向 TypeSafe Jev（System One 决策模型）的现场笔记、可运行脚本和代理技能。包含实测评估、浏览器循环、UI 文本裁判和首次点击树测试。与 Claude Fable 5.1 共同创作。
...
<details>
<summary><b>本分类更多条目</b> <sub>· 137</sub></summary>
...
- [madeye/pi-jev](https://github.com/madeye/pi-jev) - 用于更快 Pi 工作流的 Jev 辅助文件检索和请求缓存。
...
- [cassiomc1/fast-jev-compaction-alt](https://github.com/cassiomc1/fast-jev-compaction-alt) - 使用 TypeSafe 的 Jev 模型为 LLM 代理进行连续、逐字的上下文压缩.
...
- [cydevo202020/dsh-jev-warden](https://github.com/cydevo202020/dsh-jev-warden) - DeepSeek Harness 插件：通过使用 Jev 类型化决策根据冻结简报评判工具调用，强制执行代理约束技能（grill-me /。
...
- [forvela/jev-agent-browser](https://github.com/forvela/jev-agent-browser) - 由 Jev 和 agent-browser 驱动的快速、有边界的浏览器代理 — 类型化操作、研究、分类和安全编排.
...
- [iamdin/pi-jev-skill-suggestion](https://github.com/iamdin/pi-jev-skill-suggestion) - Pi 扩展，移除内联技能列表，并询问 TypeSafe Jev 要加载哪个技能。
...
在 CPU 上运行 SemIf（Jev 风格的语义 if 决策）— 无需 GPU。一次前向传递即可直接从开放模型读取类型化选项概率，另带 Web UI。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/leesk212--jev-cpu/596a11d2546a76f3.gif" width="100%" alt="leesk212/JEV-CPU animation"><br><sub>动态录屏</sub></td>
</tr></table>
...
- [evan87863/awesome-jev-examples](https://github.com/evan87863/awesome-jev-examples) - 来自 GitHub、网络、X 和 YouTube 的 TypeSafe Jev。
...
<sub>生成者 · `render.py` · 2026-09-19T20:16:12+08:00</sub>
