...
> [!NOTE]
> **实时索引** · 上次同步: `2026-09-19T22:33:29+08:00` (UTC+8)
> · 条目: **545** · 本轮新增: **56** · 实现语言: **25**
...
- [社区客户端、SDK 与适配器](#社区客户端sdk-与适配器) — **85**
- [Agent 工具链：MCP、钩子、门禁与编码 Agent](#agent-工具链mcp钩子门禁与编码-agent) — **178**
- [路由、护栏与审批](#路由护栏与审批) — **62**
- [评估、校准与基准测试](#评估校准与基准测试) — **46**
- [开放复现、权重与架构研究](#开放复现权重与架构研究) — **25**
- [应用、游戏、机器人与交互演示](#应用游戏机器人与交互演示) — **45**
- [文章、讨论与同类列表](#文章讨论与同类列表) — **40**
- [其他项目](#其他项目) — **58**
...
使用 typesafe-ai/jev 而不是行为树的游戏直觉服务。
...
使用 TypeSafe.ai 的 Jev 通过自然语言运行 Flutter 集成测试
...
EEG 能驱动 AI agent 吗？公共数据实验：心理命令检测、类似 Muse 的通道消融、Jev（typesafe.ai）集成、双层 EEG 确认。
...
Jev 有多确定？将 TypeSafe Jev 概率输出转换成一个诚实的数字，并配有图表。
...
- [blazskufca/typesafe-sdk-go](https://github.com/blazskufca/typesafe-sdk-go) - Typesafe.ai 在 Go 中的社区 SDK。
...
- [Mrlyk/jev-browser](https://github.com/Mrlyk/jev-browser) - 面向 AI 代理的浏览器自动化 CLI，由 Jev 模型的毫秒级决策和近零推理成本提供支持。
- [muse0509/jev-preflight](https://github.com/muse0509/jev-preflight) - 面向 Claude Code 的有界 Jev 风险检查：八个风险轴、一次请求、一次可选复检.
...
- [dizk/jev-lens](https://github.com/dizk/jev-lens) - jev 选择编码 agent 能看到的大型工具结果内容：核心库、pi 扩展和 Claude Code 插件。
- [dtduc-git/jev-table](https://github.com/dtduc-git/jev-table) - 使用 TypeSafe 的 Jev 为 CSV/JSONL 文件提供 AI 列 — 类型化答案、置信度、审查队列、恢复和成本预览。本地优先，自带密钥.
- [frodi-karlsson/jev-client](https://github.com/frodi-karlsson/jev-client) - 一个 typescript 客户端，用于以符合人体工程学的方式访问 Jev API。
...
- [Gilbert09/jev-cli](https://github.com/Gilbert09/jev-cli) - 用于 Claude Code、由 Jev 驱动的判断层：语义权限门控、提示注入筛查、完成验证和语义排序.
- [loicrg/jev-spam-lab](https://github.com/loicrg/jev-spam-lab) - 用于使用 TypeSafe AI 的 Jev 模型评估 Microsoft Outlook 电子邮件的 CLI.
...
- [realdubb/jev-playground](https://github.com/realdubb/jev-playground) - 用于 TypeSafe AI 的 Jev 决策模型的 Playground：提供商无关的 AI SDK 脚本、jev-filter CLI、GitHub。
- [thisisjorge/jev-control-room](https://github.com/thisisjorge/jev-control-room) - 使用 Vercel AI SDK 和 TypeSafe Jev 的可视化控制室，用于类型化 AI 评估、结构化决策和置信度感知的策略门控.
- [wsmoak/jev-sales-calls](https://github.com/wsmoak/jev-sales-calls) - 示例：TypeSafe AI 的 Jev 模型根据产品能力库读取销售电话转录记录。
...
由 TypeSafe Jev 1.13 非自回归决策模型驱动的 Claude Code 实时质量门和 Art Director Warden
...
通过 MCP 为 Codex 提供由 Jev 驱动的工具和技能选择、上下文搜索以及输出分流
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/teempai--jev-in-codex/f886e1f5fa4e6634.gif" width="100%" alt="teempai/jev-in-codex animation"><br><sub>动态录屏</sub></td>
</tr></table>
...
由 TypeSafe 为 Jev 提供的辅助 n8n 社区节点。用你定义的问题对文本进行分类、路由和评分，并获取每个答案的概率，以便将不确定项送去审核。
...
Pong，其中球每次模型决策移动一步。Jev 对战 LLMs，通过 Vercel AI Gateway，每位玩家和代理都在一个 Ably 频道上。
...
TypeSafe AI 的 Jev vs GPT-5.6 Luna vs Claude Sonnet 5 在支持工单分流上的对比（Vercel AI Gateway）
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/bunyod0987--jev-vs-llm/416153a496dc9e59.gif" width="100%" alt="BUNYOD0987/jev-vs-llm animation"><br><sub>动态录屏 · <a href="https://raw.githubusercontent.com/BUNYOD0987/jev-vs-llm/main/demo.mp4">打开视频</a></sub></td>
</tr></table>
...
<details>
<summary><b>本分类更多条目</b> <sub>· 144</sub></summary>
...
- [imMamdouhaboammar/get-fable](https://github.com/imMamdouhaboammar/get-fable) - 通过更好的规划、持久上下文、技能、钩子、故障处理和验证，让你已在使用的模型更像前沿模型一样工作。
...
- [altryne/jevify](https://github.com/altryne/jevify) - 一种 agent 技能，用于发现 TypeSafe Jev 机会、设计类型化问题，并从近期社区实验中学习.
...
- [Nyarlathoteppppp/pi-heed](https://github.com/Nyarlathoteppppp/pi-heed) - pi 编码代理的运行时约束：在每个有副作用的工具调用运行之前，根据你所说的内容进行检查。由 TypeSafe Jev 提供支持.
...
- [oldmoldycake/jev_vampire_survivors](https://github.com/oldmoldycake/jev_vampire_survivors) - TypeSafe 的 Jev 模型在 Steam 上玩 Vampire Survivors：BepInEx 插件 + Python 大脑 +。
...
- [jimmyliao/jev-storyboard-lab](https://github.com/jimmyliao/jev-storyboard-lab) - 用于结构化输出代理的 Google ADK vs Microsoft Agent Framework，以 TypeSafe Jev 作为供应商中立的 QC 门。
...
- [willfish/pi-observational-memory-jev](https://github.com/willfish/pi-observational-memory-jev) - Jev 决定保留什么。压缩从不重写转录记录.
...
- [207studio/jev-codex-tools](https://github.com/207studio/jev-codex-tools) - 实验性的选择加入式 Jev 决策工具，用于有界 Codex 会话读取和受保护的 UI 工作流.
...
- [chrishan17/claude-jev-mod](https://github.com/chrishan17/claude-jev-mod) - Claude Code 中的类型化决策：通过 OpenRouter、Vercel AI Gateway、Cloudflare Workers。
- [chy4pro/JevBrowserExt](https://github.com/chy4pro/JevBrowserExt) - Chrome 扩展，使用 TypeSafe Jev（一个亚秒级决策模型）驱动网页。browser-use/jev-ultrafast 的 MV3 移植版.
...
- [dev-willbird1936/pi-jev-compact](https://github.com/dev-willbird1936/pi-jev-compact) - 用于 Pi Coding Agent 的逐字 Jev 上下文压缩。
...
- [EthanThatOneKid/zocomputer-jev](https://github.com/EthanThatOneKid/zocomputer-jev) - 一个 Zo 技能，用于使用 Vercel AI Gateway 和 TypeSafe AI Jev 进行情境脚本编写和执行.
...
- [hangarbay/jev.mcp](https://github.com/hangarbay/jev.mcp) - 用于 TypeSafe 的 Jev 的一个 MCP 服务器：类型化、校准的决策，而不是生成文本。
- [Heman10x-NGU/openJev-verdict-2.0](https://github.com/Heman10x-NGU/openJev-verdict-2.0) - 经过校准的 151M 非自回归决策引擎，在 LocalLLaMA/typed-decisions 上击败 TypeSafe Jev 和 Laya（77.10%。
- [iamdin/pi-jev-skill-bench](https://github.com/iamdin/pi-jev-skill-bench) - Pi 技能路由基准：BM25 vs TypeSafe Jev，跨 50–500 的名单规模。
- [InfamousCube/JevPilot](https://github.com/InfamousCube/JevPilot) - 使用 TypeSafe Jev 进行计算机使用：用一个提示控制你的 PC、浏览器、游戏和 Android 手机。
...
- [JayDoubleu/cc-mod-jev](https://github.com/JayDoubleu/cc-mod-jev) - Claude Code mod：通过 OpenRouter 进行 Jev 评分的上下文剪枝。逐字压缩、对超大工具输出的可选门控、测试和评估.
...
- [kangshifu1/jev-computer-use](https://github.com/kangshifu1/jev-computer-use) - 用于 Codex 和 Chrome/Chromium 的独立 Jev computer-use 技能，具备有界动作和已验证的浏览器工作流.
- [kangshifu1/jev-skills-market](https://github.com/kangshifu1/jev-skills-market) - 社区 Jev 技能市场和助手，用于自动化测试、金融研究和语音工作流。Computer Use 是一个独立仓库.
...
- [LIFE-Electronic/bluehouse-jev-browser](https://github.com/LIFE-Electronic/bluehouse-jev-browser) - 使用 Jev 驱动 Bluehoue 的 CDP 浏览器的技能。
...
- [luxus/ha-conversation-jev](https://github.com/luxus/ha-conversation-jev) - Home Assistant 自定义组件：带 Jev 快速路径 + Grok 回退的对话代理。
...
- [namayasai/backstage-jev-operations-support](https://github.com/namayasai/backstage-jev-operations-support) - 由 Jev 提供支持的 Backstage 运营决策支持：就绪检查、事件分流、变更评审、模板和团队推荐，以及目录重排序.
- [nickylin/jev-harness](https://github.com/nickylin/jev-harness) - 由 TypeSafe Jev 驱动的代理类型化决策控制平面。
...
- [phin-tech/pi-jev-approver](https://github.com/phin-tech/pi-jev-approver) - 用于 Pi coding agent 的 Shell 命令安全门，由 TypeSafe 的 Jev 判断模型支持。
...
- [Reamd7/omp-fast-jev-compaction](https://github.com/Reamd7/omp-fast-jev-compaction) - 使用 TypeSafe Jev 决策为 Oh My Pi (omp) 进行逐字上下文压缩——joelhooks/pi-fast-jev-compaction。
...
- [shitianfang/jev-use](https://github.com/shitianfang/jev-use) - Claude Code / Codex / pi 插件，将不需要文本输出的代理步骤交给 Jev（TypeSafe 的判断模型）——约 200 ms，每次判断约。
...
- [szocpaul/jev-compaction-prime](https://github.com/szocpaul/jev-compaction-prime) - 用于 Prime Agent 的逐字、基于决策的上下文压缩——不是摘要，而是对过时的工具调用进行评分并丢弃；所有保留内容都逐字节保持完整.
...
用你的声音和指向手势在 tldraw 画布上绘图。Jev（TypeSafe System One）以每个口述词约 350 ms 的速度决定动作、目标和位置。
...
- [makefinks/jev-feed-filter](https://github.com/makefinks/jev-feed-filter) - 使用 Jev 为 X 和 YouTube feed 提供智能、动态的 AI 过滤。
- [NemanjaManic/ci-gatekeeper-bot-jev](https://github.com/NemanjaManic/ci-gatekeeper-bot-jev) - GitHub Action，使用 Jev（通过 Vercel AI Gateway 的 TypeSafe AI）在昂贵的 LLM/人工审查前低成本分流拉取请求。
...
为有用证据腾出空间。面向 RAG 的可检查上下文选择，带有 Jev 重排序和开放基准研究。
...
- [scarif-labs/jev-software-decision-benchmark](https://github.com/scarif-labs/jev-software-decision-benchmark) - 可复现的基准，用于评估 JEV 在分布偏移下作为依赖更新自动化的软件决策原语.
...
- [backmeupplz/jev_antispam_bot](https://github.com/backmeupplz/jev_antispam_bot) - 由 TypeSafe Jev 驱动的极简 grammY Telegram 反垃圾信息机器人。
- [leesk212/JEV-CPU](https://github.com/leesk212/JEV-CPU) - 在 CPU 上运行 SemIf（Jev 风格的语义 if 决策）— 无需 GPU。一次前向传递即可直接从开放模型读取类型化选项概率，另带 Web UI.
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/aslamdoctor/react-chess-game-using-jev/master/demo/demo.gif" width="100%" alt="aslamdoctor/react-chess-game-using-jev animation"><br><sub>动态录屏</sub></td>
</tr></table>
...
- [jaibhasin/jev-flappy-bird](https://github.com/jaibhasin/jev-flappy-bird) - Jev 学习在有和没有基于物理的上下文的情况下玩 flappy-bird 游戏。
...
一个优化的推理引擎，用于将 LLMs 转换为类似 Jev 的机器：针对快速、轻量且准确的决策、分类和评分进行了优化
...
<details>
<summary><b>本分类更多条目</b> <sub>· 39</sub></summary>

- [naveenreddy61/jev-experiments](https://github.com/naveenreddy61/jev-experiments) - experiments with system one model jev。
- [robzolkos/omarchy-issue-classifier](https://github.com/robzolkos/omarchy-issue-classifier) - 使用 Jev（TypeSafe 的 System One 模型）对 Omarchy。
...
- [kunchenguid/local-jev](https://github.com/kunchenguid/local-jev) - 基于 ModernBERT zero-shot（ONNX）的本地 Jev/System One 兼容 API。
...
- [Joymfl/dag-jev](https://github.com/Joymfl/dag-jev) - 通过 Jev 从无序项创建 DAG。
- [rolottr/x-jev-classifier](https://github.com/rolottr/x-jev-classifier) - Chrome 扩展，为每条 X 帖子打上类型徽章——alpha、shitpost、AI slop、bait——由 Typesafe 的 Jev 判定。
- [TanayPadar/gpt-vs-jev](https://github.com/TanayPadar/gpt-vs-jev) - 比较 GPT 生成的语言与 JEV 针对相同输入所做的结构化 Noul 决策.
- [thezem/jev-one](https://github.com/thezem/jev-one) - 一个由 TypeSafe AI Jev 驱动、基于词汇表的 TypeScript 运行时，用于安全、有状态的应用程序.
...
<sub>生成者 · `render.py` · 2026-09-19T22:33:29+08:00</sub>
