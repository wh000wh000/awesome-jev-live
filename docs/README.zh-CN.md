...
> [!NOTE]
> **实时索引** · 上次同步: `2026-09-23T09:31:07+08:00` (UTC+8)
> · 条目: **766** · 本轮新增: **61** · 实现语言: **26**
...
<sub>一个适用于你所有机器人的离板 CLI。连接它们、指挥它们，并让它们协同工作，每个都用 LLM 作为大脑，用决策 LLM 实现更便宜、更快速的步骤。Microduck、Open Duck Mini、LeRobot、XLeRobot、AlohaMini、ToddlerBot 或任何 ROS…</sub>
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<sub>Jev，TypeSafe 的 System One 模型，可与任意 OpenRouter LLM、Stockfish 以及你进行国际象棋对弈。单页 Web 应用，提供实时走子、Jev 的走子概率、已保存的棋局和胜率。</sub>
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
一个适用于你所有机器人的离板 CLI。连接它们、指挥它们，并让它们协同工作，每个都用 LLM 作为大脑，用决策 LLM 实现更便宜、更快速的步骤。Microduck、Open Duck Mini、LeRobot、XLeRobot、AlohaMini、ToddlerBot 或任何 ROS 底盘。Claude、OpenAI、Gemini、Grok，或通过 Ollama 或 vLLM 在本地运行。

<sub>🔧 在代码中的使用: `docs/adr/0043-decision-llms-are-a-wire-format-and-a-data-row.md`</sub>
...
- [keltokhy/jgrep](https://github.com/keltokhy/jgrep) - grep，但模式是描述。使用 TypeSafe 的 Jev 决策模型，按含义筛选行：每行约 200 毫秒，成本仅为千分之一美分.
- [9pings/notjev](https://github.com/9pings/notjev) - 超快速的类 Jev 服务器，模型无关，可与任何 OpenAI 兼容端点配合使用。
- [keltokhy/jsort](https://github.com/keltokhy/jsort) - 按含义排序：根据 TypeSafe 的 Jev 模型通过成对比较得出的结果，沿着一个通俗英语维度排列各行。
...
- [evoke-build/evoke](https://github.com/evoke-build/evoke) - 软件，凭反射运行。说出需求，恰当的小程序就会运行：由经过校准的分类器选择，仅在足够确定时运行，并在执行任何不可撤销的操作前询问.
- [Qew7/jev-feels](https://github.com/Qew7/jev-feels) - 由 Jev 驱动的语义决策，可作为普通的 Ruby #feels?、#decide、#score、Rails 验证和模式匹配。
- [oswarld/notion-skills](https://github.com/oswarld/notion-skills) - 面向 Notion 韩语用户的恰到好处技能。
...
- [keltokhy/jlink](https://github.com/keltokhy/jlink) - 面向经济学家的记录链接：用通俗英语编写匹配规则，为每一对记录获取概率，进行审计并注明引用。Python、CLI、Stata 和 R.
...
<details>
<summary><b>本分类更多条目</b> <sub>· 221</sub></summary>
...
- [compozy/yoshi](https://github.com/compozy/yoshi) - 面向 Claude Code 和 Codex 的上下文剪枝代理：Jev 判断哪些历史仍然需要，基于测量而非宣称.
...
- [shitianfang/jev-use](https://github.com/shitianfang/jev-use) - Claude Code / Codex / pi 插件，将无需文本输出的代理步骤交给 Jev（TypeSafe 的判断模型）——测得 p50 约为 230…
...
- [ronadin2002/jev-cua](https://github.com/ronadin2002/jev-cua) - 用于控制 macOS 的语音和文本。一个浮动栏，借助 Jev 实时选择 UI 操作，以及持续的观察—行动—验证循环.
...
- [weitianxin/JevAny](https://github.com/weitianxin/JevAny) - 面向决策模型的校准感知 RL，正扩展到复杂代理、多模态证据、长上下文和符号控制.
...
使用 System One Models 进行自托管提示词监控
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/hfnissum-byte--hunkpick/21c4c5bad7854d6e.gif" width="100%" alt="hfnissum-byte/Hunkpick animation"><br><sub>动态录屏</sub></td>
</tr></table>
...
流量边缘的类型化判断准入控制：由 TypeSafe Jev 驱动的 nginx/OpenResty 三层提示词注入和滥用过滤器。故障时放行、支持缓存和热加载。
...
开放的 System One 模型：具有校准概率的类型化决策，可在你自己的数据上训练。不生成文本。
...
- [fstandhartinger/jevbench](https://github.com/fstandhartinger/jevbench) - JevBench v1——一种针对 Jev 类类型化决策模型的基准测试：智能、低成本、快速、可靠、开放.
...
- [backmeupplz/jev_antispam_bot](https://github.com/backmeupplz/jev_antispam_bot) - 由 TypeSafe Jev 驱动的最小化 grammY Telegram 反垃圾机器人。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/yinsongxu--llm2jev/9b0b042b0dfa87b7.gif" width="100%" alt="Yinsongxu/LLM2Jev animation"><br><sub>动态录屏 · <a href="https://raw.githubusercontent.com/Yinsongxu/LLM2Jev/main/assets/mujoco.mp4">打开视频</a></sub></td>
</tr></table>
...
- [Argos1111/jev_local](https://github.com/Argos1111/jev_local) - 使用本地 LLM 复现 Jev。
...
<sub>生成者 · `render.py` · 2026-09-23T09:31:07+08:00</sub>
