...
> [!NOTE]
> **实时索引** · 上次同步: `2026-09-23T17:45:55+08:00` (UTC+8)
> · 条目: **778** · 本轮新增: **111** · 实现语言: **23**
...
<sub>针对那些无法写成模式的问题进行 grep。读取每个函数，而不是抽样读取几个。由 Jev 的 TypeSafe AI System One 模型驱动。</sub>
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
- [Agent 工具链：MCP、钩子、门禁与编码 Agent](#agent-工具链mcp钩子门禁与编码-agent) — **239**
- [路由、护栏与审批](#路由护栏与审批) — **55**
...
<sub>🔧 在代码中的使用: `src/typesafe_sdk/__init__.py`, `src/typesafe_sdk/_core/retry.py`, `src/typesafe_sdk/_core/config.py`</sub>
...
一个封装 ConvAI Innovations 的 OSS Laya system one 模型的 Elixir 库
...
针对那些无法写成模式的问题进行 grep。读取每个函数，而不是抽样读取几个。由 Jev 的 TypeSafe AI System One 模型驱动。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/clouatre-labs--decisions-judge-mcp/c8c00f2b58774857.gif" width="100%" alt="clouatre-labs/decisions-judge-mcp animation"><br><sub>动态录屏</sub></td>
</tr></table>
...
一个供你所有机器人使用的机外 CLI。连接它们、指挥它们，让它们协同工作；每个机器人都以 LLM 作为大脑（Claude、OpenAI、Gemini、Grok，或通过 Ollama 或 vLLM 在本地运行），并使用决策 LLM 来执行更便宜、更快速的步骤（Jev、Laya 和 Kev）。支持 Microduck、Open Duck Mini、LeRobot、XLeRobot、AlohaMini、ToddlerBot、Jetson 或任何 ROS。
...
用于在 iPhone 和 Mac 上通过 Apple 的 Core AI 运行聊天、视觉和语音模型的 Swift SDK。模型下载与缓存、FoundationModels 集成，以及附带 OS、SDK 和模型要求文档的可运行示例。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/john-rocky--coreai-kit/c98c7bb8cb04e76e.gif" width="100%" alt="john-rocky/coreai-kit animation"><br><sub>动态录屏</sub></td>
</tr></table>
...
jev-seo：面向编码代理的 Rust SEO 和 GEO CLI，以及 MCP 服务器：50 条规则审计、实时抓取、GEO 分数、排名波动、CI 门控。MIT，零订阅费。
...
只读交易日志和评审工具：Jev 类型化判断、agent 集成，以及可复现的金融基准。不下单，不提供建议。
...
- [9pings/notjev](https://github.com/9pings/notjev) - 超快速的类 Jev 服务器，模型无关，可与任何 OpenAI 兼容端点配合使用。
...
- [bladedevoff/stuntd](https://github.com/bladedevoff/stuntd) - 本地代理会学习你的应用中的类型化 LLM 决策，并使用 Laya 头部模型回答这些决策。兼容 Jev 和 OpenAI.
- [tumf/jev-cli](https://github.com/tumf/jev-cli) - 用于 TypeSafe Jev 的小型无依赖 CLI。
- [talmago/fast_gliner](https://github.com/talmago/fast_gliner) - 用于以 Rust 编写的 GLiNER 模型推理引擎的 Python 绑定。
- [frostney/clean-code-review](https://github.com/frostney/clean-code-review) - 由 TypeSafe 的 Jev 根据 Uncle Bob 的 Clean Code 对 pull request 中的每个代码文件进行评判，然后由 Luna…
- [Qew7/jev-feels](https://github.com/Qew7/jev-feels) - 由 Jev 驱动的语义决策，可作为普通的 Ruby #feels?、#decide、#score、Rails 验证和模式匹配。
...
- [tontoko/jev-browser](https://github.com/tontoko/jev-browser) - 一个有依据的 Jev/Playwright 核心：类型化的 SDK、持久化的 CLI，以及支持原生浏览器操作和确定性断言的 MCP 服务器.
...
- [vercel-labs/jev-ai-sdk-form-router](https://github.com/vercel-labs/jev-ai-sdk-form-router) - 使用 Jev 和 AI SDK 将表单提交路由给正确的人员.
- [ansidium/jev-codex-bridge](https://github.com/ansidium/jev-codex-bridge) - 为 Codex Desktop 和 CLI 提供模型与推理路由，配备 Windows 服务和经过验证的更新。
- [mountainMath/JevR](https://github.com/mountainMath/JevR) - 用于 TypeSafe Jev System One API 的 R 客户端。
- [shinpr/jev-reranker](https://github.com/shinpr/jev-reranker) - 使用 TypeSafe AI 的 Jev 对 JSON 的搜索结果重新排序、筛选和压缩.
...
- [nylon-memory/NylonME](https://github.com/nylon-memory/NylonME) - 面向 AI Agents 的记忆引擎，使用 Jev 可以带来更高的决策准确率.
...
用于发现、构建、评估和改进决策模型系统的代理技能。面向 Software 3.0 的组合式决策演算、评估工具集，以及有界的提示词/程序爬山优化。TypeSafe Jev 是默认的托管示例。独立于 TypeSafe。
...
<td align="center" valign="top"><a href="https://youtu.be/ZC4oge4WsCQ"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/24601--augustus/d2593af47d538d33.jpg" width="100%" alt="video"></a><br><sub><a href="https://youtu.be/ZC4oge4WsCQ">在以下平台观看 youtu.be</a> · 播放在托管站点打开；GitHub 无法内嵌播放</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/storm-software--mindctl/69298c40a77b7be5.gif" width="100%" alt="storm-software/mindctl animation"><br><sub>动态录屏</sub></td>
</tr></table>
...
<details>
<summary><b>本分类更多条目</b> <sub>· 217</sub></summary>

- [ChenneyZhuang/laya-browser-agent](https://github.com/ChenneyZhuang/laya-browser-agent) - 本地开源的 Jev 替代方案：在你自己的机器上使用 Laya（System One 模型）进行浏览器代理决策。无需云服务，也无需 API 密钥.
...
- [kydlikebtc/awesome-jev](https://github.com/kydlikebtc/awesome-jev) - 805 verified examples of Jev — TypeSafe AI。
- [Amal-David/awesome-jev](https://github.com/Amal-David/awesome-jev) - Jev 演示、项目、SDKs 和技能，带有源码链接和精选 X 画廊.
...
- [Alex314618-create/JevRev](https://github.com/Alex314618-create/JevRev) - 一个 LLM + Jev 工作流。用内置的脊柱增强你的脊椎动物大脑.
...
- [socai-io/jev-social](https://github.com/socai-io/jev-social) - 由 Jev 驱动的 Instagram、TikTok 和 LinkedIn 研究：类型化路由、真实浏览器证据、流式帖子卡片、视频捕获，以及带引用的 socai…
...
- [PromptEngineer48/laya-vs-jev-arena](https://github.com/PromptEngineer48/laya-vs-jev-arena) - Laya（开源、本地）对战 TypeSafe Jev（API）：两个 AI 模型在 Snake 中竞速，并在真人快打风格的竞技场中战斗.
...
- [jiawei686/jev-ultrafast-mcp](https://github.com/jiawei686/jev-ultrafast-mcp) - 把浏览器工作交出去：一个 MCP 服务器，其中决策模型为你的代理驱动页面，因此一个流程只需一次工具调用，而不是每次点击一轮.
...
- [Bodila51/muse-jev-playbook](https://github.com/Bodila51/muse-jev-playbook) - Muse 的 Jev 决策层：在昂贵的智能体工作之前提供快速、低成本的 TypeSafe AI 门控——置信度策略、配方、参考路由器、诚实测量.
...
- [rashedInt32/jev-mcp](https://github.com/rashedInt32/jev-mcp) - MCP 服务器，将 TypeSafe Jev 作为类型化、经过校准的判断工具提供：分类、评分、检查、批量询问。作为 Claude Code 插件发布.
- [ZephyrDeng/ego-jev](https://github.com/ZephyrDeng/ego-jev) - ego-browser 的 Jev（TypeSafe System One）内循环：每个 DOM 步骤执行一次约 0.4 秒的类型化决策，而不是一次 LLM…
- [green-dalii/pi-shift-router](https://github.com/green-dalii/pi-shift-router) - Pi…
- [ismaelsoilet/jev-harness](https://github.com/ismaelsoilet/jev-harness) - 零依赖 System One 决策测试框架：5 个语义门，在琐碎错误和 doom loops 上节省前沿 AI 代理 token.
...
- [jiangkoumo/ego-jev](https://github.com/jiangkoumo/ego-jev) - 用 Jev（TypeSafe System One）驱动 ego lite 浏览器：输入一个索引元素表，输出一个操作 + 目标，单进程.
...
- [itsmostafa/typesafe-mcp](https://github.com/itsmostafa/typesafe-mcp) - 一个用于快速且低成本评估任何内容的 mcp 连接器。让你的 AI agent 直接访问 typesafe ai 的 jev 模型.
...
非自回归 System 1 决策引擎。可在单次前向传递中，对任何文本进行类型化选择、评分和是/否决策，支持 100+ 种语言，并带有一个可按请求选择正确 checkpoint 的路由器。

<sub>🔧 在代码中的使用: `research/benchmarks/feishu_zh/run.py`</sub>
...
Jev，TypeSafe 的 System One 模型，可与任意 OpenRouter LLM、Stockfish 以及你进行国际象棋对弈。单页 Web 应用，提供实时走子、Jev 的走子概率、已保存的棋局和胜率。
...
一个没有训练策略的 Clash Royale 机器人：Jev（TypeSafe System One）根据实时游戏状态做出每个决策
...
将任何 LLM 转换为 Jev 风格的决策模型：类型化决策、真实概率、无需训练。（持续更新）
...
面向 Jev 风格决策模型的开放训练与推理栈。训练模型从共享前缀中为动态候选分支评分，并支持高基数选择、校准和快速批量推理。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/weitianxin--jevany/05825e369317a429.gif" width="100%" alt="weitianxin/JevAny animation"><br><sub>动态录屏</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/ankit-aglawe--tinyjev/bff87199840daa0f.gif" width="100%" alt="ankit-aglawe/tinyjev animation"><br><sub>动态录屏</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/fbddcz--embodied-jev/297ef7c24f9a67a8.gif" width="100%" alt="FBddcz/embodied-jev animation"><br><sub>动态录屏 · <a href="https://raw.githubusercontent.com/FBddcz/embodied-jev/main/docs/media/libero-drawer-comparison.mp4">打开视频</a></sub></td>
</tr></table>
...
<details>
<summary><b>本分类更多条目</b> <sub>· 19</sub></summary>
...
人们使用 Jev 构建内容的最完整图库，展示 TypeSafe 的 System One 模型：按场景分类的 3,400 多个项目、演示和文章，每项都附有原始链接、图片和描述。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/darthblanc/tictacjev/main/assets/screenrecording.gif" width="100%" alt="darthblanc/tictacjev animation"><br><sub>动态录屏</sub></td>
</tr></table>
...
<details>
<summary>🎮 <b><a href="https://x.com/tspy/status/2100864234523685146">Xtags — X 帖子意图标注插件</a></b> · @tspy · 👁️ observed · 5 天</summary>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/surbhit20/jev-plays-snake/main/jev-plays-snake.gif" width="100%" alt="surbhit20/jev-plays-snake animation"><br><sub>动态录屏</sub></td>
</tr></table>
...
- [heyjunpenn/awesome-jev](https://github.com/heyjunpenn/awesome-jev) - 一个经过验证、由社区维护的目录，收录了 832 个使用 Jev 构建的开源项目.
...
- [jaibhasin/jev-yt-time-saver](https://github.com/jaibhasin/jev-yt-time-saver) - 一个用 Jev 覆盖分散注意力的 YouTube 视频的 Chrome 扩展。你想看时随时显示.
...
- [Show HN: Will Jev pull the lever in the trolley problem?](https://news.ycombinator.com/item?id=49773756) - 你们一直在等待的 Jev 基准测试：Jev 会在电车难题中拉下拉杆吗？别想太多 :) 在轨道两侧放上你想放的任何东西，GPU。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/jev-chat--jev-chat-jarvis-mac/daa9ab5c1715d764.gif" width="100%" alt="jev-chat/jev-chat-jarvis-mac animation"><br><sub>动态录屏</sub></td>
</tr></table>
...
<sub>生成者 · `render.py` · 2026-09-23T17:45:55+08:00</sub>
