...
> [!NOTE]
> **ライブインデックス** · 最終同期: `2026-09-23T09:31:07+08:00` (UTC+8)
> · エントリ数: **766** · 今回の追加: **61** · 実装言語: **26**
...
<sub>すべてのロボット向けの単一のオフボードCLI。ロボットを接続し、指示を出し、協調して動作させます。それぞれが頭脳としてLLMを持ち、より安価で高速な手順のための意思決定LLMを備えています。Microduck、Open Duck…</sub>
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
<sub>Jev、TypeSafeのSystem Oneモデルが、あらゆるOpenRouter LLM、Stockfish、そしてあなたとチェスを対局します。ライブの指し手、Jevの指し手の確率、保存済みゲーム、勝率を備えた1ページのWebアプリです。</sub>
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
- [ルーティング、ガードレール、承認](#ルーティングガードレール承認) — **54**
...
すべてのロボット向けの単一のオフボードCLI。ロボットを接続し、指示を出し、協調して動作させます。それぞれが頭脳としてLLMを持ち、より安価で高速な手順のための意思決定LLMを備えています。Microduck、Open Duck Mini、LeRobot、XLeRobot、AlohaMini、ToddlerBot、またはROSベースの任意のものに対応。Claude、OpenAI、Gemini、Grok、またはOllamaやvLLM経由のローカル環境にも対応。

<sub>🔧 コード内での使用を確認: `docs/adr/0043-decision-llms-are-a-wire-format-and-a-data-row.md`</sub>
...
100%無料 ₹0のエージェントファーストSEO & GEO CLIスイートおよびMCPサーバー（Rust）で、DuckDuckGoとTypeSafe Jev System One https://akashpriyadarshii.github.io/jev-seo/を介してSemrushとOpenSEOを置き換えます
...
- [keltokhy/jgrep](https://github.com/keltokhy/jgrep) - grepだが、パターンは説明です。TypeSafeのJev意思決定モデルにより、意味で行をフィルタリングします：1行あたり約200…
- [9pings/notjev](https://github.com/9pings/notjev) - 超高速なJev風サーバー。モデル非依存で、OpenAI互換のあらゆるエンドポイントで動作。
- [keltokhy/jsort](https://github.com/keltokhy/jsort) - 意味で並べ替え：TypeSafeのJevモデルが判定したペアワイズ比較に基づき、平易な英語の尺度に沿って行を並べます。
...
- [evoke-build/evoke](https://github.com/evoke-build/evoke) - 反射的に動くソフトウェア。言えば、適切な小さなプログラムが実行されます：較正済みの分類器によって選ばれ、十分に確信がある場合にのみ実行され、取り消せないことの前…
- [Qew7/jev-feels](https://github.com/Qew7/jev-feels) - 通常のRubyとして扱うセマンティックな意思決定。Jevによる#feels?、#decide、#score、Railsバリデーション、パターンマッチング。
- [oswarld/notion-skills](https://github.com/oswarld/notion-skills) - Notionの韓国語ユーザー向けの、気の利く完璧なスキル。
...
- [keltokhy/jlink](https://github.com/keltokhy/jlink) - 経済学者向けのレコードリンケージ：照合ルールを平易な英語で書き、ペアごとの確率を取得し、監査し、引用する。Python、CLI、Stata、R.
- [mountainMath/JevR](https://github.com/mountainMath/JevR) - TypeSafe Jev System One API のための R クライアント。
- [ruban-24/switchboard](https://github.com/ruban-24/switchboard) - Claude CodeおよびCodex向けの、オープンソースでモデル非依存の判断ルーター.
- [shinpr/jev-reranker](https://github.com/shinpr/jev-reranker) - TypeSafe AIのJevを使ってJSONの検索結果を再ランク付け、フィルタリング、圧縮.
...
<details>
<summary><b>このカテゴリのその他</b> <sub>· 221</sub></summary>
...
- [compozy/yoshi](https://github.com/compozy/yoshi) - Claude CodeとCodexのためのコンテキスト刈り込みプロキシ：Jevがどの履歴がまだ必要かを判断し、主張ではなく測定済み.
...
- [shitianfang/jev-use](https://github.com/shitianfang/jev-use) - Claude Code / Codex / piプラグイン。テキスト出力を必要としないエージェントのステップをJev（TypeSafeの判断モデル）に任せます.
...
- [muratcakmak/jev-guard](https://github.com/muratcakmak/jev-guard) - Claude…
...
- [weitianxin/JevAny](https://github.com/weitianxin/JevAny) - 意思決定モデルのためのキャリブレーションを考慮したRL。複雑なエージェント、マルチモーダルな証拠、長いコンテキスト、シンボリック制御へと拡張中.
...
System One Modelsによるセルフホスト型プロンプト監視
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/hfnissum-byte--hunkpick/21c4c5bad7854d6e.gif" width="100%" alt="hfnissum-byte/Hunkpick animation"><br><sub>アニメーション記録</sub></td>
</tr></table>
...
トラフィックエッジでの型付き判断によるアクセス制御：TypeSafe Jevを利用した、nginx/OpenResty向けの3層プロンプトインジェクション・不正利用フィルター。フェイルオープン、キャッシュ対応、ホットリロード対応。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/arpit-khandelwal--jev-linkedin-slop-filter/2ba80db1b4af936f.gif" width="100%" alt="Arpit-Khandelwal/jev-linkedin-slop-filter animation"><br><sub>アニメーション記録</sub></td>
</tr></table>
...
- [raihankhan-rk/diffjury](https://github.com/raihankhan-rk/diffjury) - DiffJury — TypeSafe Jev PRリスクルーター + コードレビューコーチ。
...
オープンなSystem Oneモデル：較正済み確率による型付き意思決定を、独自データで学習可能。テキスト生成なし。
...
- [fstandhartinger/jevbench](https://github.com/fstandhartinger/jevbench) - JevBench v1 - Jevクラスの型付き意思決定モデルのためのベンチマーク：スマート、低コスト、高速、信頼性が高い、オープン.
...
- [backmeupplz/jev_antispam_bot](https://github.com/backmeupplz/jev_antispam_bot) - TypeSafe Jevを利用する、最小限のgrammY Telegramアンチスパムボット。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/surbhit20/jev-plays-snake/main/jev-plays-snake.gif" width="100%" alt="surbhit20/jev-plays-snake animation"><br><sub>アニメーション記録</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/yinsongxu--llm2jev/9b0b042b0dfa87b7.gif" width="100%" alt="Yinsongxu/LLM2Jev animation"><br><sub>アニメーション記録 · <a href="https://raw.githubusercontent.com/Yinsongxu/LLM2Jev/main/assets/mujoco.mp4">動画を開く</a></sub></td>
</tr></table>
...
- [Argos1111/jev_local](https://github.com/Argos1111/jev_local) - ローカルLLMでJevを再現。
- [yushen100/wechat-jev-assistant](https://github.com/yushen100/wechat-jev-assistant) - Windows WeChat会話分析アシスタント：ローカルで読み取り、匿名化し、TypeSafe Jevの判断と暗号化された履歴を処理。
...
- [phyous/tsai-sc](https://github.com/phyous/tsai-sc) - TypeSafe Jevが、記録された行動確率に基づき、キーボードとマウスでオリジナルのStarCraftシェアウェアを操作します.
...
<sub>生成ツール · `render.py` · 2026-09-23T09:31:07+08:00</sub>
