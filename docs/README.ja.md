...
> [!NOTE]
> **ライブインデックス** · 最終同期: `2026-09-23T20:09:57+08:00` (UTC+8)
> · エントリ数: **770** · 今回の追加: **76** · 実装言語: **22**
...
<sub>パターンとして書けない質問をgrepします。いくつかをサンプリングするのではなく、すべての関数を読み取ります。Jevと、TypeSafe AIのSystem Oneモデルを利用。</sub>
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
- [エージェントツール：MCP、フック、ゲート、コーディングエージェント](#エージェントツールmcpフックゲートコーディングエージェント) — **231**
- [ルーティング、ガードレール、承認](#ルーティングガードレール承認) — **57**
...
- [記事、ディスカッション、類似リスト](#記事ディスカッション類似リスト) — **97**
...
<sub>🔧 コード内での使用を確認: `src/typesafe_sdk/__init__.py`, `src/typesafe_sdk/_core/retry.py`, `src/typesafe_sdk/_core/config.py`</sub>
...
TypeSafe AIのJev向けカスタムコーディングハーネス：LLMが提案し、Jevが狭い質問に答え、コードが判断し、すべてのステップに証跡を残します。
...
ConvAI InnovationsのOSS Layaシステムの単一モデルをラップするElixirライブラリ
...
パターンとして書けない質問をgrepします。いくつかをサンプリングするのではなく、すべての関数を読み取ります。Jevと、TypeSafe AIのSystem Oneモデルを利用。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/clouatre-labs--decisions-judge-mcp/c8c00f2b58774857.gif" width="100%" alt="clouatre-labs/decisions-judge-mcp animation"><br><sub>アニメーション記録</sub></td>
</tr></table>
...
すべてのロボット向けの1つのオフボードCLI。ロボットを接続して指示を出し、協調して動作させられます。それぞれが頭脳としてLLM（Claude、OpenAI、Gemini、Grok、またはOllamaやvLLM経由のローカルモデル）を備え、より安価で高速な処理のための意思決定LLM（Jev、Laya、Kev）も利用します。Microduck、Open Duck Mini、LeRobot、XLeRobot、AlohaMini、ToddlerBot、Jetson、または任意のROSに対応。
...
Swift SDK：iPhoneおよびAppleのCore AIを搭載したMac上でチャット、ビジョン、音声モデルを実行するためのもの。モデルのダウンロードとキャッシュ、FoundationModelsとの統合、OS、SDK、モデル要件を記載した実行可能なサンプル。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/john-rocky--coreai-kit/c98c7bb8cb04e76e.gif" width="100%" alt="john-rocky/coreai-kit animation"><br><sub>アニメーション記録</sub></td>
</tr></table>
...
R.A.I.N. Labは、高速なローカル判断、独立した確率的評価、マルチエージェントの熟議、証拠、認可を個別の計算レイヤーに分離する実験的な科学エージェントアーキテクチャです。🐙（KarpathyのAutoResearchより前）
...
jev-seo：コーディングエージェント向けのRust SEOおよびGEO CLIとMCPサーバー：50ルールの監査、ライブクロール、GEOスコア、順位変動、CIゲート。MIT、サブスクリプション不要。
...
読み取り専用のトレーディングジャーナルおよびレビュー用ハーネス：Jev 型付き判断、エージェント統合、再現可能な金融ベンチマーク。注文なし、助言なし。
...
<details>
<summary><b>このカテゴリのその他</b> <sub>· 110</sub></summary>
...
- [agent-chaperone/agent-chaperone](https://github.com/agent-chaperone/agent-chaperone) - AIエージェントのツール呼び出しを実行前に、ツール結果をエージェントが読む前にスクリーニングします.
...
- [9pings/notjev](https://github.com/9pings/notjev) - 超高速なJev風サーバー。モデル非依存で、OpenAI互換のあらゆるエンドポイントで動作。
- [keltokhy/jsort](https://github.com/keltokhy/jsort) - 意味でソート：TypeSafeのJevモデルが判定したペアワイズ比較に基づき、平易な英語の次元に沿って行を並べ替え。
- [bladedevoff/stuntd](https://github.com/bladedevoff/stuntd) - アプリの型付きLLM意思決定を学習し、Layaヘッドで回答するローカルプロキシ。JevおよびOpenAI互換.
- [talmago/fast_gliner](https://github.com/talmago/fast_gliner) - Rust で書かれた GLiNER モデル用 Inference エンジンへの Python バインディング。
- [frostney/clean-code-review](https://github.com/frostney/clean-code-review) - プルリクエスト内のすべてのコードファイルを、TypeSafe の Jev によって Uncle Bob の Clean Code に照らして判定し、その後…
- [Qew7/jev-feels](https://github.com/Qew7/jev-feels) - 通常のRubyとして扱うセマンティックな意思決定。Jevによる#feels?、#decide、#score、Railsバリデーション、パターンマッチング。
...
- [tontoko/jev-browser](https://github.com/tontoko/jev-browser) - 1つのグラウンデッドなJev/Playwrightコア：型付きSDK、永続的なCLI、ネイティブブラウザー操作と決定論的アサーションを備えたMCPサーバー.
- [docxology/daf-jev](https://github.com/docxology/daf-jev) - daf-jev：TypeSafeのJev（System One）判定API向けの、組み合わせ可能なPythonツールキット…
- [KiritoKing/midscene-jev-runner](https://github.com/KiritoKing/midscene-jev-runner) - コミュニティ管理のMidscene Test向けJEVランナー統合。
...
- [vercel-labs/jev-ai-sdk-form-router](https://github.com/vercel-labs/jev-ai-sdk-form-router) - フォーム送信をJevとAI SDKで適切な担当者にルーティングします.
- [ansidium/jev-codex-bridge](https://github.com/ansidium/jev-codex-bridge) - Codex DesktopとCLI向けのモデルおよび推論ルーティング、Windowsサービスと検証済みアップデート付き。
...
- [kitfunso/hippo-memory](https://github.com/kitfunso/hippo-memory) - AIエージェントのための生物学に着想を得たメモリ。減衰、検索強化、統合。ランタイム依存関係ゼロ、SQLite、MCP.
...
- [nylon-memory/NylonME](https://github.com/nylon-memory/NylonME) - AI エージェント向けのメモリエンジン。Jev と連携することで、より高い意思決定精度をもたらせます.
...
意思決定モデルシステムの発見、構築、評価、改善のためのエージェントスキル。構成的意思決定計算、評価ハーネス、Software 3.0向けの制約付きプロンプト／プログラム山登り法。TypeSafe Jevがデフォルトのホスト型実例です。TypeSafeから独立しています。
...
<td align="center" valign="top"><a href="https://youtu.be/ZC4oge4WsCQ"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/24601--augustus/d2593af47d538d33.jpg" width="100%" alt="video"></a><br><sub><a href="https://youtu.be/ZC4oge4WsCQ">視聴先 youtu.be</a> · 再生はホストサイト側で開きます。GitHub にはインラインで埋め込めません</sub></td>
</tr></table>
...
<details>
<summary><b>このカテゴリのその他</b> <sub>· 209</sub></summary>

- [ChenneyZhuang/laya-browser-agent](https://github.com/ChenneyZhuang/laya-browser-agent) - ローカルで動作するオープンソースのJev代替：自分のマシン上でLaya（System Oneモデル）を使ったブラウザーエージェントの意思決定.
...
- [kydlikebtc/awesome-jev](https://github.com/kydlikebtc/awesome-jev) - Jevの検証済み例805件 — TypeSafe AIのSystem One意思決定モデル…
- [Amal-David/awesome-jev](https://github.com/Amal-David/awesome-jev) - Jevのデモ、プロジェクト、SDKsとスキル、ソースリンクと厳選されたXギャラリー付き.
...
- [socai-io/jev-social](https://github.com/socai-io/jev-social) - Instagram、TikTok、LinkedIn調査向けのJev ×…
...
- [PromptEngineer48/laya-vs-jev-arena](https://github.com/PromptEngineer48/laya-vs-jev-arena) - Laya（オープンソース、ローカル）対TypeSafe…
...
- [jiawei686/jev-ultrafast-mcp](https://github.com/jiawei686/jev-ultrafast-mcp) - ブラウザー作業を任せられます：エージェントのために意思決定モデルがページを操作するMCPサーバーです.
...
- [rashedInt32/jev-mcp](https://github.com/rashedInt32/jev-mcp) - 型付きで校正された判断ツールとしてTypeSafe Jevを公開するMCPサーバー。分類、スコアリング、チェック、バッチ形式の質問に対応.
- [ZephyrDeng/ego-jev](https://github.com/ZephyrDeng/ego-jev) - ego-browser向けのJev（TypeSafe System One）インナーループ.
- [green-dalii/pi-shift-router](https://github.com/green-dalii/pi-shift-router) - Piコーディングエージェント向けのターンごとのモデルルーティング。小型の判定器が各メッセージに対して低コスト層または高性能層を選択し、複数モデルのフェイルオーバ…
- [ismaelsoilet/jev-harness](https://github.com/ismaelsoilet/jev-harness) - 依存関係ゼロのSystem…
...
- [jiangkoumo/ego-jev](https://github.com/jiangkoumo/ego-jev) - Jev（TypeSafe System One）でego…
...
- [itsmostafa/typesafe-mcp](https://github.com/itsmostafa/typesafe-mcp) - あらゆるものを高速かつ低コストで評価するためのmcpコネクタ。AIエージェントにtypesafe aiのjevモデルへの直接アクセスを提供します.
...
非自己回帰型の System 1 意思決定エンジン。任意のテキストに対して、単一のフォワードパスで、100+ 言語に対応し、リクエストごとに適切なチェックポイントを選択するルーターを備え、型付きの選択、スコア、はい/いいえの判断を行います。

<sub>🔧 コード内での使用を確認: `research/benchmarks/feishu_zh/run.py`</sub>
...
Jev、TypeSafeのSystem Oneモデルが、あらゆるOpenRouter LLM、Stockfish、そしてあなたとチェスを対局します。ライブの指し手、Jevの指し手の確率、保存済みゲーム、勝率を備えた1ページのWebアプリです。
...
学習済みポリシーなしのClash Royaleボット：Jev（TypeSafe System One）は、ライブのゲーム状態からあらゆる決定を行う
...
Jevまたは任意のLLMでX.comフィードをフィルタリング
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/tshmieldev--sharp/016860f6bb08aa6b.gif" width="100%" alt="tshmieldev/sharp animation"><br><sub>アニメーション記録</sub></td>
</tr></table>
...
AI評価のための検証レイヤー。スコアだけでなく、計測器、データ、スコアラー、実行、数値、主張、そして自身をチェックします。
...
- [zeeshan8281/slo-router](https://github.com/zeeshan8281/slo-router) - Jev決定、ライブキューメトリクス、反事実評価、再現可能なレイテンシー/コストベンチマークを備えたSLO対応LLM推論ルーター。
...
- [Trystan-SA/laya-candle](https://github.com/Trystan-SA/laya-candle) - Layaを使用したJEV風のRust移植版、100%ローカル、Pythonなし.
...
任意のLLMをJev形式の意思決定モデルに変換：型付きの意思決定、現実的な確率、トレーニング不要。（更新を継続中）
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/nokia-applied-research--anyjev/93e61c3de618cc36.gif" width="100%" alt="nokia-applied-research/AnyJev animation"><br><sub>アニメーション記録</sub></td>
</tr></table>
...
Jevスタイルの意思決定モデル向け、オープンな学習・推論スタック。共有プレフィックスから動的な候補分岐をスコアリングするようモデルを学習でき、高カーディナリティの選択、キャリブレーション、高速なバッチ推論に対応。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/weitianxin--jevany/05825e369317a429.gif" width="100%" alt="weitianxin/JevAny animation"><br><sub>アニメーション記録</sub></td>
</tr></table>
...
毎朝、Jevに新着arXiv論文をすべて読ませ、読むべき少数の論文を浮かび上がらせます。平易な英語での関心、較正済み確率、1日約$0.06、フォークしてすぐに実行。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/ankit-aglawe--tinyjev/bff87199840daa0f.gif" width="100%" alt="ankit-aglawe/tinyjev animation"><br><sub>アニメーション記録</sub></td>
</tr></table>
...
- [ARCJ137442/jev-2048](https://github.com/ARCJ137442/jev-2048) - すべての手がJev（TypeSafe AI System One）Choiceとなる計測対応の2048ウェブラボ.
...
- [fstandhartinger/jevbench](https://github.com/fstandhartinger/jevbench) - JevBench v1 - Jevクラスの型付き意思決定モデル向けベンチマーク：スマート、低コスト、高速、信頼性が高い、オープン.
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/fbddcz--embodied-jev/297ef7c24f9a67a8.gif" width="100%" alt="FBddcz/embodied-jev animation"><br><sub>アニメーション記録 · <a href="https://raw.githubusercontent.com/FBddcz/embodied-jev/main/docs/media/libero-drawer-comparison.mp4">動画を開く</a></sub></td>
</tr></table>
...
TypeSafe の Jev API と互換性のある、ローカルでオフラインの System One サーバー。小規模なオープンモデルを使って、型付きのはい/いいえ、カテゴリ、スコアの質問に回答します。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/bl888m/jev-bot/main/assets/demo.gif" width="100%" alt="bl888m/jev-bot animation"><br><sub>アニメーション記録</sub></td>
</tr></table>
...
Jev、すなわちTypeSafeのSystem Oneモデルで人々が構築したものを集めた、最も充実したギャラリー。シナリオ別に3,400件以上のプロジェクト、デモ、解説を収録し、それぞれに元のリンク、画像、説明があります。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/darthblanc/tictacjev/main/assets/screenrecording.gif" width="100%" alt="darthblanc/tictacjev animation"><br><sub>アニメーション記録</sub></td>
</tr></table>
...
- [heyjunpenn/awesome-jev](https://github.com/heyjunpenn/awesome-jev) - Jev で構築された 832 件のオープンソースプロジェクトの、検証済みでコミュニティにより保守されているカタログ.
...
- [jaibhasin/jev-yt-time-saver](https://github.com/jaibhasin/jev-yt-time-saver) - Jevで気を散らすYouTube動画を覆い隠すChrome拡張機能。望むときはいつでも表示できます.
...
<details>
<summary>📰 <b><a href="https://typesafe.ai/team">TypeSafeにTeamページを追加 + ナビゲーションに`Enterprise`の入口を追加（プロダクト化・商業化のシグナル）</a></b> · ✅ official · 4 天</summary>
...
<details>
<summary><b>このカテゴリのその他</b> <sub>· 85</sub></summary>
...
- [Show HN: Will Jev pull the lever in the trolley problem?](https://news.ycombinator.com/item?id=49773756) - 皆さんが待ち望んでいたJevベンチマーク：Jevはトロッコ問題でレバーを引くのか?
...
ワンパスの型付き意思決定のためのオープンソース高性能System One Model。組み込み可能なCランタイムとWebAssemblyサポートを備えています。
...
微信（Windows 4.x）に横付けする返信アシスタント：ウィンドウのスクリーンショット + ローカルオフラインの OCR が相手のメッセージを読み取る → Jev が意図を判断 → 3つの候補からワンクリックで入力。送信は常に手動
...
微信メッセージの意図認識フローティングウィンドウ（macOS）：画面を見て、ローカルの小型モデルが意図とリスクを判断し、話法に基づいて返信候補を生成。完全な読み取り専用で、微信には注入しません。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/jev-chat--jev-chat-jarvis-mac/daa9ab5c1715d764.gif" width="100%" alt="jev-chat/jev-chat-jarvis-mac animation"><br><sub>アニメーション記録</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/zefan-cai--open-jev/f6f2903828782e64.gif" width="100%" alt="Zefan-Cai/Open-Jev animation"><br><sub>アニメーション記録 · <a href="https://zefan-cai.github.io/open-jev/v1-1/replay.mp4">動画を開く</a></sub></td>
</tr></table>
...
Laya の構造化意思決定モデル向けのセルフホスト型 API と Web インターフェースで、TypeSafe Jev API 形式と互換性があります。
...
<details>
<summary><b>このカテゴリのその他</b> <sub>· 58</sub></summary>
...
<sub>生成ツール · `render.py` · 2026-09-23T20:09:57+08:00</sub>
