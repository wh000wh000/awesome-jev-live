...
<p align="center"><b>每兩小時自我重建的 Jev 證據分級索引。</b></p>
...
> [!NOTE]
> **即時索引** · 上次同步: `2026-09-21T09:22:58+08:00` (UTC+8)
> · 條目: **673** · 本輪新增: **176** · 實作語言: **24**
...
<sub>每個領域取一名，依證據等級與 Star 數排名，每次同步重新計算。這是排名而非推薦；每個精選都連到下方的完整條目。優先選擇發佈了截圖或錄影的專案，以保證版面可視化。</sub>
...
<sub>由 TypeSafe AI（Jev System One 模型）提供支援的自動化資料庫遷移安全審查器</sub>
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
- [Agent 工具鏈：MCP、鉤子、閘門與編碼 Agent](#agent-工具鏈mcp鉤子閘門與編碼-agent) — **191**
- [路由、護欄與核准](#路由護欄與核准) — **62**
- [評估、校準與基準測試](#評估校準與基準測試) — **67**
- [開放重現、權重與架構研究](#開放重現權重與架構研究) — **28**
- [應用程式、遊戲、機器人與互動示範](#應用程式遊戲機器人與互動示範) — **52**
- [文章、討論與同類清單](#文章討論與同類清單) — **83**
- [其他專案](#其他專案) — **65**
...
<sub>🔧 於程式碼中發現使用: `src/typesafe_sdk/__init__.py`, `src/typesafe_sdk/_core/retry.py`, `src/typesafe_sdk/_core/config.py`, `src/typesafe_sdk/_core/logging.py`</sub>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/leddoengano--jev-eyes/45053b83afa9c677.gif" width="100%" alt="LeddoEngano/jev-eyes animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/shiftynick--jev-axi/e97a8238f41de3c1.gif" width="100%" alt="shiftynick/jev-axi animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
pull request 中的每個程式碼檔案，先由 TypeSafe 的 Jev 依 Uncle Bob 的 Clean Code 進行判斷，再由 Luna 審查。建立於 eve 和 Next.js 之上。
...
用於 TypeSafe AI 的 Jev 模型的命令列工具。針對任何文字提出是非題、選擇題與評分規準問題，並取得校準後的機率。答案會成為 shell 與 CI 的結束碼、指令碼的 JSON，以及 AI 代理程式的 MCP 工具。
...
Aside 代理使用 TypeSafe Jev（System One：Choice/Score/Noul）進行決策。這不是 Cua 綁定——Jev 是模型，Aside 是瀏覽器執行環境。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/himomohi--aside-jev/fd286962191e9c70.gif" width="100%" alt="himomohi/aside-jev animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
用 Jev 把飛書工作訊息分成四類：緊急、待辦、值得看、暫時略過。一個極簡、本機優先的訊息分類器，具備可擴充的工作區介面卡。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/ibrahemid--git-jev-stage/f8c136a32610d69a.gif" width="100%" alt="ibrahemid/git-jev-stage animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
由 TypeSafe AI 的 System One 模型 Jev 以機率判定三大雲端的功能名稱之示範
...
使用 TypeSafe AI 的 Jev 對 JSON 搜尋結果進行重新排序、篩選與壓縮。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/brnyxx--jev-ra/1f7ddcd1053825a2.gif" width="100%" alt="brnyxx/jev-ra animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/joacomarc--jev-harness-router/c134e4f34b4076d7.gif" width="100%" alt="JoacoMarc/jev-harness-router animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/bcivitcioglu/jev-chess/main/docs/demo.gif" width="100%" alt="bcivitcioglu/jev-chess animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/clueless-creations--jev-ios-ultrafast/24bc4b379cd7fe3a.gif" width="100%" alt="Clueless-Creations/jev-ios-ultrafast animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
- [juspay/neurolink](https://github.com/juspay/neurolink) - 一個 TypeScript 介面，支援三種推論類型的 40 個 AI 供應商——generate、stream 和 decide。Decide 透過。
...
- [win4r/jev-skill-suggester](https://github.com/win4r/jev-skill-suggester) - 使用 TypeSafe Jev 推薦已安裝 Skill / 使用 TypeSafe Jev 的有界已安裝 Skill 推薦。Python CLI、Codex。
...
- [carldaws/hunch](https://github.com/carldaws/hunch) - 用於 Ruby 和 Rails 的機率式控制流程——由 TypeSafe 的 Jev 驅動。
...
- [qkal/Canny](https://github.com/qkal/Canny) - 阻止 AI coding agents 在沒有證據的情況下聲稱工作已完成。決定性 hooks 做出決策，TypeSafe 的 Jev。
- [Stumble/jev-go](https://github.com/Stumble/jev-go) - 適用於 TypeSafe AI Jev / System One 的社群 Go SDK。
- [virolea/jev](https://github.com/virolea/jev) - 用於型別安全 AI Jev 模型的 Ruby 用戶端。
...
- [Brainwires/jevwire](https://github.com/Brainwires/jevwire) - 適用於代理的 Jev 決策層：MCP 伺服器、可嵌入的 DecisionModel 函式庫，以及僅升級用的 Claude Code 外掛程式（TypeSafe。
...
- [kushals256/jevcache](https://github.com/kushals256/jevcache) - 當 TypeSafe Jev 判定為相同意圖時，跳過昂貴的 LLM 呼叫。相容 OpenAI 的本機快取代理——npx。
...
<sub>🔧 於程式碼中發現使用: `src/request.ts`, `README.md`</sub>
...
以評估優先的 MCP 伺服器，適用於 TypeSafe 的 Jev——一個回傳具型別判定（noul、choice、score）及機率，而非生成文字的 System One 模型。
...
說出來，你的 Mac 就會照做。基於 Jev 的電腦使用 harness，透過 Accessibility 讀取螢幕。快速，無需視覺模型
...
將 TypeSafe Jev 連接至 Grok Bot，作為低成本決策層——使用量閘門、技能範本、範例
...
一個能嗅出 AI 寫作跡象的散文 linter。零相依、可計數規則加上一個判斷模型。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/danrwilloughby--snifftest/39b2a26b93d5f1ec.gif" width="100%" alt="DanRWilloughby/snifftest animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
由 Jev 驅動的 Instagram、TikTok 與 LinkedIn 研究：具型別路由、真實瀏覽器證據、串流貼文卡片、影片擷取，以及附引用的 socai 報告。
...
用於 Claude Code 和 Codex 的上下文修剪代理：Jev 判斷哪些歷史仍然需要，以測量而非宣稱。POC 現在在這裡，即將進入 https://github.com/compozy/compozy
...
📡 全網最全 · 全球最完整的 Jev（TypeSafe AI System One）生態系追蹤器 — 220+ 個已記錄案例 · 108 筆按信心分級的項目 · 每 3 小時驗證並重新掃描 · 包含 API 存取指南
...
TypeSafe Jev 的開放重現：一個 150M 型別化決策引擎（在一次非自回歸傳遞中完成 noul/choice/score，具校準信心度）。0.697 對 Jev 的 0.727，校準效果好 2.5 倍，快 4 倍，免費。可在 Colab T4 上 30 分鐘內訓練完成。
...
展示 TypeSafe Jev 反射決策的多無人機自主實驗室，並可選擇使用 System 2 策略指導。
...
Claude Code / Codex / pi 外掛程式，會將不需要文字輸出的代理步驟交給 Jev（TypeSafe 的判斷模型）——實測 p50 約 230 毫秒，每 1,000 次判斷約 0.02 美元，並可透過具型別升級回傳至 LLM
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/shitianfang--jev-use/a2bc98b40ee62415.gif" width="100%" alt="shitianfang/jev-use animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
由 Jev 驅動的 DeepSeek Harness 決策層
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/buberlo--dsh-jev/68586a0081e30cd1.gif" width="100%" alt="buberlo/dsh-jev animation"><br><sub>動態錄影 · <a href="https://raw.githubusercontent.com/buberlo/dsh-jev/main/docs/assets/kubernetes-comparison.mp4">開啟影片</a></sub></td>
</tr></table>
...
將瀏覽器工作交接出去：一個由決策模型驅動頁面的 MCP 伺服器，讓一個流程只需一次工具呼叫，而不是每次點擊都消耗一輪。具備基於參照的元素表、經程式碼檢查的斷言，以及零模型的巨集重播。透過 CDP 與你的 Chrome 通訊。
...
受 TypeSafe 的 Jev 和 System One 啟發的開源本機決策模型——基於 Qwen3，為 NVIDIA DGX Spark 建構。
...
<details>
<summary><b>本分類更多項目</b> <sub>· 157</sub></summary>
...
- [walidboulanouar/awesome-jev-use-cases](https://github.com/walidboulanouar/awesome-jev-use-cases) - TypeSafe AI Jev 使用案例的 Awesome 清單：74 個依按讚數排名的示範、150+ 個 GitHub 儲存庫、限制、成本與 API。
...
- [anpicasso/hermes-jev-approvals](https://github.com/anpicasso/hermes-jev-approvals) - 讓 TypeSafe Jev 擔任 Hermes Agent 智慧指令核准的審核者。在 153 個真實指令上測得，速度快 8.7 倍，提示減少 4.4。
...
- [win4r/pi-jev-router](https://github.com/win4r/pi-jev-router) - 用於 Pi Coding Agent 的任務邊界模型路由，由 TypeSafe Jev 驅動。保守策略、精確快取與可觀測的故障轉移.
...
- [bojansandhaus/jev-decisions](https://github.com/bojansandhaus/jev-decisions) - Hermes（及其他 AI Agents）的 Jev Decisions Plugin：工具風險審查、人工作業核准建議、證據檢查與本地決策日誌.
...
- [wotai-dev/typesafe-jev-tools](https://github.com/wotai-dev/typesafe-jev-tools) - 一個 Claude Code 鉤子，會詢問你正在撰寫的決策是否真的需要模型。包含 TypeSafe Jev 與 Claude Haiku 4.5 的 149。
...
- [bhavikprit/reflex-ai](https://github.com/bhavikprit/reflex-ai) - ⚡ 通用 System-1 AI 執行環境與雙腦閘道。為代理程式提供低於 15 毫秒的校準決策（noul、choice、score）。可直接置換的。
...
- [Mawfyy/jevflow](https://github.com/Mawfyy/jevflow) - 將機率式 AI 決策作為可組合的後端原語——具型別判斷（noul/score/choice）、決定性閾值與可解釋工作流程。由 TypeSafe 的 Jev。
...
TypeSafe 的 Jev（System One 模型）應用於機械與電機工程的八個最小可運作範例：CAD/CAE/CAM 路由、FEM 結果分流、DFM 篩選、BOM 對齊、防幻覺擷取。零相依性。
...
由 TypeSafe AI 的 System One 模型 Jev 驅動的智慧路由層，位於多個 LLM 供應商之前，會分析每個傳入請求的複雜度，將其路由至能以可接受品質處理該請求的最低成本模型，並持續驗證路由決策是否正確。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/collapseindex/dinostomp/main/data/exports/readme/20260915_120000_readme_pixel-dino_1200x360_s42.gif" width="100%" alt="collapseindex/dinostomp animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/valentynkit--jev-plays-pokemon-red/01942a0e5d4a3c76.gif" width="100%" alt="valentynkit/jev-plays-pokemon-red animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
用語音和指向的手指在 tldraw 畫布上繪圖。Jev（TypeSafe System One）會在每個語音詞約 350 毫秒內決定動作、目標與位置。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/gaborishka--jev-canvas/15924c327747cb97.gif" width="100%" alt="gaborishka/jev-canvas animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
使用 TypeSafe Jev 進行互動式實驗，從支援路由到具備真實 AI 決策與可見感測器輸入的 3D 駕駛模擬。
...
- [iefnaf/pi-jev](https://github.com/iefnaf/pi-jev) - 由 Jev 驅動的 Pi 擴充套件組：選擇性內容壓縮與模型路由。
- [scienthoon/jev-ood-calibration](https://github.com/scienthoon/jev-ood-calibration) - 對 TypeSafe 的 Jev 進行獨立校準測試，測試其不可能見過的任務：900 個由規則生成的支援工單（choice / score /。
...
<sub>🔧 於程式碼中發現使用: `docs/JUDGMENT_SPEC.md`</sub>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/daisuke7--jevlergy/b3d0e2fac97564b5.gif" width="100%" alt="daisuke7/jevlergy animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
用於在 RAG 中測量 Jev 重排序品質、延遲與成本的可重現基準測試
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/zwliJay/jev-forge/main/docs/assets/jevforge-demo.gif" width="100%" alt="zwliJay/jev-forge animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/mintannn--jev-asks-until-sure/ed17c81bdf0505ce.gif" width="100%" alt="mintannn/jev-asks-until-sure animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
TypeSafe 的 Jev 對比 gpt-5.4-mini 與 gpt-5.6-luna，使用四組公開分類資料集：案例、逐項答案、評分、圖表
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/robokrunch--jev-physical-ai/5199aa2bddea3bba.gif" width="100%" alt="robokrunch/jev-physical-ai animation"><br><sub>動態錄影 · <a href="https://github.com/robokrunch/jev-physical-ai/raw/main/assets/fleet-triage-demo.mp4">開啟影片</a></sub></td>
</tr></table>
...
測量 TypeSafe 的 RLCD-Jev 模型從檔案片段中找出真實秘密憑證的能力
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/tianyucodings--nanojev/da3528699bc28423.gif" width="100%" alt="TianyuCodings/NanoJev animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/louisultra--jev-deep-dive/5ddfd6ac08d9e5fd.gif" width="100%" alt="LouisUltra/jev-deep-dive animation"><br><sub>動態錄影 · <a href="https://raw.githubusercontent.com/LouisUltra/jev-deep-dive/main/assets/side_by_side_maze.mp4">開啟影片</a></sub></td>
</tr></table>
...
Laya 具型別決策模型的原生 MLX 執行階段——在 M3 Max 上進行 7–14 毫秒的短決策。不需要文字生成、PyTorch 或雲端 API。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/mizorewww--laya-mlx/18b82d54b656b475.gif" width="100%" alt="mizorewww/laya-mlx animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://i.imgur.com/wHM3jxV.gif" width="100%" alt="ekzhang/openjev-sglang animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/fbddcz--embodied-jev/67a9b52be44f8235.gif" width="100%" alt="FBddcz/embodied-jev animation"><br><sub>動態錄影 · <a href="https://github.com/FBddcz/embodied-jev/raw/refs/heads/main/docs/media/jev-hierarchical.mp4">開啟影片</a></sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/shuhansun--jev-oas-sentinel/640e046723eb6be6.gif" width="100%" alt="ShuhanSun/jev-oas-sentinel animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
<details>
<summary><b>本分類更多項目</b> <sub>· 10</sub></summary>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/zadescoxp--jev-trades/a11bc2e9272ed726.gif" width="100%" alt="zadescoxp/Jev-Trades animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/x--tspy--2100864234523685146/b80cf3173f63bdd7.gif" width="100%" alt="Xtags — X intent labeller animation"><br><sub>動態錄影 · <a href="https://video.twimg.com/amplify_video/2100858340331200512/vid/avc1/1242x720/ex2FF5-TerVxo9xX.mp4">開啟影片</a></sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/jkudish--jev-browser/d9b7631c25d7db06.gif" width="100%" alt="jkudish/jev-browser animation"><br><sub>動態錄影 · <a href="https://raw.githubusercontent.com/jkudish/jev-browser/main/assets/github-demo.mp4">開啟影片</a></sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/mrnugget/jev-shell-history/main/demo/demo.gif" width="100%" alt="mrnugget/jev-shell-history animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
<td align="center" valign="top"><a href="https://www.youtube.com/watch?v=QbYBRjOaGOo"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/w3cj--jev-chat/026f32ee3ff793cf.png" width="100%" alt="video"></a><br><sub><a href="https://www.youtube.com/watch?v=QbYBRjOaGOo">觀看平台 youtube.com</a> · 播放會在主機網站開啟；GitHub 無法內嵌播放</sub></td>
</tr></table>
...
- [atarikcaliskan/jevball](https://github.com/atarikcaliskan/jevball) - 22 個 Jev 模型，一顆球：一場 3D 足球比賽，每位球員都是自己的 Jev（TypeSafe AI System One）決策。觀看比賽，或接管 9。
...
- [realZachi/typesafe-adblock](https://github.com/realZachi/typesafe-adblock) - 🧹 有趣專案：一個 Chrome 擴充功能，會詢問微型 AI 決策模型（TypeSafe Jev）「這個 DOM。
...
<details>
<summary>📰 <b><a href="https://typesafe.ai/team">TypeSafe 新增 Team 頁面 + 導覽新增 `Enterprise` 入口（產品化／商業化訊號）</a></b> · ✅ official · 2 天</summary>
...
- [Show HN: CUA-S1 – A System One Model for Computer Use](https://news.ycombinator.com/item?id=49767564) - 你好，HN！我們是 Cua 的 Dillon 和 Francesco。我們想知道，有多少電腦使用任務實際上需要完整的通用 LLM（例如。
- [Open-sourced jev architecture last year with model,paper and dataset](https://news.ycombinator.com/item?id=49736660) - 現在大家都在談論這種架構——它不是自回歸式的，並且能使用 json schema 以極快速度進行機率預測。我早在 2025 年 3。
...
- [Show HN: Will Jev pull the lever in the trolley problem?](https://news.ycombinator.com/item?id=49773756) - 你們一直等待的 Jev 基準測試：Jev 會在電車難題中拉下操縱桿嗎？別想太多 :) 在軌道兩側放上任何你想放的東西，GPU（我新創公司的 AI。
...
- [Show HN: Jev, Fly Me to the Moon](https://news.ycombinator.com/item?id=49769916) - 我建立了 Odyssey，這是一段從地球前往月球軌道、以瀏覽器為基礎的旅程。 它使用 Jev 的 TypeSafe。
...
- [rhc98/awesome-jev](https://github.com/rhc98/awesome-jev) - 建構於 Jev（TypeSafe AI 的 System One 模型）之上的專案，由 Jev 本身策展.
...
<sub>🔧 於程式碼中發現使用: `jev_ultrafast/model.py`</sub>
...
我們每十分鐘都會詢問 Jev（TypeSafe AI 的 System One 模型）AI 是否應該殺死我們所有人。使用實際的新聞標題。
...
JEV 文件分類可運用 AI，借助 TypeSafe 的「System One」模型，快速且具成本效益地分類文字型文件。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/miladbahariqaragoz--highwayenv-but-jev-is-driving/e7ca8bffaba5f55e.gif" width="100%" alt="MiladBahariQaragoz/HighwayEnv-But-Jev-is-Driving animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/Schweik7/jev-pacman/main/demo_greedy_turn.gif" width="100%" alt="Schweik7/jev-pacman animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/uhhfeef/jev-lm/main/docs/session-whats-up.gif" width="100%" alt="uhhfeef/jev-lm animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
TypeSafe Jev 透過鍵盤和滑鼠控制原版 StarCraft shareware，並記錄動作機率。
...
OpenThai-SystemOne：開放 Thai + English System One 決策模型（0.8B、256 路 slot head、Apache-2.0）
...
- [deepanwadhwa/OpenDecision](https://github.com/deepanwadhwa/OpenDecision) - OpenDecision 是類似 typesafe jev 的開源語意決策引擎.
...
<sub>產生工具 · `render.py` · 2026-09-21T09:22:58+08:00</sub>
