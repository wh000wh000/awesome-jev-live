...
> [!NOTE]
> **即時索引** · 上次同步: `2026-09-19T22:33:29+08:00` (UTC+8)
> · 條目: **545** · 本輪新增: **56** · 實作語言: **25**
...
- [社群客戶端、SDK 與轉接器](#社群客戶端sdk-與轉接器) — **85**
- [Agent 工具鏈：MCP、鉤子、閘門與編碼 Agent](#agent-工具鏈mcp鉤子閘門與編碼-agent) — **178**
- [路由、護欄與核准](#路由護欄與核准) — **62**
- [評估、校準與基準測試](#評估校準與基準測試) — **46**
- [開放重現、權重與架構研究](#開放重現權重與架構研究) — **25**
- [應用程式、遊戲、機器人與互動示範](#應用程式遊戲機器人與互動示範) — **45**
- [文章、討論與同類清單](#文章討論與同類清單) — **40**
- [其他專案](#其他專案) — **58**
...
- [blazskufca/typesafe-sdk-go](https://github.com/blazskufca/typesafe-sdk-go) - Typesafe.ai 在 Go 中的社群 SDK。
...
- [frodi-karlsson/jev-client](https://github.com/frodi-karlsson/jev-client) - 用於以符合人體工學的方式存取 Jev API 的 TypeScript 用戶端。
...
- [Gilbert09/jev-cli](https://github.com/Gilbert09/jev-cli) - 由 Jev 驅動、用於 Claude Code 的判斷層：語義權限閘控、提示注入篩選、完成內容驗證與語義排名.
- [loicrg/jev-spam-lab](https://github.com/loicrg/jev-spam-lab) - 用於使用 TypeSafe AI 的 Jev 模型評估 Microsoft Outlook 電子郵件的 CLI.
...
- [wsmoak/jev-sales-calls](https://github.com/wsmoak/jev-sales-calls) - 範例：TypeSafe AI 的 Jev 模型根據產品功能庫讀取銷售通話轉錄內容。
...
由 TypeSafe Jev 1.13 非自回歸決策模型驅動的 Claude Code 即時品質閘門與藝術總監 Warden
...
由 TypeSafe 為 Jev 提供的 n8n 社群輔助節點。使用你定義的問題來分類、路由及評分文字，並為每個答案取得機率，讓不確定的項目可以送交審查。
...
球每次依模型決策移動一步的 Pong。透過 Vercel AI Gateway 進行 Jev 對 LLMs，每位玩家及代理程式都位於 Ably 頻道中。
...
TypeSafe AI 的 Jev 對 GPT-5.6 Luna 對 Claude Sonnet 5 的支援工單分流（Vercel AI Gateway）
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/bunyod0987--jev-vs-llm/416153a496dc9e59.gif" width="100%" alt="BUNYOD0987/jev-vs-llm animation"><br><sub>動態錄影 · <a href="https://raw.githubusercontent.com/BUNYOD0987/jev-vs-llm/main/demo.mp4">開啟影片</a></sub></td>
</tr></table>
...
<details>
<summary><b>本分類更多項目</b> <sub>· 144</sub></summary>
...
- [imMamdouhaboammar/get-fable](https://github.com/imMamdouhaboammar/get-fable) - 讓你已經使用的模型透過更完善的規劃、持久化上下文、技能、掛鉤、失敗處理與驗證，運作得更像前沿模型。
...
- [willfish/pi-observational-memory-jev](https://github.com/willfish/pi-observational-memory-jev) - Jev 決定保留哪些內容。壓縮絕不會重寫轉錄內容.
...
- [chy4pro/JevBrowserExt](https://github.com/chy4pro/JevBrowserExt) - 使用 TypeSafe Jev（一個亞秒級決策模型）操作網頁的 Chrome 擴充功能。browser-use/jev-ultrafast 的 MV3 移植版.
...
- [dev-willbird1936/pi-jev-compact](https://github.com/dev-willbird1936/pi-jev-compact) - Pi Coding Agent 的逐字 Jev 上下文壓縮。
...
- [EthanThatOneKid/zocomputer-jev](https://github.com/EthanThatOneKid/zocomputer-jev) - 使用 Vercel AI Gateway 及 TypeSafe AI Jev 進行情境腳本撰寫與執行的 Zo 技能.
...
- [Heman10x-NGU/openJev-verdict-2.0](https://github.com/Heman10x-NGU/openJev-verdict-2.0) - 經校準的 151M 非自回歸決策引擎，在 LocalLLaMA/typed-decisions 上擊敗 TypeSafe Jev 與 Laya（77.10%。
- [iamdin/pi-jev-skill-bench](https://github.com/iamdin/pi-jev-skill-bench) - Pi 技能路由基準測試：在 50–500 的名冊大小下比較 BM25 與 TypeSafe Jev。
- [InfamousCube/JevPilot](https://github.com/InfamousCube/JevPilot) - 使用 TypeSafe Jev 進行電腦操作：透過提示控制你的 PC、瀏覽器、遊戲及 Android 手機。
...
- [kangshifu1/jev-computer-use](https://github.com/kangshifu1/jev-computer-use) - 適用於 Codex 與 Chrome/Chromium 的獨立 Jev 電腦操作技能，具有限定動作與經驗證的瀏覽器工作流程.
- [kangshifu1/jev-skills-market](https://github.com/kangshifu1/jev-skills-market) - 用於自動化測試、金融研究與語音工作流程的社群 Jev 技能市場與助理。Computer Use 是獨立的儲存庫.
...
- [LIFE-Electronic/bluehouse-jev-browser](https://github.com/LIFE-Electronic/bluehouse-jev-browser) - 使用 Jev 操控 Bluehoue 的 CDP 瀏覽器的技能。
...
- [luxus/ha-conversation-jev](https://github.com/luxus/ha-conversation-jev) - Home Assistant 自訂元件：使用 Jev 快速路徑與 Grok 備援的對話代理。
...
- [namayasai/backstage-jev-operations-support](https://github.com/namayasai/backstage-jev-operations-support) - 由 Jev 驅動的 Backstage 作業決策支援：準備度檢查、事件分流、變更審查、範本與團隊建議，以及目錄重新排序.
- [nickylin/jev-harness](https://github.com/nickylin/jev-harness) - 由 TypeSafe Jev 驅動的具型別代理決策控制平面。
...
- [phin-tech/pi-jev-approver](https://github.com/phin-tech/pi-jev-approver) - 由 TypeSafe 的 Jev 判斷模型支援的 Pi coding agent Shell 命令安全閘門。
...
- [shitianfang/jev-use](https://github.com/shitianfang/jev-use) - Claude Code / Codex / pi 外掛程式，將不需要文字輸出的代理步驟交給 Jev（TypeSafe 的判斷模型）——約 200。
...
- [szocpaul/jev-compaction-prime](https://github.com/szocpaul/jev-compaction-prime) - Prime Agent 的逐字、基於決策的上下文壓縮——不使用摘要，而是評分並捨棄過時的工具呼叫；所有保留內容都逐位元組保持不變.
...
- [zzsong1023/jev-market-reflex](https://github.com/zzsong1023/jev-market-reflex) - 使用 TypeSafe AI Jev 在即時加密貨幣市場上進行快速、具型別的 AI 決策.
...
為有用的證據騰出空間。可檢查的 RAG 情境選擇，搭配 Jev 重新排序及公開基準研究。
...
- [scarif-labs/jev-software-decision-benchmark](https://github.com/scarif-labs/jev-software-decision-benchmark) - 可重現的基準測試，在分布偏移下評估 JEV 作為依賴更新自動化軟體決策原語的表現.
...
- [leesk212/JEV-CPU](https://github.com/leesk212/JEV-CPU) - 在 CPU 上執行 SemIf（Jev 風格的語意 if 決策）— 無需 GPU。從開放模型的一次前向傳播中直接讀取類型化選項機率，另附網頁 UI.
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/aslamdoctor/react-chess-game-using-jev/master/demo/demo.gif" width="100%" alt="aslamdoctor/react-chess-game-using-jev animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
將 LLMs 轉換成類似 Jev 的機器的最佳化推論引擎：針對快速、輕量且準確的決策、分類及評分進行最佳化
...
- [robzolkos/omarchy-issue-classifier](https://github.com/robzolkos/omarchy-issue-classifier) - 使用 Jev（TypeSafe 的 System One 模型）分類 Omarchy。
...
- [Joymfl/dag-jev](https://github.com/Joymfl/dag-jev) - 透過 Jev 從無序項目建立 DAG。
...
<sub>產生工具 · `render.py` · 2026-09-19T22:33:29+08:00</sub>
