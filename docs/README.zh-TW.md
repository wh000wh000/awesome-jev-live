...
> [!NOTE]
> **即時索引** · 上次同步: `2026-09-23T12:34:14+08:00` (UTC+8)
> · 條目: **770** · 本輪新增: **69** · 實作語言: **24**
...
<sub>Jev（TypeSafe 的 System One 模型）會與任何 OpenRouter LLM、Stockfish 及你下棋。單頁網頁應用程式，具備即時棋步、Jev 的棋步機率、已儲存的棋局與勝率。</sub>
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
為你所有機器人準備的一個 offboard CLI。連接它們、指揮它們，並讓它們協同工作，每個都以 LLM 作為大腦，並以決策 LLM 實現更便宜且更快速的步驟。Microduck、Open Duck Mini、LeRobot、XLeRobot、AlohaMini、ToddlerBot 或任何 ROS base。Claude、OpenAI、Gemini、Grok，或透過 Ollama 或 vLLM 在本機執行。
...
100% 免費、₹0、代理程式優先的 SEO 與 GEO CLI 套件，以及以 Rust 建置的 MCP 伺服器，透過 DuckDuckGo 與 TypeSafe Jev System One https://akashpriyadarshii.github.io/jev-seo/ 取代 Semrush 和 OpenSEO
...
SDK 優先、獨立維護的 SemIf fork，用於快速、自行託管的語意決策。
...
<details>
<summary><b>本分類更多項目</b> <sub>· 115</sub></summary>
...
- [BeatAPI/awesome-jev](https://github.com/BeatAPI/awesome-jev) - 經過原始碼審查的 JEV 相關專案集，收錄超過 50 個 GitHub 顆星的專案——整合、工具、開放模型、實驗與生態系資源.
...
- [keltokhy/jgrep](https://github.com/keltokhy/jgrep) - grep，但模式是一段描述。使用 TypeSafe 的 Jev 決策模型依意義篩選行：約 200 ms，且每行成本為一美分的千分之一.
...
- [keltokhy/jsort](https://github.com/keltokhy/jsort) - 依意義排序：沿著純英文維度排列各行，依據由 TypeSafe 的 Jev 模型判定的成對比較.
- [tumf/jev-cli](https://github.com/tumf/jev-cli) - 適用於 TypeSafe Jev 的小型、無相依性 CLI。
- [evoke-build/evoke](https://github.com/evoke-build/evoke) - 憑直覺而生的軟體。說出來，正確的小程式就會執行：由經校準的分類器選擇，只有在足夠確定時才執行，且在任何無法復原的動作前都會先詢問.
- [oswarld/notion-skills](https://github.com/oswarld/notion-skills) - 為 Notion 韓語使用者打造的精準俐落技能。
...
- [ruban-24/switchboard](https://github.com/ruban-24/switchboard) - 適用於 Claude Code 和 Codex 的開放原始碼、模型無關決策路由器.
- [shinpr/jev-reranker](https://github.com/shinpr/jev-reranker) - 使用 TypeSafe AI 的 Jev，對 JSON 搜尋結果重新排序、篩選及壓縮.
...
- [compozy/yoshi](https://github.com/compozy/yoshi) - 用於 Claude Code 與 Codex 的上下文裁剪代理：Jev 判斷仍需要哪些歷史記錄，以實測而非宣稱為準.
...
- [shitianfang/jev-use](https://github.com/shitianfang/jev-use) - Claude Code / Codex / pi 外掛，會將不需要文字輸出的代理步驟交給 Jev（TypeSafe 的判斷模型）——測得 p50 約 230…
...
- [Alex314618-create/JevRev](https://github.com/Alex314618-create/JevRev) - LLM + Jev 工作流程。為你的脊椎動物大腦注入一條脊柱.
...
- [muratcakmak/jev-guard](https://github.com/muratcakmak/jev-guard) - 針對 Claude Code 的機率評分防護欄：拒絕違反規則的編輯和未經要求的部署，將文件內容導入每個提示，並根據該回合自身的證據檢查最終答案.
...
- [weitianxin/JevAny](https://github.com/weitianxin/JevAny) - 具校準意識的 RL 決策模型，擴展至複雜代理、多模態證據、長上下文與符號控制.
...
- [lucioamor/lovable-skills](https://github.com/lucioamor/lovable-skills) - 可重複使用的 Lovable 技能，適用於應用程式審查、決策、寫作，以及 jevify 執行階段 AI 稽核與 JEV 遷移.
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/storm-software--mindctl/69298c40a77b7be5.gif" width="100%" alt="storm-software/mindctl animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
流量邊緣的具型別判斷准入控制：由 TypeSafe Jev 驅動，適用於 nginx/OpenResty 的三層提示詞注入與濫用過濾器。失效時放行、具快取功能，並支援熱重載。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/arpit-khandelwal--jev-linkedin-slop-filter/2ba80db1b4af936f.gif" width="100%" alt="Arpit-Khandelwal/jev-linkedin-slop-filter animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
- [raihankhan-rk/diffjury](https://github.com/raihankhan-rk/diffjury) - DiffJury — TypeSafe Jev PR 風險路由器 + 程式碼審查教練。
...
開放的 System One 模型：具備校準機率的具型別決策，可使用你自己的資料進行訓練。不進行文字生成。
...
- [fstandhartinger/jevbench](https://github.com/fstandhartinger/jevbench) - JevBench v1 - Jev 級型別化決策模型的基準測試：聰明、便宜、快速、可靠、開放.
...
<details>
<summary><b>本分類更多項目</b> <sub>· 11</sub></summary>
...
- [heyjunpenn/awesome-jev](https://github.com/heyjunpenn/awesome-jev) - 由社群維護且經過驗證的目錄，收錄 640 個使用 Jev 建置的開源專案.
...
- [Kevthetech143/super-jev](https://github.com/Kevthetech143/super-jev) - 適用於 TypeSafe Jev 的小型、可擴充決策轉行動工具。
...
<sub>產生工具 · `render.py` · 2026-09-23T12:34:14+08:00</sub>
