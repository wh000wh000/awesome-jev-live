...
> [!NOTE]
> **即時索引** · 上次同步: `2026-09-22T01:56:49+08:00` (UTC+8)
> · 條目: **732** · 本輪新增: **99** · 實作語言: **24**
...
- [Agent 工具鏈：MCP、鉤子、閘門與編碼 Agent](#agent-工具鏈mcp鉤子閘門與編碼-agent) — **185**
- [路由、護欄與核准](#路由護欄與核准) — **73**
- [評估、校準與基準測試](#評估校準與基準測試) — **78**
- [開放重現、權重與架構研究](#開放重現權重與架構研究) — **40**
...
- [AkashPriyadarshii/jev-seo](https://github.com/AkashPriyadarshii/jev-seo) - 100% 免費 ₹0、agent-first 的 SEO 與 GEO CLI 套件，以及以 Rust 建置的 MCP 伺服器，透過 DuckDuckGo 和…
...
- [evoke-build/evoke](https://github.com/evoke-build/evoke) - 軟體，源於反射。一個句子會變成對小程式的呼叫，由 Jev、TypeSafe AI 的分類器選定，並且只在足夠確定時執行。反射是任何人都能撰寫、分享和改進的配方.
- [arunav25/jev-mcp](https://github.com/arunav25/jev-mcp) - 將 JEV 連接到 MCP 客戶端，並使用共享資料集與可衡量的準確度，將其判斷與通用 LLMs 進行比較.
- [chengyongru/fastjev](https://github.com/chengyongru/fastjev) - SDK 優先、獨立維護的 SemIf fork，用於快速、自行託管的語意決策.
- [himomohi/aside-jev](https://github.com/himomohi/aside-jev) - Aside 代理使用 TypeSafe Jev（System One：Choice/Score/Noul）進行決策.
- [lukstei/slop-grader](https://github.com/lukstei/slop-grader) - 由 Jev 驅動、基於規則的文字檔評分器。對每一行並行執行每一條規則。不略讀，不漏行.
- [Qew7/jev-feels](https://github.com/Qew7/jev-feels) - 將語意決策作為普通的 Ruby——feels?、decide、score、Rails 驗證與由 Jev 驅動的模式比對。
...
- [iamvatsalpatel/tiershift](https://github.com/iamvatsalpatel/tiershift) - 將每個 LLM 呼叫轉移到能處理它的最便宜模型。路由由 TypeSafe Jev 在約 180 ms 內決定。無需訓練資料。政策使用純 YAML.
...
- [kitfunso/hippo-memory](https://github.com/kitfunso/hippo-memory) - 為 AI agents 設計、受生物啟發的記憶。衰減、檢索強化、鞏固。零執行階段相依套件，SQLite，MCP.
...
- [neko233-com/laya-go](https://github.com/neko233-com/laya-go) - Laya — 開放的水平 System 1 決策模型：Go 伺服器 + 代理 CLI/MCP，取代 JEV（非自迴歸結構化預測）。
...
<sub>🔧 於程式碼中發現使用: `src/request.ts`</sub>
...
用於決策模型類別（分類器、編碼器/解碼器、專用 AR heads、System One）的 Agent skill。TypeSafe Jev 是主要範例。組合代數、問題設計、驗證閘門。MIT。
...
<details>
<summary><b>本分類更多項目</b> <sub>· 163</sub></summary>
...
- [OmniJev/PlayJev](https://github.com/OmniJev/PlayJev) - 🚀🚀 一個 0.8B 類 JEV 多模態模型，直接從原始像素遊玩 GUI 遊戲.
...
- [DECRUX9812/typesafe-skill-router](https://github.com/DECRUX9812/typesafe-skill-router) - TypeSafe (Jev) 為 Hermes Agent 進行技能路由：在模型呼叫之前，命名唯一值得載入的技能.
...
- [Nyarlathoteppppp/pi-jev-context](https://github.com/Nyarlathoteppppp/pi-jev-context) - 模型效能優先。Token 節省其次。一個 Pi 擴充功能，具備 freshness-aware 讀取去重、Jev 日誌篩選，以及可搜尋的逐字回憶.
- [buluoray/JevOnly](https://github.com/buluoray/JevOnly) - 能夠「打字」並驅動任務完成的純 Jev.
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/hfnissum-byte/jevmerge/main/demo/jevmerge.gif" width="100%" alt="hfnissum-byte/jevmerge animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
- [kavehmz/typesafe-playground](https://github.com/kavehmz/typesafe-playground) - 使用 TypeSafe Jev 進行互動式實驗，從支援路由到具備真實 AI 決策與可見感測器輸入的 3D 駕駛模擬.
...
- [yzfly/edgejev](https://github.com/yzfly/edgejev) - 離線可用的本地型別化決策：4 核 CPU 單題 15.6ms。CPU 上的本機與離線 Jev / System One 推論 — ONNX +…
...
- [fstandhartinger/jevbench](https://github.com/fstandhartinger/jevbench) - JevBench v1 - 針對 Jev 級型別化決策模型的基準測試：智慧、便宜、快速、可靠、開放.
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/fbddcz--embodied-jev/6c478d975fee036f.gif" width="100%" alt="FBddcz/embodied-jev animation"><br><sub>動態錄影 · <a href="https://raw.githubusercontent.com/FBddcz/embodied-jev/main/docs/results/metaworld-hierarchy-v2-2026-09-21/figures/hierarchy-grid-fast.mp4">開啟影片</a></sub></td>
</tr></table>
...
<details>
<summary>📰 <b><a href="https://typesafe.ai/team">TypeSafe 新增 Team 頁面 + 導覽新增 `Enterprise` 入口（產品化／商業化訊號）</a></b> · ✅ official · 3 天</summary>
...
微信訊息意圖識別懸浮窗（macOS）：看螢幕 + 本地小模型判斷意圖和風險，再按話術生成回覆候選。純唯讀、不注入微信。
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/jev-jarvis--jev-jarvis/daa9ab5c1715d764.gif" width="100%" alt="jev-jarvis/jev-jarvis animation"><br><sub>動態錄影</sub></td>
</tr></table>
...
- [HexyeDEV/JevPR](https://github.com/HexyeDEV/JevPR) - 由 Jev 自動化的 PR 風險審查。
...
- [kevin9327/jev-master](https://github.com/kevin9327/jev-master) - 使用 Jev 的具型別 System One 決策：在程式碼中組合 Choice + Score + Noul.
...
- [musubi-labs/musubi-jev](https://github.com/musubi-labs/musubi-jev) - 建立在 Qwen3.5 之上的小型類 Jev 決策模型家族，你可以自行訓練與執行。
...
<sub>產生工具 · `render.py` · 2026-09-22T01:56:49+08:00</sub>
