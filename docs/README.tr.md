<p align="center">
  <img src="../assets/readme/hero.png" width="100%" alt="Awesome Jev Live">
</p>

<h1 align="center">Awesome Jev Live</h1>

<p align="center"><b>Kendini her iki saatte bir yeniden kuran, kanıt derecelendirmeli Jev dizini.</b></p>

<p align="center">
  <a href="https://awesome.re"><img src="https://awesome.re/badge-flat2.svg" alt="Awesome"></a>
  <img src="https://img.shields.io/badge/entries-799-0d9488" alt="entries">
  <img src="https://img.shields.io/badge/languages-20-1f6feb" alt="languages">
  <img src="https://img.shields.io/badge/refresh-every%202h-16a34a" alt="refresh">
  <a href="../LICENSE"><img src="https://img.shields.io/badge/license-MIT-lightgrey" alt="MIT"></a>
  <a href="https://wh000wh000.github.io/awesome-jev-live/"><img src="https://img.shields.io/badge/site-searchable-1f6feb" alt="site"></a>
</p>

<p align="center"><sub><a href="../README.md">English</a> · <a href="README.zh-CN.md">简体中文</a> · <a href="README.zh-TW.md">繁體中文</a> · <a href="README.ja.md">日本語</a> · <a href="README.ko.md">한국어</a> · <a href="README.es.md">Español</a> · <a href="README.fr.md">Français</a> · <a href="README.de.md">Deutsch</a> · <a href="README.pt-BR.md">Português (Brasil)</a> · <a href="README.ru.md">Русский</a> · <a href="README.it.md">Italiano</a> · <a href="README.ar.md">العربية</a> · <a href="README.hi.md">हिन्दी</a> · <b>Türkçe</b> · <a href="README.vi.md">Tiếng Việt</a> · <a href="README.th.md">ไทย</a> · <a href="README.id.md">Bahasa Indonesia</a> · <a href="README.pl.md">Polski</a> · <a href="README.nl.md">Nederlands</a> · <a href="README.uk.md">Українська</a></sub></p>

> [!NOTE]
> **Canlı dizin** · Son eşitleme: `2026-09-25T18:10:37+08:00` (UTC+8)
> · Kayıt: **799** · Bu turda yeni: **298** · Uygulama dilleri: **24**

<sub>Aşağıdaki her kayıt bu depodaki iş akışı tarafından toplandı, filtrelendi ve yeniden kontrol edildi. Sayılar ve zaman damgaları elle yazılmış bir anlık görüntüden değil, kaynaklardan gelir.</sub>

<a id="featured"></a>

## 🔥 Günün seçkisi — her kategorinin birincisi

<sub>Her kategoriden bir kayıt; kanıt düzeyi ve yıldıza göre sıralanır, her eşitlemede yeniden hesaplanır. Bu bir sıralamadır, tavsiye değil; her seçim aşağıdaki tam kartına bağlanır. Ekran görüntüsü veya kayıt yayınlayan projeler tercih edilir; böylece şerit görsel kalır.</sub>

<table>
<tr>
<td width="50%" valign="top">
<b>🏛️ <a href="https://github.com/typesafe-ai/skills">typesafe-ai/skills</a></b>
<sub>⭐2113 · ✅ official</sub>
<sub>TypeSafe&#x27;ın System One API ile geliştirme için aracı becerileri</sub>
</td>
<td width="50%" valign="top">
<img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/john-rocky--coreai-kit/415a9014b32db230.jpg" width="100%" alt="typesafe-ai/skills">
<b>🧰 <a href="https://github.com/john-rocky/coreai-kit">john-rocky/coreai-kit</a></b>
<sub>⭐114 · Swift · 👁️ observed</sub>
<sub>Apple&#x27;ın Core AI teknolojisiyle iPhone ve Mac üzerinde sohbet, görüntü ve konuşma modellerini çalıştırmak için Swift SDK. Model indirme ve önbelleğe alma,…</sub>
</td>
</tr>
<tr>
<td width="50%" valign="top">
<img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/gargpratyush--jev-router/361cf042aa7f2e59.png" width="100%" alt="john-rocky/coreai-kit">
<b>🤖 <a href="https://github.com/gargpratyush/jev-router">gargpratyush/jev-router</a></b>
<sub>⭐399 · JavaScript · 🔎 inferred</sub>
<sub>jev-router kullanarak göreviniz için claude code&#x27;daki en ucuz modele yönlendirin</sub>
</td>
<td width="50%" valign="top">
<img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/nandhakishorm--laya/e01e49f0d842b461.png" width="100%" alt="gargpratyush/jev-router">
<b>🛡️ <a href="https://github.com/NandhaKishorM/laya">NandhaKishorM/laya</a></b>
<sub>⭐23705 · Python · 👁️ observed</sub>
<sub>Otokorelasyonsuz Sistem 1 karar motoru. Tek bir ileri geçişte, 100&#x27;den fazla dilde, her türlü metin üzerinde tür belirtilmiş seçim, puan ve evet/hayır kararları; her…</sub>
</td>
</tr>
<tr>
<td width="50%" valign="top">
<img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/ikermoel--open-alternative-jev/41dab050f73a168f.png" width="100%" alt="NandhaKishorM/laya">
<b>🧪 <a href="https://github.com/ikermoel/open-alternative-jev">ikermoel/open-alternative-jev</a></b>
<sub>⭐54 · Python · 👁️ observed</sub>
<sub>TypeSafe&#x27;s Jev için açık kaynaklı alternatif: tek ileri geçişte herhangi bir açık ağırlıklı LLM&#x27;ten yazım denetimli, kalibre edilmiş kararlar sağlayan System One…</sub>
</td>
<td width="50%" valign="top">
<img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/tianyucodings--nanojev/da3528699bc28423.gif" width="100%" alt="ikermoel/open-alternative-jev">
<b>🔬 <a href="https://github.com/TianyuCodings/NanoJev">TianyuCodings/NanoJev</a></b>
<sub>⭐2220 · Python · 🔎 inferred</sub>
<sub>Jev&#x27;ın nano kopyası: paralel kararlar, dinamik adaylar ve uçtan uca eğitim işlem hattı.</sub>
</td>
</tr>
<tr>
<td width="50%" valign="top">
<img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/zadescoxp--jev-trades/d74708c101b60531.png" width="100%" alt="TianyuCodings/NanoJev">
<b>🎮 <a href="https://github.com/zadescoxp/Jev-Trades">zadescoxp/Jev-Trades</a></b>
<sub>⭐32 · Python · 👁️ observed</sub>
<sub>TypeSafe AI&#x27;ın Jev olarak adlandırılan yepyeni ilk sistem one modeline sahip alım satım botu</sub>
</td>
<td width="50%" valign="top">
<img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/ayyazzafar--needle-jev/8b925374f5727b3e.png" width="100%" alt="zadescoxp/Jev-Trades">
<b>📰 <a href="https://github.com/ayyazzafar/needle-jev">ayyazzafar/needle-jev</a></b>
<sub>⭐0 · JavaScript · 🔎 inferred</sub>
<sub>Semantic find-in-page using TypeSafe Jev with your own API key. Modified from Needle in Shubhamsaboo/awesome-llm-apps (Apache-2.0).</sub>
</td>
</tr>
</table>

## İçindekiler

- [Jev nedir?](#jev-nedir)
- [Kayıtlar nasıl derecelendirilir](#kayıtlar-nasıl-derecelendirilir)
- [Resmî SDK'lar ve geliştirici araçları](#resmî-sdklar-ve-geliştirici-araçları) — **6**
- [Topluluk istemcileri, SDK'lar ve bağdaştırıcılar](#topluluk-istemcileri-sdklar-ve-bağdaştırıcılar) — **129**
- [Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları](#ajan-araçları-mcp-kancalar-geçitler-ve-kodlama-ajanları) — **249**
- [Yönlendirme, koruma kuralları ve onaylar](#yönlendirme-koruma-kuralları-ve-onaylar) — **58**
- [Değerlendirme, kalibrasyon ve kıyaslamalar](#değerlendirme-kalibrasyon-ve-kıyaslamalar) — **81**
- [Açık yeniden üretimler, ağırlıklar ve mimari araştırması](#açık-yeniden-üretimler-ağırlıklar-ve-mimari-araştırması) — **38**
- [Uygulamalar, oyunlar, robotik ve etkileşimli demolar](#uygulamalar-oyunlar-robotik-ve-etkileşimli-demolar) — **54**
- [Yazılar, tartışmalar ve kardeş listeler](#yazılar-tartışmalar-ve-kardeş-listeler) — **102**
- [Diğer projeler](#diğer-projeler) — **82**
- [Uygulama diline göre projeler](#uygulama-diline-göre-projeler)
- [Bu liste nasıl güncel kalıyor](#bu-liste-nasıl-güncel-kalıyor)

## Jev nedir?

Jev, TypeSafe AI'ın ilk **System One modelidir**. Düzyazı yazmaz. Bir durum ile cevap uzayını önceden sizin tanımladığınız soruları alır ve kodunuzun dallanabileceği olasılık dağılımlarıyla birlikte tipli değerler döndürür.

|                       |                                                                                                                                                                                                |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Biçim**             | `state + typed questions` → `constrained answers + probabilities` → `your code`                                                                                                                |
| **Temel öğeler**      | `Choice` (≤255 seçenekten birini seçer), `Score` (2–10 arası bir rubrik), `Noul` (olasılıksal evet/hayır)                                                                                      |
| **Uç nokta**          | `POST https://api.typesafe.ai/v1/systemone`, model `jev-1.13.0` / takma ad `jev-latest`                                                                                                        |
| **Uygun kullanımlar** | sınırlı bir iş akışı içinde yönlendirme, triyaj, puanlama, moderasyon, doğrulama ve düşük gecikmeli geçitler                                                                                   |
| **Bilinen sınırlar**  | sayma güvenilir değil, çok katmanlı dolaylılık zayıf ve resmî materyal dokuz sınıf pürüzlülük adlandırıyor. Şema-geçerli çıktı doğru kararla aynı şey değildir — kendi verinizle kalibre edin. |

## Kayıtlar nasıl derecelendirilir

Bu alandaki çoğu liste dahil ettiğini iddia eder. Bu liste gerçekte ne kadarını doğruladığını söyler ve buna göre filtrelemenize izin verir.

| Derece       | Anlamı                                                                                                                                              |
| ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `official`   | TypeSafe AI'ın kendisi tarafından yayımlandı.                                                                                                       |
| `observed`   | Açılıp okunabilen kamuya açık bir artefakt — gerçek kaynak kodu, gerçek yapılandırma ya da depo adında veya konusunda açık bir TypeSafe/Jev beyanı. |
| `inferred`   | Belirsiz bir sinyal ile onu destekleyen sözcük dağarcığı üzerinden eşleşti, ancak henüz satır satır okunmadı.                                       |
| `unverified` | İlgili görünüyor, bağımsız olarak doğrulanmış hiçbir şey yok. Yalnızca keşif amacıyla listelendi.                                                   |

<a id="official-sdk"></a>

## Resmî SDK&#x27;lar ve geliştirici araçları

TypeSafe'in kendi yayımladığı her şey. Buradan başlayın.

<details>
<summary>🏛️ <b><a href="https://github.com/typesafe-ai/skills">typesafe-ai/skills</a></b> · ⭐2113 · ✅ official · 13 天 · ⭐+147</summary>

##### 📝 Özet

TypeSafe'ın System One API ile geliştirme için aracı becerileri

> 💡 Tedarikçinin kendi aracı becerileri. Sürekli güncellendiği için TypeSafe'ın Jev'in bir aracıdan nasıl yönlendirilmesini amaçladığına dair bir belirtime en yakın şeydir.

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Resmî SDK&#x27;lar ve geliştirici araçları` |
| Kanıt    | `official`                                   |

##### 📊 Veri

| Ölçüt          | Değer           |
| -------------- | --------------- |
| Yıldız         | **2113** (+147) |
| Son push       | 2026-09-12      |
| İlk listelenme | 2026-09-18      |

</details>

<details>
<summary>🏛️ <b><a href="https://github.com/typesafe-ai/system-one-adapter-python">typesafe-ai/system-one-adapter-python</a></b> · ⭐296 · Python · ✅ official · 2 天 · ⭐+23</summary>

##### 📝 Özet

LLM APIs tarafından desteklenen, doğrudan kullanılabilir TypeSafeClient yerine geçen istemci

> 💡 Aynı arayüzü sıradan bir LLM sağlayıcısıyla destekleyen, doğrudan kullanılabilir bir alternatif. Bir karara bağlanmadan önce, kendi verileriniz üzerinde tür belirtilmiş bir kararı bir istemle A/B testine tabi tutmanın dürüst yoludur.

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Resmî SDK&#x27;lar ve geliştirici araçları` |
| Kanıt    | `official`                                   |
| Dil      | Python                                       |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **296** (+23) |
| Son push       | 2026-09-22    |
| İlk listelenme | 2026-09-18    |

</details>

<details>
<summary>🏛️ <b><a href="https://github.com/typesafe-ai/typesafe-sdk-js">typesafe-ai/typesafe-sdk-js</a></b> · ⭐235 · TypeScript · ✅ official · 9 天 · ⭐+7</summary>

##### 📝 Özet

TypeSafe API için resmî TypeScript/JavaScript kitaplığı

> 💡 Yanıt türünün sorduğunuz sorudan çıkarıldığı TypeScript istemcisi; böylece uyuşmayan bir dönüş türü çalışma zamanında beklenmedik bir durum olmak yerine derleme hatası olur.

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Resmî SDK&#x27;lar ve geliştirici araçları` |
| Kanıt    | `official`                                   |
| Dil      | TypeScript                                   |

##### 📊 Veri

| Ölçüt          | Değer        |
| -------------- | ------------ |
| Yıldız         | **235** (+7) |
| Son push       | 2026-09-15   |
| İlk listelenme | 2026-09-18   |

</details>

<details>
<summary>🏛️ <b><a href="https://github.com/typesafe-ai/typesafe-sdk-python">typesafe-ai/typesafe-sdk-python</a></b> · ⭐223 · Python · ✅ official · 3 天 · ⭐+14</summary>

##### 📝 Özet

TypeSafe API için resmî Python kitaplığı

> 💡 Eşzamanlı ve eşzamansız istemciler. Bir API anahtarından tür belirtilmiş bir karara giden en hızlı yol ve topluluk istemcilerinin karşılaştırıldığı başvuru noktası.

<sub>🔧 Kodda kullanıldığı tespit edildi: `src/typesafe_sdk/__init__.py`</sub>

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Resmî SDK&#x27;lar ve geliştirici araçları` |
| Kanıt    | `official`                                   |
| Dil      | Python                                       |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **223** (+14) |
| Son push       | 2026-09-21    |
| İlk listelenme | 2026-09-18    |

</details>

<details>
<summary>🏛️ <b><a href="https://github.com/TypeSafeAI/jev-harness">TypeSafeAI/jev-harness</a></b> · ⭐14 · TypeScript · ✅ official · 0 天 · ⭐+6</summary>

##### 📝 Özet

TypeSafe AI'ın Jev'i için özel bir kodlama harness'i: bir LLM önerilerde bulunur, Jev dar soruları yanıtlar, kararı kod verir ve her adım bir makbuz bırakır.

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Resmî SDK&#x27;lar ve geliştirici araçları` |
| Kanıt    | `official`                                   |
| Dil      | TypeScript                                   |

##### 📊 Veri

| Ölçüt          | Değer       |
| -------------- | ----------- |
| Yıldız         | **14** (+6) |
| Son push       | 2026-09-25  |
| İlk listelenme | 2026-09-22  |

🏷 `agent-harness` · `agentic-coding` · `ai-agents` · `ai-governance` · `ai-safety` · `code-review` · `coding-agent` · `developer-tools`

</details>

<details>
<summary>🏛️ <b><a href="https://github.com/typesafe-ai/typesafe-ai.github.io">typesafe-ai/typesafe-ai.github.io</a></b> · ⭐2 · HTML · ✅ official · 112 天</summary>

##### 📝 Özet

Üst kaynakta açıklama yayımlanmamış.

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Resmî SDK&#x27;lar ve geliştirici araçları` |
| Kanıt    | `official`                                   |
| Dil      | HTML                                         |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **2**      |
| Son push       | 2026-06-04 |
| İlk listelenme | 2026-09-18 |

</details>

<a id="community-sdk"></a>

## Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar

System One uç noktası için tipli istemciler; topluluğun sıra geldiği ölçüde çok dilde.

<details>
<summary>🧰 <b><a href="https://github.com/jexp/neo4jev">jexp/neo4jev</a></b> · ⭐134 · Jupyter · 👁️ observed · 7 天 · ⭐+27</summary>

##### 📝 Özet

Typesafe.ai System One Model Jev, komşu ilişkiler üzerinde bir sınıflandırıcı kullanarak Neo4j grafiğinde gezinir

##### 📌 Temel bilgiler

| Alan     | Değer                                                   |
| -------- | ------------------------------------------------------- |
| Kategori | `Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar` |
| Kanıt    | `observed`                                              |
| Dil      | Jupyter                                                 |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **134** (+27) |
| Son push       | 2026-09-18    |
| İlk listelenme | 2026-09-18    |

</details>

<details>
<summary>🧰 <b><a href="https://github.com/john-rocky/coreai-kit">john-rocky/coreai-kit</a></b> · ⭐114 · Swift · 👁️ observed · 0 天 · ⭐+3</summary>

##### 📝 Özet

Apple'ın Core AI teknolojisiyle iPhone ve Mac üzerinde sohbet, görüntü ve konuşma modellerini çalıştırmak için Swift SDK. Model indirme ve önbelleğe alma, FoundationModels entegrasyonu ve belgelenmiş işletim sistemi, SDK ve model gereksinimleriyle çalıştırılabilir örnekler.

<sub>🔧 Kodda kullanıldığı tespit edildi: `Sources/CoreAIOps/CoreAI+SystemOne.swift`</sub>

##### 📌 Temel bilgiler

| Alan     | Değer                                                   |
| -------- | ------------------------------------------------------- |
| Kategori | `Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar` |
| Kanıt    | `observed`                                              |
| Dil      | Swift                                                   |

##### 📊 Veri

| Ölçüt          | Değer        |
| -------------- | ------------ |
| Yıldız         | **114** (+3) |
| Son push       | 2026-09-24   |
| İlk listelenme | 2026-09-23   |

🏷 `apple` · `apple-core-ai` · `asr` · `core-ai` · `coreai` · `foundation-models` · `ios` · `ios-27`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/john-rocky--coreai-kit/415a9014b32db230.jpg" width="100%" alt="john-rocky/coreai-kit screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/john-rocky--coreai-kit/c98c7bb8cb04e76e.gif" width="100%" alt="john-rocky/coreai-kit animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

</details>

<details>
<summary>🧰 <b><a href="https://github.com/bodepudimuneendra-netizen/laya-jev-GraphRAG">bodepudimuneendra-netizen/laya-jev-GraphRAG</a></b> · ⭐28 · Python · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

Değiştirilebilir System One modelleri (yerel Laya / bulut Jev) kullanan, veritabanından bağımsız bir Agentic GraphRAG çatısı. Herhangi bir grafik veritabanı için eksiksiz 4 aşamalı işlem hattı ve özel A* dolaşımı sunan tak-çalıştır zekâ katmanı.

##### 📌 Temel bilgiler

| Alan     | Değer                                                   |
| -------- | ------------------------------------------------------- |
| Kategori | `Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar` |
| Kanıt    | `observed`                                              |
| Dil      | Python                                                  |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **28**     |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-25 |

🏷 `astar-algorithm` · `eval-harness` · `fine-tuning` · `graphdatabase` · `graprag` · `jev` · `laya` · `neo4j`

</details>

<details>
<summary>🧰 <b><a href="https://github.com/MrJev/awesome-jev">MrJev/awesome-jev</a></b> · ⭐11 · Python · 👁️ observed · 0 天 · ⭐+1</summary>

##### 📝 Özet

Jev, TypeSafe AI'in System One modeli için derlenmiş proje, entegrasyon ve kaynak listesi.

##### 📌 Temel bilgiler

| Alan     | Değer                                                   |
| -------- | ------------------------------------------------------- |
| Kategori | `Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar` |
| Kanıt    | `observed`                                              |
| Dil      | Python                                                  |

##### 📊 Veri

| Ölçüt          | Değer       |
| -------------- | ----------- |
| Yıldız         | **11** (+1) |
| Son push       | 2026-09-25  |
| İlk listelenme | 2026-09-18  |

🏷 `ai-agents` · `awesome` · `awesome-list` · `jev` · `mcp` · `typesafe`

</details>

<details>
<summary>🧰 <b><a href="https://github.com/agencyenterprise/jev-recipes">agencyenterprise/jev-recipes</a></b> · ⭐10 · TypeScript · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

200'den fazla tak-çalıştır Jev tarifi: ajanlar, RAG, destek, kod inceleme ve müzik için metni yönlendiren, derecelendiren, geçitleyen, karşılaştıran ve etiketleyen küçük, kalibre edilmiş AI kararları. JavaScript veya TypeScript üzerinden içe aktarın ya da herhangi bir dilden JSON ile CLI'ü çağırın.

##### 📌 Temel bilgiler

| Alan     | Değer                                                   |
| -------- | ------------------------------------------------------- |
| Kategori | `Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar` |
| Kanıt    | `observed`                                              |
| Dil      | TypeScript                                              |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **10**     |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-25 |

🏷 `jev` · `jev-ai` · `jev-api` · `jev-model` · `system-one-model` · `system-one-models` · `typesafe` · `typesafe-ai`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/agencyenterprise--jev-recipes/0f08086836b893d8.gif" width="100%" alt="agencyenterprise/jev-recipes screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/agencyenterprise--jev-recipes/0f08086836b893d8.gif" width="100%" alt="agencyenterprise/jev-recipes animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

</details>

<details>
<summary>🧰 <b><a href="https://github.com/ckaraca/awesome-jev">ckaraca/awesome-jev</a></b> · ⭐10 · Python · 👁️ observed · 3 天 · ⭐+1</summary>

##### 📝 Özet

Hızlı, yazım denetimli kararlar için TypeSafe AI's System One modeli olan Jev üzerine kurulmuş araçlar, entegrasyonlar ve denemelerden derlenmiş bir liste.

##### 📌 Temel bilgiler

| Alan     | Değer                                                   |
| -------- | ------------------------------------------------------- |
| Kategori | `Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar` |
| Kanıt    | `observed`                                              |
| Dil      | Python                                                  |

##### 📊 Veri

| Ölçüt          | Değer       |
| -------------- | ----------- |
| Yıldız         | **10** (+1) |
| Son push       | 2026-09-21  |
| İlk listelenme | 2026-09-19  |

🏷 `ai-agents` · `awesome` · `awesome-list` · `computer-use` · `jev` · `llm` · `system-one` · `typesafe`

</details>

<details>
<summary>🧰 <b><a href="https://github.com/Premo-Cloud/typesafe-sdk-java">Premo-Cloud/typesafe-sdk-java</a></b> · ⭐7 · Java · 👁️ observed · 1 天 · ⭐+4</summary>

##### 📝 Özet

Topluluk Java SDK for Jev, TypeSafe'ın System One modeli: türlendirilmiş sorular girer, kalibre edilmiş olasılıklarla türlendirilmiş yanıtlar çıkar. Java 17+, Spring Boot starter (resmî olmayan)

##### 📌 Temel bilgiler

| Alan     | Değer                                                   |
| -------- | ------------------------------------------------------- |
| Kategori | `Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar` |
| Kanıt    | `observed`                                              |
| Dil      | Java                                                    |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **7** (+4) |
| Son push       | 2026-09-23 |
| İlk listelenme | 2026-09-18 |

🏷 `java` · `jev` · `sdk` · `spring-boot` · `system-one` · `system-one-models` · `typesafe`

</details>

<details>
<summary>🧰 <b><a href="https://github.com/nshkrdotcom/typesafe_sdk">nshkrdotcom/typesafe_sdk</a></b> · ⭐6 · Elixir · 👁️ observed · 5 天 · ⭐+1</summary>

##### 📝 Özet

Birleşik LLM entegrasyonları, akışlı metin ve yapılandırılmış çıktılar, araç çağırma ve aracı iş akışları sağlayan, resmî TypeScript AI SDK'nin (ai / ai-sdk) deyimsel ve tür güvenli bir Elixir portu. Jev onların mevcut amiral gemisi modelidir ve ilk System One modelidir.

##### 📌 Temel bilgiler

| Alan     | Değer                                                   |
| -------- | ------------------------------------------------------- |
| Kategori | `Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar` |
| Kanıt    | `observed`                                              |
| Dil      | Elixir                                                  |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **6** (+1) |
| Son push       | 2026-09-20 |
| İlk listelenme | 2026-09-18 |

🏷 `agents` · `ai` · `ai-sdk` · `artificial-intelligence` · `elixir` · `function-calling` · `generative-ai` · `jev`

</details>

<details>
<summary>🧰 <b><a href="https://github.com/xingwudao/OpenJev">xingwudao/OpenJev</a></b> · ⭐6 · Python · 👁️ observed · 7 天 · ⭐+1</summary>

##### 📝 Özet

OpenJev: TypeSafe.ai kavramlarına dayanan, Jev esintili bağımsız bir System One karar API. Choice, puan ve noul ilkelleri, yerel sahte sunucu, Python ve TypeScript SDKs. Gerçek çıkarım planlanmaktadır; TypeSafe AI ile bağlantılı değildir.

##### 📌 Temel bilgiler

| Alan     | Değer                                                   |
| -------- | ------------------------------------------------------- |
| Kategori | `Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar` |
| Kanıt    | `observed`                                              |
| Dil      | Python                                                  |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **6** (+1) |
| Son push       | 2026-09-18 |
| İlk listelenme | 2026-09-18 |

🏷 `ai-guardrails` · `classification` · `decision-api` · `jev` · `mock-server` · `openjev` · `probabilistic-inference` · `python-sdk`

</details>

<details>
<summary>🧰 <b><a href="https://github.com/kisshan13/typesafe-ai-go">kisshan13/typesafe-ai-go</a></b> · ⭐5 · Go · 👁️ observed · 4 天 · ⭐+1</summary>

##### 📝 Özet

TypeSafe AI System One değerlendirmesi API için topluluk tarafından sürdürülen Go SDK; türü belirlenmiş sorular, akıcı oluşturucular, yeniden denemeler ve örneklerle.

##### 📌 Temel bilgiler

| Alan     | Değer                                                   |
| -------- | ------------------------------------------------------- |
| Kategori | `Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar` |
| Kanıt    | `observed`                                              |
| Dil      | Go                                                      |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **5** (+1) |
| Son push       | 2026-09-20 |
| İlk listelenme | 2026-09-21 |

🏷 `ai` · `jev` · `jev-go-sdk` · `sdk-go` · `structured-output` · `system-one-models` · `typesafe-ai`

</details>

<details>
<summary>🧰 <b><a href="https://github.com/ChenYCL/jev-browser-skill">ChenYCL/jev-browser-skill</a></b> · ⭐2 · JavaScript · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

Browser use & computer use for coding agents, powered by TypeSafe Jev: calibrated judgments from a System One model, control loop in code. ego lite / Chrome / Safari · CLI + MCP

##### 📌 Temel bilgiler

| Alan     | Değer                                                   |
| -------- | ------------------------------------------------------- |
| Kategori | `Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar` |
| Kanıt    | `observed`                                              |
| Dil      | JavaScript                                              |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **2**      |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-25 |

🏷 `agent-skill` · `browser-automation` · `claude-code` · `claude-desktop` · `codex` · `computer-use` · `cursor` · `ego-lite`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/chenycl--jev-browser-skill/c6a8dc00e793f2a1.png" width="100%" alt="ChenYCL/jev-browser-skill screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/chenycl--jev-browser-skill/6f1223bb1aa99b1b.gif" width="100%" alt="ChenYCL/jev-browser-skill animation"><br><sub>hareketli kayıt · <a href="https://raw.githubusercontent.com/ChenYCL/jev-browser-skill/main/assets/demo-wikipedia.mp4">Videoyu aç</a></sub></td>
</tr></table>

</details>

<details>
<summary>🧰 <b><a href="https://github.com/clouatre-labs/decisions-judge-mcp">clouatre-labs/decisions-judge-mcp</a></b> · ⭐2 · JavaScript · 👁️ observed · 1 天</summary>

##### 📝 Özet

Typed decisions for AI agents as an MCP tool: yes/no probability (noul), choice, and score in one fast request. Backed by the TypeSafe System One model.

##### 📌 Temel bilgiler

| Alan     | Değer                                                   |
| -------- | ------------------------------------------------------- |
| Kategori | `Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar` |
| Kanıt    | `observed`                                              |
| Dil      | JavaScript                                              |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **2**      |
| Son push       | 2026-09-24 |
| İlk listelenme | 2026-09-21 |

🏷 `agent` · `ai` · `cli` · `decision-making` · `decisions` · `judge` · `llm` · `mcp`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/clouatre-labs--decisions-judge-mcp/c8c00f2b58774857.gif" width="100%" alt="clouatre-labs/decisions-judge-mcp screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/clouatre-labs--decisions-judge-mcp/c8c00f2b58774857.gif" width="100%" alt="clouatre-labs/decisions-judge-mcp animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

</details>

<details>
<summary>🧰 <b><a href="https://github.com/Bonzokoles/36_chambers">Bonzokoles/36_chambers</a></b> · Python · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

Federated knowledge retrieval engine for AI agents. Routes queries across specialized backends (FTS5, ChromaDB, graph) via a registry-first orchestrator with TypeSafe/Jev AI routing and evidence re-ranking. Read-only by default, security-gated by design.

##### 📌 Temel bilgiler

| Alan     | Değer                                                   |
| -------- | ------------------------------------------------------- |
| Kategori | `Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar` |
| Kanıt    | `observed`                                              |
| Dil      | Python                                                  |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-24 |
| İlk listelenme | 2026-09-25 |

🏷 `ai-agents` · `ai-infrastructure` · `chromadb` · `embeddings` · `federated-search` · `fts5` · `information-retrieval` · `jev`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/Bonzokoles/36_chambers/main/assets/36_chambers_shaolin_matrix_hero.jpg" width="100%" alt="Bonzokoles/36_chambers screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

<sub>Yeniden dağıtım lisansı belirtilmediği için varlık üst depodan doğrudan bağlanmıştır.</sub>

</details>

<details>
<summary>🧰 <b><a href="https://github.com/OpenByteInc/QuantDinger">OpenByteInc/QuantDinger</a></b> · ⭐12140 · Python · 🔎 inferred · 0 天 · ⭐+96</summary>

##### 📝 Özet

Açık kaynaklı AI Trading OS, ajan ticareti ve vibe trading; Jev System One entegrasyonuyla. Python stratejilerini araştırın ve oluşturun, geriye dönük test yapın ve kripto, hisse senetleri ile forex genelinde kâğıt üzerinde/canlı işlem gerçekleştirin. Yerleşik kullanıcı yönetimi, faturalandırma, ödemeler ve mutabakat ile kendi çok kiracılı trading SaaS'ınızı başlatın.

##### 📌 Temel bilgiler

| Alan     | Değer                                                   |
| -------- | ------------------------------------------------------- |
| Kategori | `Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar` |
| Kanıt    | `inferred`                                              |
| Dil      | Python                                                  |

##### 📊 Veri

| Ölçüt          | Değer           |
| -------------- | --------------- |
| Yıldız         | **12140** (+96) |
| Son push       | 2026-09-25      |
| İlk listelenme | 2026-09-20      |

🏷 `agent` · `alpaca` · `backtesting` · `binance` · `crypto` · `exchange` · `finance` · `fintech`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/openbyteinc--quantdinger/bad53c6f91086527.jpg" width="100%" alt="OpenByteInc/QuantDinger screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🧰 <b><a href="https://github.com/rokbenko/quackd">rokbenko/quackd</a></b> · ⭐237 · Python · 🔎 inferred · 0 天 · ⭐+8</summary>

##### 📝 Özet

Tüm robotlarınız için tek bir CLI. Onları bağlayın, komut verin ve her birinin beyni için bir LLM (Claude, OpenAI, Gemini, Grok veya Ollama ya da vLLM üzerinden yerel) ve çoktan seçmeli kararlar için bir karar LLM'si (Jev, Laya, Kev) ile birlikte çalışmalarını sağlayın. Microduck, Open Duck Mini, LeRobot, XLeRobot, AlohaMini, ToddlerBot veya herhangi bir ROS 2 tabanını bir dizüstü bilgisayardan, harici olarak çalıştırır.

##### 📌 Temel bilgiler

| Alan     | Değer                                                   |
| -------- | ------------------------------------------------------- |
| Kategori | `Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar` |
| Kanıt    | `inferred`                                              |
| Dil      | Python                                                  |

##### 📊 Veri

| Ölçüt          | Değer        |
| -------------- | ------------ |
| Yıldız         | **237** (+8) |
| Son push       | 2026-09-25   |
| İlk listelenme | 2026-09-22   |

🏷 `ai-agents` · `alohamini` · `claude` · `gemini` · `jetson` · `jev` · `lerobot` · `local-llm`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/rokbenko--quackd/8336d43f24fa6b41.png" width="100%" alt="rokbenko/quackd screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/rokbenko--quackd/2060eda6dcb35de6.gif" width="100%" alt="rokbenko/quackd animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

</details>

<details>
<summary>🧰 <b><a href="https://github.com/nidhi-singh02/agent-router">nidhi-singh02/agent-router</a></b> · ⭐78 · TypeScript · 🔎 inferred · 0 天 · **NEW**</summary>

##### 📝 Özet

Bir görev için Cursor, Claude Code, Codex veya OpenCode + model/efor seçen ve ardından onu başlatan CLI. Jev ve Herdr tarafından desteklenir

##### 📌 Temel bilgiler

| Alan     | Değer                                                   |
| -------- | ------------------------------------------------------- |
| Kategori | `Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar` |
| Kanıt    | `inferred`                                              |
| Dil      | TypeScript                                              |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **78**     |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-25 |

🏷 `agents` · `ai` · `claude-code` · `cli` · `codex` · `coding` · `developer-tools` · `herdr`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/nidhi-singh02--agent-router/976e58ae0d278abd.jpg" width="100%" alt="nidhi-singh02/agent-router screenshot"></td>
<td align="center" valign="top"><a href="https://img.youtube.com/vi/7w8eRWnUUA8/maxresdefault.jpg"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/nidhi-singh02--agent-router/976e58ae0d278abd.jpg" width="100%" alt="video"></a><br><sub><a href="https://img.youtube.com/vi/7w8eRWnUUA8/maxresdefault.jpg">İzleme yeri img.youtube.com</a> · oynatma kaynak sitede açılır; GitHub bunu satır içi gömemez</sub></td>
</tr></table>

</details>

<details>
<summary>🧰 <b><a href="https://github.com/AkashPriyadarshii/jev-seo">AkashPriyadarshii/jev-seo</a></b> · ⭐67 · Rust · 🔎 inferred · 0 天 · ⭐+28</summary>

##### 📝 Özet

jev-seo: kodlama ajanları için Rust SEO ve GEO CLI ile MCP sunucusu: 50 kural denetimi, canlı taramalar, GEO puanları, sıralama değişimi, CI kapıları. MIT, abonelik yok.

##### 📌 Temel bilgiler

| Alan     | Değer                                                   |
| -------- | ------------------------------------------------------- |
| Kategori | `Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar` |
| Kanıt    | `inferred`                                              |
| Dil      | Rust                                                    |

##### 📊 Veri

| Ölçüt          | Değer        |
| -------------- | ------------ |
| Yıldız         | **67** (+28) |
| Son push       | 2026-09-25   |
| İlk listelenme | 2026-09-18   |

🏷 `ahrefs-alternative` · `answer-engine-optimization` · `claude-code` · `cli` · `coding-agents` · `generative-engine-optimization` · `geo` · `jev`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/akashpriyadarshii--jev-seo/8761e36834a3cc5d.png" width="100%" alt="AkashPriyadarshii/jev-seo screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🧰 <b><a href="https://github.com/AboveColin/HA-Jev">AboveColin/HA-Jev</a></b> · ⭐63 · Python · 🔎 inferred · 0 天 · ⭐+10</summary>

##### 📝 Özet

Evinize bir soru sorun, bir sayı alın. TypeSafe Jev için Home Assistant entegrasyonu: sensörler olarak tür belirtilmiş yanıtlar, otomasyonlar için dört eylem ve Assist için bir konuşma aracısı.

##### 📌 Temel bilgiler

| Alan     | Değer                                                   |
| -------- | ------------------------------------------------------- |
| Kategori | `Topluluk istemcileri, SDK&#x27;lar ve bağdaştırıcılar` |
| Kanıt    | `inferred`                                              |
| Dil      | Python                                                  |

##### 📊 Veri

| Ölçüt          | Değer        |
| -------------- | ------------ |
| Yıldız         | **63** (+10) |
| Son push       | 2026-09-25   |
| İlk listelenme | 2026-09-18   |

🏷 `ai` · `assist` · `conversation` · `custom-components` · `decision-model` · `hacs` · `home-assistant` · `home-automation`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/abovecolin--ha-jev/122e5d87482d8b38.png" width="100%" alt="AboveColin/HA-Jev screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary><b>Bu kategoride daha fazla</b> <sub>· 111</sub></summary>

- [Barneyjm/decision-circuits](https://github.com/Barneyjm/decision-circuits) - Decision circuits: typed questions to a System One model, calibrated…
- [GitHub30/OpenJev](https://github.com/GitHub30/OpenJev) - Open-weight System One model (Jev-compatible): calibrated noul / choice / score…
- [hemanth/jevql](https://github.com/hemanth/jevql) - PostgreSQL-compatible query language powered by TypeSafe Jev System One models.
- [jqueryscript/awesome-jev](https://github.com/jqueryscript/awesome-jev) - A curated list of TypeSafe Jev resources, SDKs, agents, MCP servers…
- [juangchuank-ops/jev2api](https://github.com/juangchuank-ops/jev2api) - TypeSafe Jev (typesafe.ai) 反向代理网关：原生 /v1/systemone 透传 + OpenAI 兼容层，含号池与控制台.
- [ryancole/jev-dotnet](https://github.com/ryancole/jev-dotnet) - .NET client for Jev, TypeSafe.
- [basi/typesafe-ai-php-client](https://github.com/basi/typesafe-ai-php-client) - PHP client for the typesafe.ai System One API (Jev).
- [olivere/systemone](https://github.com/olivere/systemone) - Small, standard-library-only Go library for typed decisions with Jev and other…
- [PromtEngineer/jev-harness](https://github.com/PromtEngineer/jev-harness) - A Pi agent harness built around TypeSafe.
- [SC0d3r/jev-systemone](https://github.com/SC0d3r/jev-systemone) - Typed TypeScript client for Jev System One models, Choice, Score &amp; Noul…
- [systemonemodels/systemonemodels-sdk](https://github.com/systemonemodels/systemonemodels-sdk)
- [realZachi/pg-jev](https://github.com/realZachi/pg-jev) - Postgres tablolarınıza düz dille sorular sorun.
- [sutro-sh/jev-align](https://github.com/sutro-sh/jev-align) - Jev ve GEPA kullanarak insan geri bildirimlerinden kalibre edilmiş AI Functions…
- [juspay/neurolink](https://github.com/juspay/neurolink) - Bir AI sinir sisteminin boru katmanı — üç çıkarım türünde sağlayıcı nöronlarını…
- [keltokhy/jgrep](https://github.com/keltokhy/jgrep) - grep, ancak desen bir açıklamadır. Satırları anlamlarına göre TypeSafe.
- [qkal/Canny](https://github.com/qkal/Canny) - AI kodlama ajanlarının kanıt olmadan işin tamamlandığını iddia etmesini…
- [Ying-Kai-Liao/jev-browser](https://github.com/Ying-Kai-Liao/jev-browser) - Bir LLM&#x27;ın planladığı ve Jev&#x27;in (Typesafe System One) karar verdiği tarayıcı…
- [fazlerocks/jevmail](https://github.com/fazlerocks/jevmail) - Gmail için açık kaynaklı AI e-posta önceliklendirme aracı.
- [yzfly/awesome-jev-zh](https://github.com/yzfly/awesome-jev-zh) - Jev / TypeSafe System One Çince seçki listesi: resmî kaynaklar, SDK, popüler…
- [AkashPriyadarshii/jev-curate](https://github.com/AkashPriyadarshii/jev-curate) - TypeSafe Jev için yüksek aktarım hızlı sentetik ve ön eğitim veri kümesi…
- [sharziki/semdecide](https://github.com/sharziki/semdecide) - Unix pipelines ve CI için TypeSafe AI Jev tarafından desteklenen türlendirilmiş…
- [burnigtm/jev-mcp](https://github.com/burnigtm/jev-mcp) - Cursor, Codex ve herhangi bir MCP istemcisinde TypeSafe Jev.
- [tacticocc/Jevbridge](https://github.com/tacticocc/Jevbridge) - ACP ve MCP bağdaştırıcısı; TypeSafe Jev ile herhangi bir LLM arasında köprü…
- [pinecone-io/cultivar](https://github.com/pinecone-io/cultivar) - Agent Skills ve Docs.
- [danvega/jev-spring-boot-starter](https://github.com/danvega/jev-spring-boot-starter) - Spring MVC ve RestClient kullanan TypeSafe Jev için basit bir Spring Boot 4…
- [dannote/jev](https://github.com/dannote/jev) - OTP için TypeSafe Jev: bir GenServer.
- [win4r/jev-skill-suggester](https://github.com/win4r/jev-skill-suggester) - TypeSafe Jev ile yüklü Skill.
- [shaharia-lab/jev-cli](https://github.com/shaharia-lab/jev-cli) - TypeSafe AI.
- [lukstei/slop-grader](https://github.com/lukstei/slop-grader) - Metin dosyaları için Jev destekli, kural tabanlı değerlendirici.
- [AkashPriyadarshii/jev-superpowers](https://github.com/AkashPriyadarshii/jev-superpowers) - TypeSafe Jev System One türlendirilmiş kararlarıyla yükseltilmiş, AI kodlama…
- [keltokhy/jsort](https://github.com/keltokhy/jsort) - anlama göre sıralayın: TypeSafe.
- [chengyongru/fastjev](https://github.com/chengyongru/fastjev) - SDK öncelikli, hızlı ve kendi sunucusunda barındırılabilen anlamsal kararlar…
- [Ray-Hughes/jevalyn](https://github.com/Ray-Hughes/jevalyn) - Rails uygulamanız için karar katmanı. TypeSafe.
- [CheshiAI/Cheshi](https://github.com/CheshiAI/Cheshi) - Jev destekli konuşma belleği: geçmiş oturumları bulun ve kararları özgün…
- [evoke-build/evoke](https://github.com/evoke-build/evoke) - Yazılım, refleksle. Söyleyin, doğru küçük program çalışsın: kalibre edilmiş bir…
- [utk2103/jev-studio](https://github.com/utk2103/jev-studio) - jev ile denemeler yapıyorsanız buradan daha kolay olacaktır.
- [arjun988/Kev](https://github.com/arjun988/Kev) - Açık kaynaklı System One karar motoru.
- [saibimajdi/typesafeai-dotnet-sdk](https://github.com/saibimajdi/typesafeai-dotnet-sdk) - TypeSafe AI System One API için topluluk .NET SDK — yapılandırılmış, güven…
- [himomohi/aside-jev](https://github.com/himomohi/aside-jev) - Aside ajanları TypeSafe Jev ile karar verir (System One: Choice/Score/Noul).
- [tontoko/jev-browser](https://github.com/tontoko/jev-browser) - Tek bir temellendirilmiş Jev/Playwright çekirdeği: tür belirtilmiş SDK, kalıcı…
- [devbackend/jevgo](https://github.com/devbackend/jevgo) - TypeSafe AI System One API (Jev) için resmî olmayan Go istemcisi — yazılı…
- [docxology/daf-jev](https://github.com/docxology/daf-jev) - daf-jev: TypeSafe&#x27;s Jev (System One) karar API&#x27;ü için birleştirilebilir Python…
- [brnyxx/jev-ra](https://github.com/brnyxx/jev-ra) - Browser use for coding agents, 3-5x faster than browser-use. MCP server + CLI;
- [Gaurav-Gosain/jev-go](https://github.com/Gaurav-Gosain/jev-go) - Go client for TypeSafe.
- [mountainMath/JevR](https://github.com/mountainMath/JevR) - TypeSafe Jev System One API için R istemcisi.
- [Olti1947/jev-java](https://github.com/Olti1947/jev-java) - TypeSafe AI Jev System One karar motoru için deyimsel Java SDK.
- [Stumble/jev-go](https://github.com/Stumble/jev-go) - TypeSafe AI Jev / System One için topluluk Go SDK.
- [sumleo/prompt2jev](https://github.com/sumleo/prompt2jev) - Doğal dili, bir LLM istemini veya birini çalıştıran kodu bir TypeSafe Jev…
- [AkashPriyadarshii/jev-git](https://github.com/AkashPriyadarshii/jev-git) - Sub-second Git pre-commit &amp; pre-push semantic reflex gate powered by TypeSafe…
- [Butochnikov/laravel-typesafe-jev](https://github.com/Butochnikov/laravel-typesafe-jev) - Türlenmiş yanıtlar, asenkron istekler, kapsamlı bağımlılık enjeksiyonu ve test…
- [keltokhy/jselect](https://github.com/keltokhy/jselect) - Useful evidence for your AI, within a token budget.
- [AboveColin/jevclient](https://github.com/AboveColin/jevclient) - TypeSafe Jev için asenkron Python istemcisi.
- [AkashPriyadarshii/jev-scout](https://github.com/AkashPriyadarshii/jev-scout) - Zero-hallucination open-source repo and crate scout powered by TypeSafe AI Jev…
- [diorrego/toolgate-experiment](https://github.com/diorrego/toolgate-experiment) - Benchmarks of MCP tool selection accuracy and latency.
- [KKloudTarus/taurus-jev-sdk-go](https://github.com/KKloudTarus/taurus-jev-sdk-go) - Unofficial, dependency-free Go client for the TypeSafe AI System One API and…
- [pewriebontal/typesafe-sdk-cpp](https://github.com/pewriebontal/typesafe-sdk-cpp) - An unofficial CPP 20 SDK for the TypeSafe API.
- [zhirschtritt/typesafe-go](https://github.com/zhirschtritt/typesafe-go) - TypeSafe AI API için deyimsel Go SDK.
- [Kadihx/jev-x-kit](https://github.com/Kadihx/jev-x-kit) - Offline $0 decision layer for coding agents: Choice/Score/Noul primitives…
- [DhanushNehru/jev-sec-audit](https://github.com/DhanushNehru/jev-sec-audit) - Lightning-fast AI supply chain security auditor using Jev (System 1 models).
- [EdgeForgeLab/jev-kol-mcp](https://github.com/EdgeForgeLab/jev-kol-mcp) - TypeScript MCP server that finds TikTok and YouTube micro-KOLs, tags their…
- [FlyPig23/Codex_ChatGPT_JEV_Switch](https://github.com/FlyPig23/Codex_ChatGPT_JEV_Switch) - Fork of codex-with-chatgpt: TypeSafe Jev + deterministic rules decide when…
- [heiwa4126/jev-bun1](https://github.com/heiwa4126/jev-bun1) - TypeSafe の Jev を TypeScript SDK で使ってみる最初の 1 歩.
- [initrd/himalaya-jev-mail-classify](https://github.com/initrd/himalaya-jev-mail-classify) - Label and prioritise Gmail with a language model.
- [itscloud0/codex-jev-native-router](https://github.com/itscloud0/codex-jev-native-router) - Experimental native Codex Desktop and CLI model routing with Jev and a…
- [kunobi-ninja/kunobi-jev](https://github.com/kunobi-ninja/kunobi-jev) - Rust client for the TypeSafe System One API (Jev).
- [Lasimeri/Mechanical-Jev](https://github.com/Lasimeri/Mechanical-Jev) - The asking side of Jev (TypeSafe System One: noul, choice, score) in Rust…
- [nekowasabi/jev-routing](https://github.com/nekowasabi/jev-routing) - Claude Code, Codex ve Grok Build için Go Jev harness.
- [Oranquelui/astra-jev-harness](https://github.com/Oranquelui/astra-jev-harness) - Agent Skills for Codex Desktop and Claude Code, plus a Codex CLI harness.
- [Pioneer113/jev-chrome-mcp](https://github.com/Pioneer113/jev-chrome-mcp) - Runs the jev-browser-use click loop in Google Chrome for any MCP client.
- [quaeast/vllm2jev](https://github.com/quaeast/vllm2jev) - Jev-compatible adapter for vLLM: use any vLLM-served LLM for Choice, Score, and…
- [silkyland/use-jev](https://github.com/silkyland/use-jev) - MCP server + CLI exposing TypeSafe.
- [TOSUKUi/jev-bridge](https://github.com/TOSUKUi/jev-bridge) - Jev-style /v1/systemone API in front of any OpenAI-compatible LLM server…
- [acidkill/JevCompass](https://github.com/acidkill/JevCompass) - Tool and skill suggestions for Codex Desktop and CLI, with local-first privacy…
- [AuroraPixel/jev-browser-use](https://github.com/AuroraPixel/jev-browser-use) - Jev-powered browser automation for Codex and Claude: native reasoning…
- [bspiritxp/jev-cli](https://github.com/bspiritxp/jev-cli) - CLI client for TypeSafe Jev (System One) structured decisions: noul / choice /…
- [chayan-bit/Jev-Frame](https://github.com/chayan-bit/Jev-Frame) - Typed, auditable Jev decisions for Python agents, with an optional shared…
- [codebam/jev-guardrails](https://github.com/codebam/jev-guardrails) - Jev-backed guardrails for agent tool calls: library, native…
- [DejaAI2/JevNext](https://github.com/DejaAI2/JevNext) - Decision model on a Qwen3-0.6B backbone with LoRA: fused decision endpoint…
- [Fayegram-AI/jev-adapter](https://github.com/Fayegram-AI/jev-adapter) - JavaScript SDK for structured decisions with native Jev and configurable model…
- [hajime-goshima/jev-lab](https://github.com/hajime-goshima/jev-lab) - TypeSafe AI の Jev を Ruby で試した実践例（感情分析・CloudTrailのリスク評価・フォームのリアルタイム検証）.
- [JingHao-Leon/awesome-jev-apps](https://github.com/JingHao-Leon/awesome-jev-apps) - Jev 优质应用与生态精选｜System One 决策模型：开源应用·SDK·平台集成·开源复刻·教程 | curated apps &amp; SDKs for…
- [jumboly/jev-client](https://github.com/jumboly/jev-client) - TypeSafe AI Jev client (Vercel AI Gateway / direct API).
- [KHAEntertainment/jev-skill](https://github.com/KHAEntertainment/jev-skill) - Agent skill: decide when and how to integrate TypeSafe.
- [kkdev92/jev-dotnet](https://github.com/kkdev92/jev-dotnet) - An unofficial .NET 10 client for Jev, TypeSafe.
- [mheers/typesafeai-systemone-jev-go](https://github.com/mheers/typesafeai-systemone-jev-go) - Typed Go client for the TypeSafe System One API (Jev): structured questions and…
- [mhmdkzr/jev](https://github.com/mhmdkzr/jev) - TypeSafe.
- [monotykamary/jev-fabric](https://github.com/monotykamary/jev-fabric) - Native process orchestration with typed, explicit Jev decisions.
- [plurp911/jev-cli](https://github.com/plurp911/jev-cli) - Independent, unofficial CLI for TypeSafe AI.
- [Quintui/jev-use-cases](https://github.com/Quintui/jev-use-cases) - Demo app: Jev (TypeSafe System One) use cases with the AI SDK and shadcn/ui.
- [saif27217/jev-hermes](https://github.com/saif27217/jev-hermes) - Fast, cheap, typed decisions for AI agents — stdlib-only Python client for…
- [Shakibuzzaman3104/claude-jev-funnel](https://github.com/Shakibuzzaman3104/claude-jev-funnel) - Claude Code plugin + zero-dependency CLI for TypeSafe.
- [Sidneeuncharged29/jev-visual](https://github.com/Sidneeuncharged29/jev-visual) - Run vision-language model inference on Apple Silicon with Qwen3.5-0.8B via MLX;
- [sjungwon03/jev-langgraph-example](https://github.com/sjungwon03/jev-langgraph-example) - JEV: Proxmox MCP &amp; LangGraph Autonomous Infrastructure Platform.
- [snevadalabs/jev-kmp](https://github.com/snevadalabs/jev-kmp) - Kotlin Multiplatform SDK for the TypeSafe / Jev System One API — typed…
- [sushilduseja/jev-gateway](https://github.com/sushilduseja/jev-gateway) - OpenAI-compatible LLM gateway for the Groq API with pre-generation guardrail +…
- [Talya1412/jev-harness](https://github.com/Talya1412/jev-harness) - TypeSafe Jev (System One) integrations for OMP, MCP, Claude Code, and Pi…
- [xnuonux/jev-reflex](https://github.com/xnuonux/jev-reflex) - A portable Jev decision sidecar for coding agents: MCP, CLI, pi, source-bound…
- [Yasir-Khan-7/jev-sentinel](https://github.com/Yasir-Khan-7/jev-sentinel) - Prompt injection protection &amp; tool-call guardrails for LangChain / LangGraph AI…
- [yodablocks/duckdb-jev](https://github.com/yodablocks/duckdb-jev) - Semantic ORDER BY for DuckDB, backed by TypeSafe AI.
- [zotoio/jev-feature-demo](https://github.com/zotoio/jev-feature-demo) - Typesafe Jev (System One) feature demo — fixture-first CLI + JevClient;
- [kitfunso/hippo-memory](https://github.com/kitfunso/hippo-memory) - AI ajanları için biyolojik esinli bellek.
- [pithings/advocaat](https://github.com/pithings/advocaat) - TypeSafe Jev tarafından desteklenen, verileriniz hakkında yapay zekâya sorular…
- [giuliosmall/pg_typesafe](https://github.com/giuliosmall/pg_typesafe) - TypeSafe AI (Jev) kategorik sınıflandırması için alfa öncesi PostgreSQL uzantısı.
- [obie/ruby_decision_model](https://github.com/obie/ruby_decision_model) - Typesafe Jev gibi karar modelleri için Ruby istemcisi.
- [Das-rebel/a3m-router](https://github.com/Das-rebel/a3m-router) - ⚡ Uyarlanabilir çok modelli LLM yönlendirici — 80+ sağlayıcı, Jev System One…
- [zhulinchng/jevper](https://github.com/zhulinchng/jevper) - OpenAI benzeri istemciler üzerinde Jev biçimli (TypeSafe System One)…
- [gilljon/typesafe-ai-rs](https://github.com/gilljon/typesafe-ai-rs) - TypeSafe AI System One API için bağımsız asenkron ve bloklayan Rust SDK.
- [y0usaf/typesafe-cli](https://github.com/y0usaf/typesafe-cli) - Kabuktan Jev yazım denetimli sorular sorun: düzyazı yerine sayı olarak noul…
- [geilt/typesafe-cli](https://github.com/geilt/typesafe-cli) - TypeSafe System One için CLI ve aracı becerisi (Jev): türlenmiş Choice, Score…
- [JabbaKadabra/SystemOneDotNet](https://github.com/JabbaKadabra/SystemOneDotNet) - .NET client for TypeSafe System One (Jev) — typed questions in, typed answers…
- [Query-farm/vgi-typesafe](https://github.com/Query-farm/vgi-typesafe) - TypeSafe System One sorularını (choice, noul, score) DuckDB/SQL.

</details>

<a id="agent-tooling"></a>

## Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları

En hızlı büyüyen kategori: bir ajanın sonraki eyleminin önüne tipli bir karar koyan kancalar, MCP sunucuları ve geçitler.

<details>
<summary>🤖 <b><a href="https://github.com/tamaratran/fast-jev-compaction">tamaratran/fast-jev-compaction</a></b> · ⭐6768 · TypeScript · 🔎 inferred · 7 天 · ⭐+302</summary>

##### 📝 Özet

Claude sıkıştırma özetini Jev kararıyla değiştiren Code eklentisi: her araç çağrısı ve sonucu tek bir hızlı istekte puanlanır, eskimiş olanlar kaldırılır veya kısaltılır, korunan her şey aynen bırakılır.

> 💡 Bir kodlama aracısının bağlam sıkıştırma özetini tür belirtilmiş bir kararla değiştirir. Mevcut bir işlem hattını yeniden oluşturmak yerine, bu işlem hattındaki tek bir LLM çağrısını değiştirmenin temiz bir örneğidir.

##### 📌 Temel bilgiler

| Alan     | Değer                                                        |
| -------- | ------------------------------------------------------------ |
| Kategori | `Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları` |
| Kanıt    | `inferred`                                                   |
| Dil      | TypeScript                                                   |

##### 📊 Veri

| Ölçüt          | Değer           |
| -------------- | --------------- |
| Yıldız         | **6768** (+302) |
| Son push       | 2026-09-18      |
| İlk listelenme | 2026-09-18      |

</details>

<details>
<summary>🤖 <b><a href="https://github.com/gargpratyush/jev-router">gargpratyush/jev-router</a></b> · ⭐399 · JavaScript · 🔎 inferred · 5 天 · ⭐+44</summary>

##### 📝 Özet

jev-router kullanarak göreviniz için claude code'daki en ucuz modele yönlendirin

> 💡 Her turu, onu işleyebilecek en ucuz modele yönlendirir. Bir System One modeli için temel maliyet azaltma kullanım alanıdır.

##### 📌 Temel bilgiler

| Alan     | Değer                                                        |
| -------- | ------------------------------------------------------------ |
| Kategori | `Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları` |
| Kanıt    | `inferred`                                                   |
| Dil      | JavaScript                                                   |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **399** (+44) |
| Son push       | 2026-09-19    |
| İlk listelenme | 2026-09-18    |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/gargpratyush--jev-router/361cf042aa7f2e59.png" width="100%" alt="gargpratyush/jev-router screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🤖 <b><a href="https://github.com/0xNatoshi/jev-codex-router">0xNatoshi/jev-codex-router</a></b> · ⭐272 · JavaScript · 🔎 inferred · 3 天 · ⭐+29</summary>

##### 📝 Özet

Codex için tur başına model ve akıl yürütme yönlendirmesi; Jev (TypeSafe System One) tarafından yönlendirilir: her tur için modeli, düşünme derinliğini ve hız modunu seçer.

> 💡 Tür belirtilmiş kararlarla yönlendirilen bir kodlama aracısı için tur başına model ve akıl yürütme çabası yönlendirmesi.

##### 📌 Temel bilgiler

| Alan     | Değer                                                        |
| -------- | ------------------------------------------------------------ |
| Kategori | `Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları` |
| Kanıt    | `inferred`                                                   |
| Dil      | JavaScript                                                   |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **272** (+29) |
| Son push       | 2026-09-22    |
| İlk listelenme | 2026-09-18    |

🏷 `ai` · `codex` · `jev` · `llm` · `macos` · `routing` · `typesafe`

</details>

<details>
<summary>🤖 <b><a href="https://github.com/AbdelStark/awesome-typesafe-jev">AbdelStark/awesome-typesafe-jev</a></b> · ⭐513 · HTML · 👁️ observed · 0 天 · ⭐+26</summary>

##### 📝 Özet

Awesome Jev: TypeSafe'in System One modeline yönelik, SDKs, canlı demolar, ajan araçları ve bağımsız değerlendirmeler içeren kaynak destekli bir saha rehberi.

##### 📌 Temel bilgiler

| Alan     | Değer                                                        |
| -------- | ------------------------------------------------------------ |
| Kategori | `Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları` |
| Kanıt    | `observed`                                                   |
| Dil      | HTML                                                         |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **513** (+26) |
| Son push       | 2026-09-24    |
| İlk listelenme | 2026-09-21    |

🏷 `agent-skills` · `ai-agents` · `awesome` · `awesome-jev` · `awesome-list` · `awesome-lists` · `curated-list` · `developer-tools`

</details>

<details>
<summary>🤖 <b><a href="https://github.com/fatwang2/awesome-jev">fatwang2/awesome-jev</a></b> · ⭐206 · JavaScript · 👁️ observed · 1 天 · ⭐+5</summary>

##### 📝 Özet

Yeniden kullanılabilir, yalnızca Jev kullanan GitHub inceleme iş akışına sahip, kaynaklarla desteklenen bir Jev proje dizini.

<sub>🔧 Kodda kullanıldığı tespit edildi: `entries/nshkrdotcom--typesafe_sdk.json`, `entries/vinnie357--typesafe_sdk_ex.json`</sub>

##### 📌 Temel bilgiler

| Alan     | Değer                                                        |
| -------- | ------------------------------------------------------------ |
| Kategori | `Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları` |
| Kanıt    | `observed`                                                   |
| Dil      | JavaScript                                                   |

##### 📊 Veri

| Ölçüt          | Değer        |
| -------------- | ------------ |
| Yıldız         | **206** (+5) |
| Son push       | 2026-09-24   |
| İlk listelenme | 2026-09-18   |

🏷 `awesome` · `awesome-list` · `github-actions` · `jev` · `open-source` · `typesafe`

</details>

<details>
<summary>🤖 <b><a href="https://github.com/valentynkit/awesome-jev-typesafe">valentynkit/awesome-jev-typesafe</a></b> · ⭐138 · JavaScript · 👁️ observed · 0 天 · ⭐+8</summary>

##### 📝 Özet

TypeSafe's Jev ile yazım denetimli kararlar, ilk System One modeli

##### 📌 Temel bilgiler

| Alan     | Değer                                                        |
| -------- | ------------------------------------------------------------ |
| Kategori | `Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları` |
| Kanıt    | `observed`                                                   |
| Dil      | JavaScript                                                   |

##### 📊 Veri

| Ölçüt          | Değer        |
| -------------- | ------------ |
| Yıldız         | **138** (+8) |
| Son push       | 2026-09-24   |
| İlk listelenme | 2026-09-19   |

🏷 `ai-agents` · `awesome` · `awesome-list` · `claude-code` · `directory` · `jev` · `llm-agents` · `system-one`

</details>

<details>
<summary>🤖 <b><a href="https://github.com/kraayenjon/awesome-jev">kraayenjon/awesome-jev</a></b> · ⭐137 · 👁️ observed · 4 天 · ⭐+21</summary>

##### 📝 Özet

Jev kullanım alanları, projeleri, SDKs ve kaynaklarından derlenmiş bir liste. Jev, yazılımda hızlı, yazım denetimli kararlar için TypeSafe AI's System One modelidir — kalibre edilmiş olasılıklarla Choice, Score ve Noul.

##### 📌 Temel bilgiler

| Alan     | Değer                                                        |
| -------- | ------------------------------------------------------------ |
| Kategori | `Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları` |
| Kanıt    | `observed`                                                   |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **137** (+21) |
| Son push       | 2026-09-21    |
| İlk listelenme | 2026-09-19    |

🏷 `agents` · `ai` · `ai-agents` · `api` · `artificial-intelligence` · `automation` · `awesome` · `awesome-list`

</details>

<details>
<summary>🤖 <b><a href="https://github.com/dbreunig/building-with-jev-skill">dbreunig/building-with-jev-skill</a></b> · ⭐133 · 👁️ observed · 7 天 · ⭐+4</summary>

##### 📝 Özet

Jev, TypeSafe'in System One modelini çağıran programlar yazmak ve geliştirmek için bir beceri

> 💡 Jev'i çağıran bir program yerine Jev'ı çağıran programlar yazmak için bir beceri. Ayrım önemlidir: tek bir uygulamayı değil, tasarım kurallarını kodlar.

##### 📌 Temel bilgiler

| Alan     | Değer                                                        |
| -------- | ------------------------------------------------------------ |
| Kategori | `Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları` |
| Kanıt    | `observed`                                                   |

##### 📊 Veri

| Ölçüt          | Değer        |
| -------------- | ------------ |
| Yıldız         | **133** (+4) |
| Son push       | 2026-09-17   |
| İlk listelenme | 2026-09-18   |

</details>

<details>
<summary>🤖 <b><a href="https://github.com/GhalebDweikat/winnow">GhalebDweikat/winnow</a></b> · ⭐86 · Python · 👁️ observed · 6 天 · ⭐+14</summary>

##### 📝 Özet

Claude Code için kalibre edilmiş bir bağlam eleği: her araç sonucu bağlama girmeden önce bir System One modeli tarafından değerlendirilir.

##### 📌 Temel bilgiler

| Alan     | Değer                                                        |
| -------- | ------------------------------------------------------------ |
| Kategori | `Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları` |
| Kanıt    | `observed`                                                   |
| Dil      | Python                                                       |

##### 📊 Veri

| Ölçüt          | Değer        |
| -------------- | ------------ |
| Yıldız         | **86** (+14) |
| Son push       | 2026-09-19   |
| İlk listelenme | 2026-09-18   |

🏷 `claude-code` · `claude-code-plugin` · `context-management` · `jev` · `llm-agents` · `typesafe`

</details>

<details>
<summary>🤖 <b><a href="https://github.com/carlaiau/jev-reranking">carlaiau/jev-reranking</a></b> · ⭐9 · Python · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

TREC koleksiyonları üzerinde arama motoru denemeleri. Şu anda typesafe.ai'nin JEV modeliyle sıfır örnekli yeniden sıralama uygulamalarına odaklanıyor

##### 📌 Temel bilgiler

| Alan     | Değer                                                        |
| -------- | ------------------------------------------------------------ |
| Kategori | `Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları` |
| Kanıt    | `observed`                                                   |
| Dil      | Python                                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **9**      |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-25 |

🏷 `agentic-researcher` · `autoresearch` · `bm25` · `information-retrieval` · `trec` · `trec-eval`

</details>

<details>
<summary>🤖 <b><a href="https://github.com/ChenneyZhuang/laya-browser-agent">ChenneyZhuang/laya-browser-agent</a></b> · ⭐9 · Python · 👁️ observed · 0 天 · ⭐+4</summary>

##### 📝 Özet

Yerel, açık kaynaklı Jev alternatifi: kendi makinenizde Laya (System One modeli) ile tarayıcı etmeni kararları. Bulut yok, API anahtarı yok. Playwright/CDP, MCP dostu.

##### 📌 Temel bilgiler

| Alan     | Değer                                                        |
| -------- | ------------------------------------------------------------ |
| Kategori | `Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları` |
| Kanıt    | `observed`                                                   |
| Dil      | Python                                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **9** (+4) |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-21 |

🏷 `agent` · `ai-agents` · `apple-silicon` · `browser-agent` · `browser-automation` · `computer-use` · `decision-model` · `jev`

</details>

<details>
<summary>🤖 <b><a href="https://github.com/caijinchun/nanojev-arena">caijinchun/nanojev-arena</a></b> · ⭐7 · HTML · 👁️ observed · 5 天</summary>

##### 📝 Özet

NanoJev Snake Arena: 1'e 4 insan-AI savaş gemisi savaşları + 100 ajanlı sürü simülatörü. Jev System-One modelinin yerel demosu (açık kaynaklı mini kopya).

##### 📌 Temel bilgiler

| Alan     | Değer                                                        |
| -------- | ------------------------------------------------------------ |
| Kategori | `Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları` |
| Kanıt    | `observed`                                                   |
| Dil      | HTML                                                         |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **7**      |
| Son push       | 2026-09-19 |
| İlk listelenme | 2026-09-20 |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/caijinchun--nanojev-arena/9b519386aa658fa3.png" width="100%" alt="caijinchun/nanojev-arena screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🤖 <b><a href="https://github.com/yohanargentina-oss/Foq">yohanargentina-oss/Foq</a></b> · ⭐7 · Python · 👁️ observed · 4 天 · ⭐+1</summary>

##### 📝 Özet

⚡ Foq — Jev'e ücretsiz, yerel, açık kaynaklı alternatif. Yaklaşık 25 ms'de tiplendirilmiş System 1 kararları — bekleme listesi yok, bulut yok, token başına ücret yok. foq.fr

##### 📌 Temel bilgiler

| Alan     | Değer                                                        |
| -------- | ------------------------------------------------------------ |
| Kategori | `Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları` |
| Kanıt    | `observed`                                                   |
| Dil      | Python                                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **7** (+1) |
| Son push       | 2026-09-20 |
| İlk listelenme | 2026-09-20 |

🏷 `agentic-ai` · `ai` · `decision-engine` · `foq` · `gguf` · `inference` · `jev` · `jev-alternative`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/yohanargentina-oss--foq/95e5ad1668a398fd.png" width="100%" alt="yohanargentina-oss/Foq screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🤖 <b><a href="https://github.com/mansicer/jev-plays">mansicer/jev-plays</a></b> · ⭐3 · Python · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

A System One model (jev) plays Craftax; an LLM sets the goals

##### 📌 Temel bilgiler

| Alan     | Değer                                                        |
| -------- | ------------------------------------------------------------ |
| Kategori | `Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları` |
| Kanıt    | `observed`                                                   |
| Dil      | Python                                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **3**      |
| Son push       | 2026-09-24 |
| İlk listelenme | 2026-09-25 |

🏷 `craftax` · `crafter` · `game-agent` · `jev` · `llm-agent` · `system-one` · `typesafe`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/mansicer--jev-plays/f2257590ff0d0bd8.png" width="100%" alt="mansicer/jev-plays screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/mansicer--jev-plays/c4f1ec41afcb4ae9.gif" width="100%" alt="mansicer/jev-plays animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

</details>

<details>
<summary>🤖 <b><a href="https://github.com/KNambiarDJsc/second-thought">KNambiarDJsc/second-thought</a></b> · ⭐1 · Python · 👁️ observed · 2 天</summary>

##### 📝 Özet

Learning infrastructure for typed probabilistic decisions from System One models (Laya, and typed-decision providers you bring yourself).

##### 📌 Temel bilgiler

| Alan     | Değer                                                        |
| -------- | ------------------------------------------------------------ |
| Kategori | `Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları` |
| Kanıt    | `observed`                                                   |
| Dil      | Python                                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **1**      |
| Son push       | 2026-09-23 |
| İlk listelenme | 2026-09-22 |

🏷 `active-learning` · `ai-agents` · `calibration` · `drift-detection` · `fine-tuning` · `human-in-the-loop` · `jev` · `laya`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/knambiardjsc--second-thought/5ebe54ed2a7f082e.jpg" width="100%" alt="KNambiarDJsc/second-thought screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🤖 <b><a href="https://github.com/cedarmuse-creator/jev-decision-maker-at-meteora">cedarmuse-creator/jev-decision-maker-at-meteora</a></b> · Python · 👁️ observed · 0 天</summary>

##### 📝 Özet

Decision Maker at Meteora - a decision agent for Meteora DLMM liquidity on Solana. Named after Jev, TypeSafe AI's System One model (https://console.typesafe.ai/home), which answers its typed Noul and Score questions. Ranks the live pool universe, runs live rug cards, and holds 3-5 pools under explicit hard gates and book rules.

##### 📌 Temel bilgiler

| Alan     | Değer                                                        |
| -------- | ------------------------------------------------------------ |
| Kategori | `Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları` |
| Kanıt    | `observed`                                                   |
| Dil      | Python                                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-24 |
| İlk listelenme | 2026-09-21 |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/cedarmuse-creator/jev-decision-maker-at-meteora/main/jev-flowchart.png" width="100%" alt="cedarmuse-creator/jev-decision-maker-at-meteora screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

<sub>Yeniden dağıtım lisansı belirtilmediği için varlık üst depodan doğrudan bağlanmıştır.</sub>

</details>

<details>
<summary>🤖 <b><a href="https://github.com/NatBrian/pokemon-showdown-jev-agent">NatBrian/pokemon-showdown-jev-agent</a></b> · JavaScript · 👁️ observed · 2 天</summary>

##### 📝 Özet

Autonomous Pokemon Showdown battle agent powered by Jev AI (System One Model by TypeSafe AI), with a validated poke-env harness, safe fallback decisions, and a live dashboard for transparent battle state, legal moves, probabilities, telemetry, and Jev decision history.

##### 📌 Temel bilgiler

| Alan     | Değer                                                        |
| -------- | ------------------------------------------------------------ |
| Kategori | `Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları` |
| Kanıt    | `observed`                                                   |
| Dil      | JavaScript                                                   |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-22 |
| İlk listelenme | 2026-09-22 |

🏷 `ai-agent` · `ai-observability` · `autonomous-agent` · `battle-ai` · `decision-engine` · `fastapi` · `game-ai` · `jev`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/NatBrian/pokemon-showdown-jev-agent/master/artifacts/infographic.png" width="100%" alt="NatBrian/pokemon-showdown-jev-agent screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

<sub>Yeniden dağıtım lisansı belirtilmediği için varlık üst depodan doğrudan bağlanmıştır.</sub>

</details>

<details>
<summary>🤖 <b><a href="https://github.com/Shai-Koffman/jev-web-agent">Shai-Koffman/jev-web-agent</a></b> · Python · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

A web-browsing agent driven by Jev (TypeSafe's System One model): Playwright + typed, calibrated decisions, confidence gate, human in the loop

##### 📌 Temel bilgiler

| Alan     | Değer                                                        |
| -------- | ------------------------------------------------------------ |
| Kategori | `Ajan araçları: MCP, kancalar, geçitler ve kodlama ajanları` |
| Kanıt    | `observed`                                                   |
| Dil      | Python                                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-24 |
| İlk listelenme | 2026-09-25 |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/shai-koffman--jev-web-agent/f7378c804eabb474.png" width="100%" alt="Shai-Koffman/jev-web-agent screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary><b>Bu kategoride daha fazla</b> <sub>· 231</sub></summary>

- [BYK/jev-mcp](https://github.com/BYK/jev-mcp) - TypeSafe.
- [kolawong/fast-compaction-dsh](https://github.com/kolawong/fast-compaction-dsh) - Verdict-based context compaction for DeepSeek Harness — replaces lossy LLM…
- [rajasekharponakala/jev-mcp](https://github.com/rajasekharponakala/jev-mcp) - MCP server wrapping TypeSafe.
- [andyrewlee/awesome-system-one](https://github.com/andyrewlee/awesome-system-one) - A curated list of System One models, open implementations, agents, SDKs, and…
- [Excalibur9527/dsh-jev](https://github.com/Excalibur9527/dsh-jev) - DSH Plugin（DeepSeek Harness 插件）：每轮对话调用 typesafe.ai systemone(jev)…
- [JoasASantos/Raze](https://github.com/JoasASantos/Raze) - Offensive-security agent built on Raze, a System One model in the Jev family…
- [llt22/jev-lab](https://github.com/llt22/jev-lab) - TypeSafe.
- [ahoo/cpa-plugin-systemone](https://github.com/ahoo/cpa-plugin-systemone) - Native SystemOne (Jev) provider for CLIProxyAPI: chat-compatible jev-1.13…
- [api-evangelist/typesafe-ai](https://github.com/api-evangelist/typesafe-ai) - TypeSafe AI is a San Francisco AI lab building System One models — a class of…
- [BargLabs/jev-judge-calibration](https://github.com/BargLabs/jev-judge-calibration) - Preregistered calibration test of TypeSafe Jev 1.13 as a judge of agent…
- [BFLabsAI/bf-jev-deep-research](https://github.com/BFLabsAI/bf-jev-deep-research) - Agent Skill + estudo verbatim sobre a Jev (TypeSafe AI System One Model)…
- [ChenneyZhuang/laya-computer-use](https://github.com/ChenneyZhuang/laya-computer-use) - Local computer use on macOS with Laya decisions — read any window.
- [CrowBe/weave](https://github.com/CrowBe/weave) - Agent Harness for System One model.
- [dakotac1994/awesome-jev](https://github.com/dakotac1994/awesome-jev) - Awesome list for Jev — TypeSafe AI.
- [dfranco-projects/jev-guardbench](https://github.com/dfranco-projects/jev-guardbench) - Can a System One model (TypeSafe Jev, open-source Kev) replace an LLM-as-judge…
- [Jev-trading/Jev-trading](https://github.com/Jev-trading/Jev-trading) - Jev-trading is a desktop application that combines the ultra-fast neural…
- [jms-dcksn/uipath-jev-guardrail-connector](https://github.com/jms-dcksn/uipath-jev-guardrail-connector) - TypeSafe Jev System One modeliyle desteklenen UiPath kendi guardrail.
- [mani-aiml/jev-demos](https://github.com/mani-aiml/jev-demos) - Demos with Jev, TypeSafe.
- [ringozzt/jev-research](https://github.com/ringozzt/jev-research) - Jev (TypeSafe AI System One Model) 技术趋势调研：GitHub 生态、工程应用与 Agent Harness 落地建议.
- [SnackTerminator/small-decision-model-cross-domain-degradation](https://github.com/SnackTerminator/small-decision-model-cross-domain-degradation) - Jev-class small decision models (System One, 395M-596M): controlled evaluation…
- [stern9/jev-bench](https://github.com/stern9/jev-bench) - Benchmark TypeSafe.
- [TareqAlhashash/jev-claude-codereviewer](https://github.com/TareqAlhashash/jev-claude-codereviewer) - AI code reviewer demo pairing Jev (typesafe.ai) instant triage with an optional…
- [TobyNoSkillSon/Verdict](https://github.com/TobyNoSkillSon/Verdict) - A local runtime for System One models (decision models like Laya and Jev) on…
- [browser-use/jev-ultrafast](https://github.com/browser-use/jev-ultrafast) - En hızlı ve en ucuz web aracısı.
- [YaoApp/yao](https://github.com/YaoApp/yao) - ✨ Tüm ajanlarınız ve çalışma alanlarınız, sahip olduğunuz her cihazda tek bir…
- [jev-chat/jev-chat-jarvis](https://github.com/jev-chat/jev-chat-jarvis) - Telefona yüklenen sohbet yardımcısı: QQ / X / 飞书 içindeki karşı tarafı anlar…
- [reticlehq/reticle](https://github.com/reticlehq/reticle) - AI ajanları kod üretebilir, ancak oluşturduklarını anlamakta hâlâ zorlanır.
- [kerpopule/hermes-jev-skills](https://github.com/kerpopule/hermes-jev-skills) - Hermes aracıları için Jev destekli model yönlendirme, bellek, sıkıştırma…
- [devagrawal09/jev-review](https://github.com/devagrawal09/jev-review) - TypeSafe Jev ile oluşturulmuş aşamalı kod inceleme iş akışı ve yerel pano.
- [logicrw/awesome-jev-projects](https://github.com/logicrw/awesome-jev-projects) - Harika Jev: kaynaklarla desteklenen açık kaynak ekosistemi radarı, sade dille…
- [wy-coliney/jev-browser-use](https://github.com/wy-coliney/jev-browser-use) - Tarayıcı işlemleri 5–10 kat daha hızlı: Jev tıklar, Codex düşünür ve doğrular.
- [wuyoscar/jev-skill](https://github.com/wuyoscar/jev-skill) - Jev kullanım örnekleri, iş akışları ve aracı becerilerinden oluşan harika bir…
- [kydlikebtc/awesome-jev](https://github.com/kydlikebtc/awesome-jev) - Jev için 1207 herkese açık kaynak, karar kalıbına göre indekslenmiş TypeSafe…
- [jkudish/jev-mcp](https://github.com/jkudish/jev-mcp) - TypeSafe.
- [Alex314618-create/JevRev](https://github.com/Alex314618-create/JevRev) - HER ŞEYİ değiştiren bir LLM + Jev iş akışı.
- [malevrigns/agent-jev](https://github.com/malevrigns/agent-jev) - AgentJev-0.6B — AI ajanları için hızlı bir &#x27;System One&#x27; karar modeli: herhangi…
- [Heman10x-NGU/openJev-verdict-2.0](https://github.com/Heman10x-NGU/openJev-verdict-2.0) - TypeSafe Jev ve LocalLLaMA/typed-decisions üzerinde Laya.
- [NiazMorshed2007/jev-review](https://github.com/NiazMorshed2007/jev-review) - Jev tarafından desteklenen, yapay zekâ kodlama aracılarıyla sürekli yazılım…
- [BillionsBobby/JevRouter](https://github.com/BillionsBobby/JevRouter) - Modeller, araçlar ve alt aracılar için Jev destekli hafif bir yönlendirici.
- [vinilana/jev-gateway](https://github.com/vinilana/jev-gateway) - Araç çağırma akıl yürütmesi için jev.
- [walidboulanouar/awesome-jev-use-cases](https://github.com/walidboulanouar/awesome-jev-use-cases) - TypeSafe AI Jev kullanım örneklerinin harika listesi: beğeniye göre sıralanmış…
- [RomanSlack/jev-drone](https://github.com/RomanSlack/jev-drone) - MuJoCo&#x27;da, döngü içinde 2,5 Hz&#x27;de küçük bir yargı modeli (TypeSafe Jev) bulunan…
- [Amal-David/awesome-jev](https://github.com/Amal-David/awesome-jev) - Jev demoları, projeleri, SDKs ve becerileri;
- [tamaratran/jev-pruner](https://github.com/tamaratran/jev-pruner) - Claude Code eklentisi: model görmeden önce uzun Bash çıktısını TypeSafe Jev ile…
- [y0usaf/pi-jev](https://github.com/y0usaf/pi-jev) - Pi kodlama aracısı için karar katmanı olarak TypeSafe Jev: ölçümlenmiş bir araç…
- [supercorp-ai/supercov](https://github.com/supercorp-ai/supercov) - Kodlama ajanları için kapsam, güvenlik ve kod kalitesi.
- [cookiespiggy/agentic-rl](https://github.com/cookiespiggy/agentic-rl) - Agentic RL Çince sıfırdan eğitim (25 bölüm): kavramlardan GRPO uygulamasına…
- [openlayer-ai/jevals](https://github.com/openlayer-ai/jevals) - Jev kararları olarak ajan değerlendirmeleri ve koruma kuralları: iz başına bir…
- [Bodila51/grok-bot-jev](https://github.com/Bodila51/grok-bot-jev) - TypeSafe Jev&#x27;i Grok Bot&#x27;a ucuz bir karar katmanı olarak bağlayın - kullanım…
- [angel291592/Intent-Router](https://github.com/angel291592/Intent-Router) - AI ajanları için amaç derleyicisi — belirsiz istekleri türlendirilmiş…
- [socai-io/jev-social](https://github.com/socai-io/jev-social) - Açık kaynaklı Agent Skill ve yerel demo: Instagram, TikTok ve LinkedIn…
- [yikangy873-gif/jev-desktop](https://github.com/yikangy873-gif/jev-desktop) - Codex Computer Use içinde TypeSafe Jev eylem seçimi.
- [ThinkFlowLab/system1-agents](https://github.com/ThinkFlowLab/system1-agents) - Aracılar için beyin görevi gören System 1 karar modelleri (Jev, Laya, Cua-S1)…
- [iamaamir/system-one](https://github.com/iamaamir/system-one) - TypeScript ve Pi için sağlayıcıdan bağımsız System One çalışma zamanı.
- [Nisaka520/JevIntent](https://github.com/Nisaka520/JevIntent) - 微信（FkWeChat eklentisi）：Mesajı uzun basarak niyeti / duyguyu / yanıt tutumunu…
- [TheoOliveira/pi-jev](https://github.com/TheoOliveira/pi-jev) - TypeSafe Jev kullanarak Pi kodlama aracısı için anlamsal araç yönlendirmesi ve…
- [intikhab49/open-jev-typed-decision-engine](https://github.com/intikhab49/open-jev-typed-decision-engine) - TypeSafe Jev.
- [shantanugoel/ask-jev-skill](https://github.com/shantanugoel/ask-jev-skill) - Hermes ve diğer aracılar için typesafe&#x27;ın jev&#x27;ine soru sorma becerisi.
- [samdotmak/jev-recall](https://github.com/samdotmak/jev-recall) - Benzerliğe göre değil, ilgi düzeyine göre alın: Bir AI asistanının anılarını…
- [OmniJev/PlayJev](https://github.com/OmniJev/PlayJev) - 🚀🚀 Ham piksellerden doğrudan GUI oyunları oynayan 0.8B JEV benzeri çok modlu…
- [jonathanavis96/jev-kit](https://github.com/jonathanavis96/jev-kit) - TypeSafe&#x27;ın Jev&#x27;ini Claude Code ile çalıştırmak için ihtiyacınız olan her şey…
- [PyModel/jev-judge-mcp](https://github.com/PyModel/jev-judge-mcp) - MCP ajanları için türlendirilmiş yargı araçları.
- [PerryLink/jevcore](https://github.com/PerryLink/jevcore) - DeepSeek Harness, Model Context Protocol ve yalın Node için TypeSafe Jev;
- [Zaious/jev-capability-atlas](https://github.com/Zaious/jev-capability-atlas) - TypeSafe&#x27;s Jev&#x27;in gerçekte ne zaman ayakta kaldığına ve ne zaman çöktüğüne dair…
- [everyinfra/jev-radar](https://github.com/everyinfra/jev-radar) - 📡 Eksiksiz · Jev (TypeSafe AI System One) ekosisteminin dünyanın en kapsamlı…
- [jomatsu/pi-jev-auto-mode](https://github.com/jomatsu/pi-jev-auto-mode) - Pi kodlama aracısı için Jev (TypeSafe System One) destekli otomatik mod: bash…
- [FrancoisChastel/jev-code](https://github.com/FrancoisChastel/jev-code) - Jev, TypeSafe.
- [anandi1989/awesome-jev-usecases](https://github.com/anandi1989/awesome-jev-usecases) - Gerçek dünyadaki Jev (TypeSafe AI System One) kullanım örneklerinin kanıt…
- [khordoo/jev-reflex-autonomy-lab](https://github.com/khordoo/jev-reflex-autonomy-lab) - Hızlı Sistem 1 refleks kararları ve çarpışmadan kaçınma için Jev kullanan…
- [0x7067/claude-jev](https://github.com/0x7067/claude-jev) - Claude Code eklentisi: kural denetimleri, birebir sıkıştırma ve istem…
- [abhishek085/open-spark-jev](https://github.com/abhishek085/open-spark-jev) - TypeSafe.
- [bgivenb/flick-computer-use](https://github.com/bgivenb/flick-computer-use) - MCP ajanları için hızlı tarayıcı ve macOS bilgisayar kullanımı.
- [philippdubach/pi-jev-router](https://github.com/philippdubach/pi-jev-router) - Jev tabanlı, pi için minimal Pareto-optimal OpenRouter model yönlendiricisi.
- [iamtoomas/JevLint](https://github.com/iamtoomas/JevLint) - Jev tarafından desteklenen yapılandırılabilir anlamsal linting;
- [24601/Augustus](https://github.com/24601/Augustus) - Karar-modeli sistemlerini bulma, oluşturma, değerlendirme ve iyileştirme için…
- [ranjan2829/AskJev](https://github.com/ranjan2829/AskJev) - AskJev — herhangi bir web sitesi için Jev otomatik pilotu + geri döndürülemez…
- [ZephyrDeng/ego-jev](https://github.com/ZephyrDeng/ego-jev) - ego-browser için Jev (TypeSafe System One) iç döngüsü — bir LLM turu yerine her…
- [daftAI2026/awesome-jev](https://github.com/daftAI2026/awesome-jev) - Seçkili TypeSafe Jev / System One GitHub projeleri, açık kaynak alternatifleri…
- [inso1337/revl](https://github.com/inso1337/revl) - Güvenli, evrensel uzay-zamansal birleştirilebilirlik (Cordis paradigması) ve…
- [AskTheWay/dsh-jev-interceptor](https://github.com/AskTheWay/dsh-jev-interceptor) - DeepSeek Harness.
- [ismaelsoilet/jev-harness](https://github.com/ismaelsoilet/jev-harness) - Sıfır bağımlılıklı System One karar düzeneği: önemsiz hatalarda ve çıkmaz…
- [ajensenwaud/hermes-jev-plugin](https://github.com/ajensenwaud/hermes-jev-plugin) - TypeSafe Jev (System One) decision tools for Hermes Agent: jev_check /…
- [erkamyaman/jev-enforce](https://github.com/erkamyaman/jev-enforce) - 📏 Claude Code plugin that makes Claude follow your AGENTS.md: every reply and…
- [imMamdouhaboammar/get-fable](https://github.com/imMamdouhaboammar/get-fable) - Make the model you already use work more like a frontier model with better…
- [kinde-starter-kits/jev-agent-authorization](https://github.com/kinde-starter-kits/jev-agent-authorization) - Jev agent authorization for MCP tool calls: Kinde identity and permissions plus…
- [rthomas24/jev-realtime-trading](https://github.com/rthomas24/jev-realtime-trading) - TypeSafe.
- [zhangxaochen/dsh-jev](https://github.com/zhangxaochen/dsh-jev) - DeepSeek Harness (dsh) için Jev (System One karar modeli) eklenti paketi.
- [abhishekashokvkumar/jev-mcp-dispatcher](https://github.com/abhishekashokvkumar/jev-mcp-dispatcher) - Tamamen TypeSafe.
- [noetion/dsh-jev](https://github.com/noetion/dsh-jev) - TypeSafe Jev noul, choice ve score yanıtları için jev_ask kaydeden DSH paketi.
- [parkavenue9639/jevloop](https://github.com/parkavenue9639/jevloop) - A Jev-driven general-purpose agent harness for faster, lower-cost execution…
- [24601/rh-guard](https://github.com/24601/rh-guard) - Reward-hack radar for coding agents: structural denies + TypeSafe Jev System…
- [alexei-led/pi-model-router](https://github.com/alexei-led/pi-model-router) - Independent Pi extension for four-tier model routing with optional…
- [Charlyhno-eng/jev-codex-pilot](https://github.com/Charlyhno-eng/jev-codex-pilot) - Smart Codex overlay with JEV model routing, context optimization &amp; Kanban…
- [deepansh-saxena/jev-guardrails](https://github.com/deepansh-saxena/jev-guardrails) - Comparing LLM-as-judge vs TypeSafe Jev for agent guardrails: same rules, same…
- [Emenowicz/jev-sap-commerce](https://github.com/Emenowicz/jev-sap-commerce) - SAP Commerce extension using TypeSafe.
- [godspede/construct-auto-classifier](https://github.com/godspede/construct-auto-classifier) - Effect-based safety gate for AI coding agents.
- [JustineDevs/meta-architect](https://github.com/JustineDevs/meta-architect) - Meta-Architect (MA) is a workflow layer that adds architecture, evidence, and…
- [nanami-0713/dsh-jev-decide](https://github.com/nanami-0713/dsh-jev-decide) - DSH plugin: register TypeSafe Jev (System One decision model) as an agent tool…
- [pCwOrM/werr](https://github.com/pCwOrM/werr) - Zero-memory System-1 decision engine &amp; TypeSafe Jev wire-compatible runtime…
- [samtay32/jev-system-architect](https://github.com/samtay32/jev-system-architect) - TypeSafe AI Jev/System One için sistem mimarisi becerisi — belirsiz anlamsal…
- [shimo4228/jev-skill-router](https://github.com/shimo4228/jev-skill-router) - Claude Code plugin: asks TypeSafe Jev which installed skill fits each prompt…
- [simota/tenbin](https://github.com/simota/tenbin) - MCP server and agent skill for the TypeSafe AI System One API (Jev): decompose…
- [wangkuangkuang/jev-mcp-server](https://github.com/wangkuangkuang/jev-mcp-server) - MCP server for Jev (TypeSafe System One): the three official question types…
- [cnrai/openpave-jev](https://github.com/cnrai/openpave-jev) - PAVE skill for non-autoregressive decision models (Jev / Laya / TypeSafe)…
- [CrowdLinker/JevPromptCoach](https://github.com/CrowdLinker/JevPromptCoach) - Claude Code plugin that scores how well you prompt a coding agent, and shows…
- [Ex8-ca/jev-router](https://github.com/Ex8-ca/jev-router) - Hermes plugin for TypeSafe Jev decision routing via OpenRouter + session-start…
- [Jessie-QingYu/jev-in-the-wild](https://github.com/Jessie-QingYu/jev-in-the-wild) - Real-world Jev use cases, open-source projects, benchmarks and criticism — what…
- [jms-dcksn/jev-pii-guardrail](https://github.com/jms-dcksn/jev-pii-guardrail) - LLM sınırında özel PII algılama guardrail.
- [omni-/ask-jev](https://github.com/omni-/ask-jev) - Ajan tabanlı kodlama oturumlarını bağımsız ve düşük maliyetli biçimde…
- [robokrunch/awesome-jev](https://github.com/robokrunch/awesome-jev) - A curated list of resources for Jev — TypeSafe AI.
- [0xshikhar/jev-fuse](https://github.com/0xshikhar/jev-fuse) - A governed execution layer between typed-decision models (TypeSafe Jev) and…
- [aitofy-dev/jev-awesome-skills](https://github.com/aitofy-dev/jev-awesome-skills) - Open-source Jev skills for Claude Code, Codex, Cursor, and Grok.
- [alexadark/jev-openrouter-skill](https://github.com/alexadark/jev-openrouter-skill) - A Codex skill that asks TypeSafe Jev through OpenRouter which model and…
- [cbruyndoncx/AskJev-MCP](https://github.com/cbruyndoncx/AskJev-MCP) - MCP server for TypeSafe.
- [charetterat/awesome-jev-essentials](https://github.com/charetterat/awesome-jev-essentials) - The Jev projects actually worth your time — picked, compared, kept current.
- [CompleteDotTech/jev-factorio-agent](https://github.com/CompleteDotTech/jev-factorio-agent) - Jev picks what, code owns how - a System One Factorio agent driven by…
- [ctaxnagomi/instruct-jev](https://github.com/ctaxnagomi/instruct-jev) - INSTRUCT_JEV - TypeSafe AI Jev / System One instruction corpus…
- [darwintechlab/claude-jev](https://github.com/darwintechlab/claude-jev) - Jev System One for Claude Code — typed Choice/Noul/Score via live TypeSafe API…
- [de-niji/jev-hermes](https://github.com/de-niji/jev-hermes) - Jev for Hermes Agent: cheap typed decisions via TypeSafe Jev on OpenRouter.
- [DoGMaTiiC/hermes-jev](https://github.com/DoGMaTiiC/hermes-jev) - Hermes Agent plugin: route each turn to the one skill that fits, via TypeSafe…
- [holasoymalva/jev-test-impact](https://github.com/holasoymalva/jev-test-impact) - Ultra-fast test impact analysis powered by Jev. Run only the tests that matter.
- [ieee0824/jev-mcp](https://github.com/ieee0824/jev-mcp) - A Rust MCP server for TypeSafe AI Jev structured decisions.
- [jiawei686/jev-legal-clause-mcp](https://github.com/jiawei686/jev-legal-clause-mcp) - 合同条款风险点标注 MCP (Jev noul): 10 类风险概率 + 人工复审建议.
- [jkudish/jev-agent-tools](https://github.com/jkudish/jev-agent-tools) - Jev transport/provider layer: multi-provider transport layer that supports…
- [JYeswak/jev_playground](https://github.com/JYeswak/jev_playground) - Measure what Jev can actually do before you build on it.
- [laguagu/jev-skills](https://github.com/laguagu/jev-skills) - Practical agent skills and examples for building with Jev.
- [legostin/jev-mcp](https://github.com/legostin/jev-mcp) - MCP server that lets Claude Code, Codex &amp; AI agents drive a real Chrome browser…
- [liu-x27/XavierJev](https://github.com/liu-x27/XavierJev) - A decision layer for agent control flow: yes/no, choice and rubric questions…
- [luw2007/omp-jev-extensions](https://github.com/luw2007/omp-jev-extensions) - OMP / pi-coding-agent extensions that delegate acceptance gating and subagent…
- [mywwave/cursor-jev](https://github.com/mywwave/cursor-jev) - Cursor plugin that routes subagents with TypeSafe Jev and exposes Choice…
- [Panebianco00/jev-claude](https://github.com/Panebianco00/jev-claude) - Route Claude Code.
- [pedroknigge/mcp_jev](https://github.com/pedroknigge/mcp_jev) - TypeSafe Jev (System One) paketlerini yerel olarak çalıştırmak için açık MCP…
- [punkcanyang/hermes-jev-router](https://github.com/punkcanyang/hermes-jev-router) - Hermes Agent plugin: TypeSafe Jev model routing + trim-then-compress.
- [sypherin/jev-trace-classifier](https://github.com/sypherin/jev-trace-classifier) - Application of TypeSafe Jev (noul judgment primitive) on the collusion.wiki…
- [themsquared/jev-benchmark](https://github.com/themsquared/jev-benchmark) - Reproducible benchmark for TypeSafe AI.
- [TheWayWithin/jev-bench](https://github.com/TheWayWithin/jev-bench) - Does the cited source actually say it? A 42-claim benchmark: Jev.
- [Thneoly/r2r-jev](https://github.com/Thneoly/r2r-jev) - Persistent governance for AI agents — turn Jev judgments into replayable…
- [TimMikeladze/JevLang](https://github.com/TimMikeladze/JevLang) - A policy engine for LLM decisions: declare routes, gates and actions once in…
- [ab2webco/orca-jev-advisor](https://github.com/ab2webco/orca-jev-advisor) - Jev (TypeSafe)-backed decision layer for Orca Lab: judges what agents are about…
- [Aether-254/ask-jev](https://github.com/Aether-254/ask-jev) - MCP server and Codex/Claude plugin for TypeSafe Jev typed decisions.
- [aleksvega/jev-stack](https://github.com/aleksvega/jev-stack) - One command (npx jev-stack) to install the Jev-powered agent toolset: context…
- [AnthonyDavidAdams/jev-search](https://github.com/AnthonyDavidAdams/jev-search) - Decision-model agentic search: fetch and parse deterministically, send only the…
- [anusornc/jev-decision-engine](https://github.com/anusornc/jev-decision-engine) - Ultra-fast System 1 AI structured decision engine &amp; type-safe DSL for AI…
- [apolenkov/jev-codex-router-lab](https://github.com/apolenkov/jev-codex-router-lab) - Evidence-first TypeScript lab for advisory Jev skill routing.
- [apscot/jev-auto-router-skill](https://github.com/apscot/jev-auto-router-skill) - Automatic model + effort routing for coding agents, decided by TypeSafe.
- [atharvabaodhankar/jev-vs-the-clock](https://github.com/atharvabaodhankar/jev-vs-the-clock) - High-stakes real-time AI defusal arena pitting TypeSafe Jev.
- [bismawy/pi-jev-eye](https://github.com/bismawy/pi-jev-eye) - Ultra-lean supervisor for Pi coding agent using local regex filters…
- [bobhuff0/ramey-memo-opus55-jev](https://github.com/bobhuff0/ramey-memo-opus55-jev) - Re-analysis of the 1947 Roswell Ramey memo scans with Claude Opus 5.5 and…
- [boriscardano/herdr-jev-router](https://github.com/boriscardano/herdr-jev-router) - Advisory Jev routing for child agents on stock Herdr: Jev picks the harness…
- [bpmforbusiness/jev-agent-harness](https://github.com/bpmforbusiness/jev-agent-harness) - Jev (TypeSafe AI System One) agent harness guide + video manual — from the…
- [buildaistack/jev-agent-harness](https://github.com/buildaistack/jev-agent-harness) - A Java Spring Boot agent harness powered by JEV and LLM intelligence.
- [Capitalofgeorgiapolitician1569/mobile-jev](https://github.com/Capitalofgeorgiapolitician1569/mobile-jev) - Automate real Android tasks on live phones with Jev, Mobilerun, and TypeSafe—no…
- [chalkychalk42/jev](https://github.com/chalkychalk42/jev) - A guide-directed leveling agent for a private TBC 2.4.3 server, built so it…
- [chayan-bit/jev-harness](https://github.com/chayan-bit/jev-harness) - Attach bounded, auditable Jev advisory, shadow judgments, and threshold…
- [chuloontop4-code/jev-instance-review](https://github.com/chuloontop4-code/jev-instance-review) - Best Roblox Studio Instance Reviewer 2026: Typed Jev Criteria and Exact…
- [Clementtang/jev-eval](https://github.com/Clementtang/jev-eval) - Stance test of TypeSafe Jev vs Claude on Taiwan sovereignty questions, in…
- [codaaiteam/jev-agent-tools](https://github.com/codaaiteam/jev-agent-tools) - Open-source single-file tools for AI agent builders, powered by Jev.
- [codaaiteam/jev-chat-ai](https://github.com/codaaiteam/jev-chat-ai) - Jev Chat — one AI chat where Jev decides what happens next: answer directly…
- [codaaiteam/jev-loop-detector](https://github.com/codaaiteam/jev-loop-detector) - Catch an AI agent stuck in a loop — Jev grades each step…
- [codaaiteam/jev-skill-router](https://github.com/codaaiteam/jev-skill-router) - Route each task to the one tool an agent should call, with Jev — with…
- [coderexpert123/jev-browser-wingman](https://github.com/coderexpert123/jev-browser-wingman) - Browser automation where TypeSafe.
- [CSlawyer1985/dsh-jev-router](https://github.com/CSlawyer1985/dsh-jev-router) - DSH 插件 · 用 Jev（TypeSafe System One）判定推理强度：默认只切思考强度（零缓存代价），自动模型路由出厂关闭 + 硬门禁 +…
- [danieljohnmorris/omp-jev-router](https://github.com/danieljohnmorris/omp-jev-router) - OMP extension that routes each turn to a model, and each delegated task to an…
- [dannyowelch/jev-skill-suggester](https://github.com/dannyowelch/jev-skill-suggester) - Jev skill suggestion demo: rank and gate which agent skill to load for a…
- [dansya-arsana/jev-harness](https://github.com/dansya-arsana/jev-harness) - Jev (TypeSafe) as the decision layer for Claude Code: permission gate, skill…
- [Donatasramanauskas007/open-jev](https://github.com/Donatasramanauskas007/open-jev) - Score options in one pass with Gemma 3 4B on MLX or PyTorch, no decoding, for…
- [drycool/jev-router](https://github.com/drycool/jev-router) - Ultra-low latency System-1 decision router for AI agents powered by Laya, FTS5…
- [edoproch/compact-jev](https://github.com/edoproch/compact-jev) - /compact-jev: Jev-guided verbatim compaction for Claude Code through Vercel AI…
- [ericwanderlust/jev-codex-router](https://github.com/ericwanderlust/jev-codex-router) - Auto (Jev) model routing designed for Codex GPT-6 Luna, Sol, and Astra.
- [eSaadster/jev-effort-router](https://github.com/eSaadster/jev-effort-router) - Claude Code plugin: per-prompt reasoning-effort routing with TypeSafe Jev.
- [flaviomartil/ralph-jev](https://github.com/flaviomartil/ralph-jev) - Autonomous agent loop with a TypeSafe Jev completion judge.
- [frederico-kluser/jev-agent-skill](https://github.com/frederico-kluser/jev-agent-skill) - Decisões tipadas em milissegundos com o Jev (System One da TypeSafe) via…
- [gordan-code/dsh-jev-advisor](https://github.com/gordan-code/dsh-jev-advisor) - Jev (TypeSafe System One) gives the human a second opinion, not the agent.
- [haowenyang188-arch/dsh-jev-judge](https://github.com/haowenyang188-arch/dsh-jev-judge) - TypeSafe Jev SystemOne structured-judgment plugin for DeepSeek Harness…
- [HAR5HA-7663/jev-compact](https://github.com/HAR5HA-7663/jev-compact) - Prune-not-summarise context compaction for Claude Code, guided by Jev.
- [hugues-vnsgn/jev-ios-bridge](https://github.com/hugues-vnsgn/jev-ios-bridge) - Bridge that lets Claude Code verify an iOS app on a simulator, with TypeSafe.
- [HunterXing/typesafe-ai-jev-skill](https://github.com/HunterXing/typesafe-ai-jev-skill) - Portable Agent Skill for TypeSafe System One / Jev with custom provider…
- [hypnguyen1209/jev-paseo](https://github.com/hypnguyen1209/jev-paseo) - model-agnostic typed-decision judge plugin for Paseo.
- [j-ctang/claude-code-jev-prune](https://github.com/j-ctang/claude-code-jev-prune) - Jev Prune for Claude Code: lossless context pruning, compaction and filtering.
- [jasonli0226/jev-playground](https://github.com/jasonli0226/jev-playground) - Experiments with Jev (TypeSafe classifier via OpenRouter): an incident-triage…
- [JoaoClemer/jev-lab](https://github.com/JoaoClemer/jev-lab) - O Jev (TypeSafe AI) como guardrail em tempo real e como pré-avaliador de…
- [jozso39/jev-model-router-for-claude](https://github.com/jozso39/jev-model-router-for-claude) - Claude Code per-turn model routing with TypeSafe Jev through an OpenRouter API…
- [Juniebentonitic2834/NanoJev](https://github.com/Juniebentonitic2834/NanoJev) - Build a 0.6B parallel decision model that turns states into probability…
- [kevinlupera/jev-strands-router](https://github.com/kevinlupera/jev-strands-router) - Route Strands agent queries to the right Amazon Bedrock model tier using Jev…
- [King4s/jev-loop](https://github.com/King4s/jev-loop) - Jev (TypeSafe) decides, Claude Code, Codex or Hermes builds: a decision-driven…
- [koojy/jev-plugins](https://github.com/koojy/jev-plugins) - Jev checks for development workflows, organized as plugins.
- [koosbcom/jev-delegate](https://github.com/koosbcom/jev-delegate) - Codex skill and MCP tools for Jev-backed workspace search ranking and…
- [laihenyi/pi-Jev-browser](https://github.com/laihenyi/pi-Jev-browser) - Browser and macOS desktop agent for pi: Jev (TypeSafe System One) chooses each…
- [LHLLHL-hailong/jev_enhance_harness](https://github.com/LHLLHL-hailong/jev_enhance_harness) - Pi agent harness + Jev (TypeSafe System One) 结构化决策模型接入；含 Pi 扩展点审查与门控方案.
- [liyifan2004/obsidian-jev-inbox-router](https://github.com/liyifan2004/obsidian-jev-inbox-router) - Obsidian plugin: route incoming notes to the right folder using JEV.
- [louis-szeto/open-jev-bridge](https://github.com/louis-szeto/open-jev-bridge) - MCP plugin to connect jev-like system one API (local hosted or typesafe jev) to…
- [maxkimambo/jev-mcp](https://github.com/maxkimambo/jev-mcp) - Let TypeSafe Jev read files, logs and search results so your coding agent reads…
- [michaljach/jev-relay](https://github.com/michaljach/jev-relay) - Two-model relay for Claude Code and Codex: Jev makes the small decisions, the…
- [mjvmsteixeira/jev-browser-mcp](https://github.com/mjvmsteixeira/jev-browser-mcp) - Agente de browser para o Claude Code: jev-ultrafast + Jev (TypeSafe) com…
- [nadeem4/jev-demo](https://github.com/nadeem4/jev-demo) - Decision Arena: TypeSafe.
- [olivdx/jev-mcp](https://github.com/olivdx/jev-mcp) - Jev-powered decision layer for coding agents.
- [PedroAlvarado/jev-scout](https://github.com/PedroAlvarado/jev-scout) - Agent Skill that finds where TypeSafe Jev decision models would pay off in a…
- [PhiDung-hub/jev-router](https://github.com/PhiDung-hub/jev-router) - Exact plain-language routing for agent dashboards: resolves sessions, PRs and…
- [PromptEngineer48/langchain-jev-tutorial](https://github.com/PromptEngineer48/langchain-jev-tutorial) - LangChain + Jev (TypeSafe) tutorial: a support-ops agent whose small decisions…
- [rashedInt32/jev-reach](https://github.com/rashedInt32/jev-reach) - chrome-devtools-mcp plus one tool: Jev walks the browser to the spot, your…
- [Razeefshaik/JevEval](https://github.com/Razeefshaik/JevEval) - Jev Model evaluation for multi agent routing.
- [seanmphelps-ai/jev-ux-eve](https://github.com/seanmphelps-ai/jev-ux-eve) - JEV-UX decision layer console + Eve agent harness on Vercel.
- [Shalimov04/open-jev](https://github.com/Shalimov04/open-jev) - Distil a prompt into a small, fast, calibrated classifier.
- [shoemoney/jev-the-spire2](https://github.com/shoemoney/jev-the-spire2) - An agent plays Slay the Spire 2. Single-call factored decisions via TypeSafe…
- [Simon-zj1/jev-exam](https://github.com/Simon-zj1/jev-exam) - 用 Jev（TypeSafe System One 决策模型）把上传的学习材料变成可自动判分的自助考试：逐得分点判定、置信门控、错题本与薄弱点.
- [sisodias/jev-agent-skills](https://github.com/sisodias/jev-agent-skills) - Portable Jev skills for intent, completion, routing, Camofox browser loops and…
- [slashdaemon/jev-expert](https://github.com/slashdaemon/jev-expert) - A Claude Code skill: an authoritative reference for TypeSafe AI.
- [suenot/codex-jev-router-benchmarks](https://github.com/suenot/codex-jev-router-benchmarks) - Reproducible benchmarks, reports, and traces for Codex Jev Router.
- [tmbmartell/jev-mcp](https://github.com/tmbmartell/jev-mcp) - Servidor MCP que expõe o Jev (TypeSafe System One) como ferramenta de…
- [triggeredcode/jev-compiler](https://github.com/triggeredcode/jev-compiler) - Compile decision policies into inspectable, measurable TypeSafe Jev programs.
- [twilso24/jev_router](https://github.com/twilso24/jev_router) - Standalone TypeSafe Jev-based auto-routing for Agent Zero.
- [usualoma/mt-plugin-jev](https://github.com/usualoma/mt-plugin-jev) - Natural-language search for Movable Type 9 with OpenAI embeddings and Jev.
- [vimulatus/agent-browser-plugin-jev](https://github.com/vimulatus/agent-browser-plugin-jev) - agent-browser plugin: Jev drives the browser from one goal or one policy.
- [wendyeq/dsh-jev-router](https://github.com/wendyeq/dsh-jev-router) - Optional session-pinned Jev routing plugin for DeepSeek Harness.
- [xz-dev/pi-jev-todo-audit](https://github.com/xz-dev/pi-jev-todo-audit) - Pi extension that audits rpiv-todo board drift every 10 agent loops using…
- [yangzhou-chaofan/awesome-jev-prompt](https://github.com/yangzhou-chaofan/awesome-jev-prompt) - x / github / en yeni kaynaklardan jev için en yeni 100 vitrin (güncel tutun).
- [yanmad27/ask-jev](https://github.com/yanmad27/ask-jev) - Ask Jev before asking you — a Claude Code plugin that answers AskUserQuestion…
- [yinjf2005/econpaper-architect-jev-v1.0.1](https://github.com/yinjf2005/econpaper-architect-jev-v1.0.1) - EconPaper Architect is an architect skill tailored for economic academic papers…
- [yottayoshida/jev-intent-review](https://github.com/yottayoshida/jev-intent-review) - Verify a pull request against the intent that caused it, across the whole…
- [ZeroX-01/jev-atlas](https://github.com/ZeroX-01/jev-atlas) - Continuously updated public index of real TypeSafe JEV projects, videos…
- [Zyw052/astrbot_plugin_jev_radar](https://github.com/Zyw052/astrbot_plugin_jev_radar) - Jev 意图雷达 — 用 TypeSafe SystemOne(Jev) 读懂消息背后的意图、情绪与风险.
- [rmalde/minecraft-agent](https://github.com/rmalde/minecraft-agent) - Minecraft için Astra planner ve JEV controller;
- [notque/vexjoy-agent](https://github.com/notque/vexjoy-agent) - Jev Akıllı Yönlendirmeli VexJoy AI Agent - /do, düz İngilizce istekleri doğru…
- [itsmostafa/typesafe-mcp](https://github.com/itsmostafa/typesafe-mcp) - Her şeyi hızlı ve ucuza değerlendirmek için bir mcp bağlayıcısı.
- [keeltrace/hermes-nerve](https://github.com/keeltrace/hermes-nerve) - Nerve, Hermes ajanları için denetleyici bir sinir sistemidir;
- [Mawfyy/jevflow](https://github.com/Mawfyy/jevflow) - Oluşturulabilir arka uç ilkelleri olarak olasılıksal AI kararları — türü…
- [bhavikprit/instinct-ai](https://github.com/bhavikprit/instinct-ai) - ⚡ Evrensel System-1 AI Runtime ve Dual-Brain Gateway.
- [FFatTiger/new-api-plugin-typesafe](https://github.com/FFatTiger/new-api-plugin-typesafe) - QuantumNous/new-api için TypeSafe AI System One (Jev) görev eklentisi — yerel…
- [JanOstrowka/typesafe-assist](https://github.com/JanOstrowka/typesafe-assist) - TypeSafe.
- [jon-devlapaz/tink-route](https://github.com/jon-devlapaz/tink-route) - Dynamic, confidence-aware Agent Skill routing with TypeSafe Jev and Tink.
- [alanhuangyoo/wev](https://github.com/alanhuangyoo/wev) - Local System-One decision models: typed questions in, calibrated probabilities…

</details>

<a id="routing-guardrails"></a>

## Yönlendirme, koruma kuralları ve onaylar

Üretim biçimli kullanım senaryosu — her isteği gerçekten kaldırabilecek en ucuz modele gönderin ve sonuç üzerinde deterministik bir kontrol tutun.

<details>
<summary>🛡️ <b><a href="https://github.com/NandhaKishorM/laya">NandhaKishorM/laya</a></b> · ⭐23705 · Python · 👁️ observed · 0 天 · ⭐+4825</summary>

##### 📝 Özet

Otokorelasyonsuz Sistem 1 karar motoru. Tek bir ileri geçişte, 100'den fazla dilde, her türlü metin üzerinde tür belirtilmiş seçim, puan ve evet/hayır kararları; her istek için doğru kontrol noktasını seçen bir yönlendiriciyle.

<sub>🔧 Kodda kullanıldığı tespit edildi: `research/benchmarks/feishu_zh/run.py`</sub>

##### 📌 Temel bilgiler

| Alan     | Değer                                      |
| -------- | ------------------------------------------ |
| Kategori | `Yönlendirme, koruma kuralları ve onaylar` |
| Kanıt    | `observed`                                 |
| Dil      | Python                                     |

##### 📊 Veri

| Ölçüt          | Değer             |
| -------------- | ----------------- |
| Yıldız         | **23705** (+4825) |
| Son push       | 2026-09-24        |
| İlk listelenme | 2026-09-23        |

🏷 `calibration` · `classification` · `decision-model` · `huggingface` · `jev` · `modernbert` · `multilingual` · `nlp`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/nandhakishorm--laya/e01e49f0d842b461.png" width="100%" alt="NandhaKishorM/laya screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🛡️ <b><a href="https://github.com/Dicklesworthstone/skillranker">Dicklesworthstone/skillranker</a></b> · ⭐120 · Rust · 👁️ observed · 0 天 · ⭐+6</summary>

##### 📝 Özet

TypeSafe.ai tarafından Jev ile desteklenen Rust CLI. Canlı oturum bağlamını kullanarak bir sonraki adım için ajan becerilerini sıralar. Claude Code kancalarını, yapılandırılmış JSON, çekimser kalmayı ve yerel geri bildirimi içerir. Bir TypeSafe API anahtarı gerektirir.

> 💡 Aracı becerilerini tür belirtilmiş bir kararla sıralar. Daha önce bir istem olan her türlü

##### 📌 Temel bilgiler

| Alan     | Değer                                      |
| -------- | ------------------------------------------ |
| Kategori | `Yönlendirme, koruma kuralları ve onaylar` |
| Kanıt    | `observed`                                 |
| Dil      | Rust                                       |

##### 📊 Veri

| Ölçüt          | Değer        |
| -------------- | ------------ |
| Yıldız         | **120** (+6) |
| Son push       | 2026-09-25   |
| İlk listelenme | 2026-09-18   |

🏷 `agent-skills` · `ai-agents` · `asupersync` · `claude-code` · `cli` · `developer-tools` · `frankentui` · `jev`

</details>

<details>
<summary>🛡️ <b><a href="https://github.com/choxos/jevchess">choxos/jevchess</a></b> · ⭐5 · JavaScript · 👁️ observed · 5 天</summary>

##### 📝 Özet

Jev, TypeSafe'ın System One modeli; herhangi bir OpenRouter LLM'ına, Stockfish'e ve size karşı satranç oynar. Canlı hamleler, Jev'ın hamle olasılıkları, kaydedilmiş oyunlar ve galibiyet oranlarını içeren tek sayfalık web uygulaması.

##### 📌 Temel bilgiler

| Alan     | Değer                                      |
| -------- | ------------------------------------------ |
| Kategori | `Yönlendirme, koruma kuralları ve onaylar` |
| Kanıt    | `observed`                                 |
| Dil      | JavaScript                                 |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **5**      |
| Son push       | 2026-09-20 |
| İlk listelenme | 2026-09-22 |

🏷 `ai-benchmark` · `chess` · `chess-ai` · `javascript` · `jev` · `llm` · `openrouter` · `stockfish`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/choxos--jevchess/79bfe0b56d9171a0.jpg" width="100%" alt="choxos/jevchess screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/choxos--jevchess/85b5a7320b4978a5.gif" width="100%" alt="choxos/jevchess animation"><br><sub>hareketli kayıt · <a href="https://raw.githubusercontent.com/choxos/jevchess/main/documentation/tour.mp4">Videoyu aç</a></sub></td>
</tr></table>

</details>

<details>
<summary>🛡️ <b><a href="https://github.com/alperenerol/jev-1.13-mini-benchmark">alperenerol/jev-1.13-mini-benchmark</a></b> · ⭐1 · Python · 👁️ observed · 3 天</summary>

##### 📝 Özet

Mini benchmark of TypeSafe's jev-1.13 structured decision model (OpenRouter Decisions API) on labeled support-triage: noul/choice/score, consistency, cost, lessons learned

##### 📌 Temel bilgiler

| Alan     | Değer                                      |
| -------- | ------------------------------------------ |
| Kategori | `Yönlendirme, koruma kuralları ve onaylar` |
| Kanıt    | `observed`                                 |
| Dil      | Python                                     |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **1**      |
| Son push       | 2026-09-21 |
| İlk listelenme | 2026-09-20 |

</details>

<details>
<summary>🛡️ <b><a href="https://github.com/AI-Unleashed/jev-showcase">AI-Unleashed/jev-showcase</a></b> · JavaScript · 👁️ observed · 2 天</summary>

##### 📝 Özet

Interactive showcase of TypeSafe's Jev System One model (choice / score / noul) via OpenRouter — zero-dependency Node app

##### 📌 Temel bilgiler

| Alan     | Değer                                      |
| -------- | ------------------------------------------ |
| Kategori | `Yönlendirme, koruma kuralları ve onaylar` |
| Kanıt    | `observed`                                 |
| Dil      | JavaScript                                 |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-22 |
| İlk listelenme | 2026-09-23 |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/ai-unleashed--jev-showcase/48f868d90067934c.png" width="100%" alt="AI-Unleashed/jev-showcase screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🛡️ <b><a href="https://github.com/omkarghugarkar007/system-one-model-finetuning">omkarghugarkar007/system-one-model-finetuning</a></b> · Python · 👁️ observed · 2 天</summary>

##### 📝 Özet

Fine-tune small models that return calibrated probabilities instead of text. A measured recipe for System One models (Laya, Jev): text classification, routing and scoring with confidence you can act on. Trains on a laptop, no GPU.

##### 📌 Temel bilgiler

| Alan     | Değer                                      |
| -------- | ------------------------------------------ |
| Kategori | `Yönlendirme, koruma kuralları ve onaylar` |
| Kanıt    | `observed`                                 |
| Dil      | Python                                     |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-23 |
| İlk listelenme | 2026-09-23 |

🏷 `calibration` · `classification` · `confidence-estimation` · `fine-tuning` · `huggingface` · `laya` · `llm-alternative` · `machine-learning`

</details>

<details>
<summary>🛡️ <b><a href="https://github.com/renjithwarrier94/pi-model-router">renjithwarrier94/pi-model-router</a></b> · TypeScript · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

A lightweight Pi extension which reads each user message and uses a System One model like Jev to assess its complexity and route that to an approriate LLM from the configured list

##### 📌 Temel bilgiler

| Alan     | Değer                                      |
| -------- | ------------------------------------------ |
| Kategori | `Yönlendirme, koruma kuralları ve onaylar` |
| Kanıt    | `observed`                                 |
| Dil      | TypeScript                                 |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-25 |

</details>

<details>
<summary>🛡️ <b><a href="https://github.com/RubenVP2/system-one-model-gateway">RubenVP2/system-one-model-gateway</a></b> · Rust · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

Üst kaynakta açıklama yayımlanmamış.

##### 📌 Temel bilgiler

| Alan     | Değer                                      |
| -------- | ------------------------------------------ |
| Kategori | `Yönlendirme, koruma kuralları ve onaylar` |
| Kanıt    | `observed`                                 |
| Dil      | Rust                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-25 |

</details>

<details>
<summary>🛡️ <b><a href="https://github.com/wjdjdakf17/jev-study">wjdjdakf17/jev-study</a></b> · TypeScript · 👁️ observed · 4 天</summary>

##### 📝 Özet

Jev(TypeSafe AI System One Model) 스터디 — 타입화된 결정·RLCD·confidence-gated routing을 한국어 노트와 TypeScript 목업으로 정리

##### 📌 Temel bilgiler

| Alan     | Değer                                      |
| -------- | ------------------------------------------ |
| Kategori | `Yönlendirme, koruma kuralları ve onaylar` |
| Kanıt    | `observed`                                 |
| Dil      | TypeScript                                 |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-21 |
| İlk listelenme | 2026-09-20 |

🏷 `ai` · `decision-model` · `jev` · `jevlike` · `llm` · `study-notes` · `system-one-model` · `typesafe`

</details>

<details>
<summary>🛡️ <b><a href="https://github.com/klauswg/jev-guard">klauswg/jev-guard</a></b> · ⭐36 · Java · 🔎 inferred · 2 天 · **NEW**</summary>

##### 📝 Özet

Borsa para yatırma ve çekimleri için gerçek zamanlı risk önceliklendirme ağ geçidi — Jev (TypeSafe System One) yalnızca önceliklendirmeyi ele alır; hüküm verme deterministik kodda kalır.

##### 📌 Temel bilgiler

| Alan     | Değer                                      |
| -------- | ------------------------------------------ |
| Kategori | `Yönlendirme, koruma kuralları ve onaylar` |
| Kanıt    | `inferred`                                 |
| Dil      | Java                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **36**     |
| Son push       | 2026-09-22 |
| İlk listelenme | 2026-09-25 |

</details>

<details>
<summary>🛡️ <b><a href="https://github.com/kiwi0719/jev-edge">kiwi0719/jev-edge</a></b> · ⭐35 · Lua · 🔎 inferred · 2 天 · **NEW**</summary>

##### 📝 Özet

Trafiğin kenarında türlendirilmiş yargı erişim kontrolü: nginx/OpenResty için üç katmanlı prompt enjeksiyonu ve kötüye kullanım filtresi; TypeSafe Jev tarafından desteklenir. Hata durumunda açık, önbellekli ve sıcak yeniden yüklenebilir.

##### 📌 Temel bilgiler

| Alan     | Değer                                      |
| -------- | ------------------------------------------ |
| Kategori | `Yönlendirme, koruma kuralları ve onaylar` |
| Kanıt    | `inferred`                                 |
| Dil      | Lua                                        |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **35**     |
| Son push       | 2026-09-22 |
| İlk listelenme | 2026-09-25 |

🏷 `api-gateway` · `ext-authz` · `jev` · `llm-security` · `lua` · `nginx` · `openresty` · `prompt-injection`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/kiwi0719--jev-edge/1960f8efc8d61697.webp" width="100%" alt="kiwi0719/jev-edge screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🛡️ <b><a href="https://github.com/bytelabs-oss/clash-jev">bytelabs-oss/clash-jev</a></b> · ⭐33 · Python · 🔎 inferred · 3 天 · ⭐+3</summary>

##### 📝 Özet

Eğitilmiş politikası olmayan bir Clash Royale botu: Jev (TypeSafe System One) her kararı canlı oyun durumundan verir

##### 📌 Temel bilgiler

| Alan     | Değer                                      |
| -------- | ------------------------------------------ |
| Kategori | `Yönlendirme, koruma kuralları ve onaylar` |
| Kanıt    | `inferred`                                 |
| Dil      | Python                                     |

##### 📊 Veri

| Ölçüt          | Değer       |
| -------------- | ----------- |
| Yıldız         | **33** (+3) |
| Son push       | 2026-09-21  |
| İlk listelenme | 2026-09-22  |

</details>

<details>
<summary>🛡️ <b><a href="https://github.com/rupeshpoojary9/poorjev">rupeshpoojary9/poorjev</a></b> · ⭐9 · Python · 🔎 inferred · 4 天 · ⭐+2</summary>

##### 📝 Özet

Açık kaynaklı, yerel Jev alternatifi: kanıtlanabilir biçimde kalibre edilmiş güvene sahip bir System One karar katmanı (ECE 0.170→0.071). Türü belirtilmiş kararlar, çevrimdışı çalışır, API anahtarı ve bekleme listesi gerekmez.

##### 📌 Temel bilgiler

| Alan     | Değer                                      |
| -------- | ------------------------------------------ |
| Kategori | `Yönlendirme, koruma kuralları ve onaylar` |
| Kanıt    | `inferred`                                 |
| Dil      | Python                                     |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **9** (+2) |
| Son push       | 2026-09-21 |
| İlk listelenme | 2026-09-21 |

🏷 `calibration` · `classification` · `conformal-prediction` · `jev` · `jev-alternative` · `llm` · `llm-guardrails` · `local-llm`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/rupeshpoojary9--poorjev/c5e0e3beebd5391e.png" width="100%" alt="rupeshpoojary9/poorjev screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🛡️ <b><a href="https://github.com/kenhuangus/jev-usecases">kenhuangus/jev-usecases</a></b> · ⭐8 · Python · 🔎 inferred · 3 天 · ⭐+3</summary>

##### 📝 Özet

Güven eşiği kontrollü karar mantığına sahip üretim TypeSafe Jev (System One) kullanım durumu düzenekleri

##### 📌 Temel bilgiler

| Alan     | Değer                                      |
| -------- | ------------------------------------------ |
| Kategori | `Yönlendirme, koruma kuralları ve onaylar` |
| Kanıt    | `inferred`                                 |
| Dil      | Python                                     |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **8** (+3) |
| Son push       | 2026-09-21 |
| İlk listelenme | 2026-09-20 |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/kenhuangus--jev-usecases/be919255190f6495.png" width="100%" alt="kenhuangus/jev-usecases screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🛡️ <b><a href="https://github.com/gaborishka/jev-canvas">gaborishka/jev-canvas</a></b> · ⭐7 · JavaScript · 🔎 inferred · 6 天 · ⭐+3</summary>

##### 📝 Özet

Sesiniz ve işaret parmağınızla bir tldraw tuvali üzerinde çizim yapın. Jev (TypeSafe System One), eylemi, hedefi ve yeri söylenen kelime başına yaklaşık 350 ms içinde belirler.

##### 📌 Temel bilgiler

| Alan     | Değer                                      |
| -------- | ------------------------------------------ |
| Kategori | `Yönlendirme, koruma kuralları ve onaylar` |
| Kanıt    | `inferred`                                 |
| Dil      | JavaScript                                 |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **7** (+3) |
| Son push       | 2026-09-19 |
| İlk listelenme | 2026-09-23 |

🏷 `hand-tracking` · `jev` · `mediapipe` · `openrouter` · `tldraw` · `typesafe` · `voice-control` · `web-speech-api`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/gaborishka--jev-canvas/15924c327747cb97.gif" width="100%" alt="gaborishka/jev-canvas screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/gaborishka--jev-canvas/15924c327747cb97.gif" width="100%" alt="gaborishka/jev-canvas animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

</details>

<details>
<summary>🛡️ <b><a href="https://github.com/collapseindex/dinostomp">collapseindex/dinostomp</a></b> · ⭐6 · Python · 🔎 inferred · 2 天</summary>

##### 📝 Özet

Yapay zekâ değerlendirmeleri için bir doğrulama katmanı. Yalnızca puanı değil, değerlendirme aracını da kontrol eder: verileri, puanlayıcıyı, çalıştırmaları, sayıları, iddiaları ve kendisini.

##### 📌 Temel bilgiler

| Alan     | Değer                                      |
| -------- | ------------------------------------------ |
| Kategori | `Yönlendirme, koruma kuralları ve onaylar` |
| Kanıt    | `inferred`                                 |
| Dil      | Python                                     |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **6**      |
| Son push       | 2026-09-23 |
| İlk listelenme | 2026-09-23 |

🏷 `ai-safety` · `audit` · `benchmark` · `calibration` · `dino` · `evals` · `evaluation` · `harness`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/collapseindex/dinostomp/main/data/exports/readme/20260915_120000_readme_pixel-dino_architecture_1200x850_s42.png" width="100%" alt="collapseindex/dinostomp screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/collapseindex/dinostomp/main/data/exports/readme/20260915_120000_readme_pixel-dino_1200x360_s42.gif" width="100%" alt="collapseindex/dinostomp animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

<sub>Yeniden dağıtım lisansı belirtilmediği için varlık üst depodan doğrudan bağlanmıştır.</sub>

</details>

<details>
<summary>🛡️ <b><a href="https://github.com/cgbarlow/jev-test">cgbarlow/jev-test</a></b> · ⭐1 · JavaScript · 🔎 inferred · 0 天 · **NEW**</summary>

##### 📝 Özet

Prototype: AI-assisted NZQA marking from rubric criteria alone, using TypeSafe Jev for guardrails, criterion scores and confidence-based triage.  NOT ENDORSED BY NZQA - CONCEPT ONLY

##### 📌 Temel bilgiler

| Alan     | Değer                                      |
| -------- | ------------------------------------------ |
| Kategori | `Yönlendirme, koruma kuralları ve onaylar` |
| Kanıt    | `inferred`                                 |
| Dil      | JavaScript                                 |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **1**      |
| Son push       | 2026-09-24 |
| İlk listelenme | 2026-09-25 |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://github.com/user-attachments/assets/9be48041-f703-4177-b890-013824f2aa37" width="100%" alt="cgbarlow/jev-test screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

<sub>Yeniden dağıtım lisansı belirtilmediği için varlık üst depodan doğrudan bağlanmıştır.</sub>

</details>

<details>
<summary>🛡️ <b><a href="https://github.com/mingleiw/jev-oncall">mingleiw/jev-oncall</a></b> · ⭐1 · Python · 🔎 inferred · 0 天</summary>

##### 📝 Özet

Incident triage on TypeSafe Jev — the model judges, plain code decides. Routing on probability distributions with a human-review middle band and fail-open defaults.

##### 📌 Temel bilgiler

| Alan     | Değer                                      |
| -------- | ------------------------------------------ |
| Kategori | `Yönlendirme, koruma kuralları ve onaylar` |
| Kanıt    | `inferred`                                 |
| Dil      | Python                                     |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **1**      |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-22 |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/mingleiw--jev-oncall/8c9018eb3dd0ab76.png" width="100%" alt="mingleiw/jev-oncall screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary><b>Bu kategoride daha fazla</b> <sub>· 40</sub></summary>

- [ollaya-dev/ollaya](https://github.com/ollaya-dev/ollaya) - Açık karar modellerini yerel olarak çalıştırın: Laya, decider, NLI ve…
- [vynnlee/jev-mail](https://github.com/vynnlee/jev-mail) - TypeSafe Jev System One tarafından desteklenen otonom 7/24 Zero-Inbox Gmail…
- [ajanm007/jevrag](https://github.com/ajanm007/jevrag) - A pluggable decision substrate for RAG pipelines;
- [atliq/jev-ai-use-cases](https://github.com/atliq/jev-ai-use-cases) - Hands-on LangChain examples of Jev, TypeSafe AI.
- [bitnovus/jev-spam-eval](https://github.com/bitnovus/jev-spam-eval) - Zero-shot spam filtering with TypeSafe Jev Noul questions, compared with TF-IDF…
- [dtduc-git/jev-packs](https://github.com/dtduc-git/jev-packs) - Evidence-gated registry of Jev question packs — curated questions, golden cases…
- [igrejaborabora/lus222-jev-challenge](https://github.com/igrejaborabora/lus222-jev-challenge) - SAAM · JEV Decision Demo — drone FPV, três pilotos, Vercel AI Gateway.
- [4nt0ineB/jev-from-java](https://github.com/4nt0ineB/jev-from-java) - Calling TypeSafe.
- [aawhan0/JevRev](https://github.com/aawhan0/JevRev) - Benchmarking Jev-based AI routing for cost-efficient inference.
- [advillalba/jev-system-one-demo](https://github.com/advillalba/jev-system-one-demo) - Shell command risk assessment with Jev (TypeSafe System One) via OpenRouter.
- [AkashAi7/jev-vs-llm-benchmark](https://github.com/AkashAi7/jev-vs-llm-benchmark) - Provider-agnostic Jev vs LLM benchmark for quality, latency, tokens, routing…
- [arthurcolle/jev-reasoning-lab](https://github.com/arthurcolle/jev-reasoning-lab) - Portable Jev notebooks, routing experiments, and a bounded adversarial peer.
- [atmaneayoubdev/jev-ar](https://github.com/atmaneayoubdev/jev-ar) - Arabic intent routing for RAG apps and assistants: MSA, Emirati, Saudi and…
- [Blue-And-White/Grill-Jev](https://github.com/Blue-And-White/Grill-Jev) - Use when the user wants Jev-assisted task execution or delegates task decisions…
- [cristiancolon/jev-hft](https://github.com/cristiancolon/jev-hft) - Research pipeline testing whether TypeSafe.
- [dirnbauer/typo3-webcon-jev](https://github.com/dirnbauer/typo3-webcon-jev) - Typed decisions from TypeSafe AI.
- [GMR714/jev-decision-lab](https://github.com/GMR714/jev-decision-lab) - Measured bilingual support routing benchmark with Jev decisions, calibrated…
- [gordan-code/jev-entropy-gate](https://github.com/gordan-code/jev-entropy-gate) - Decide which code-migration sites can be safely auto-rewritten, using Jev.
- [harodggg/jev-x-filter](https://github.com/harodggg/jev-x-filter) - 信息过滤器 · Jev —— Chrome MV3 扩展：用 Jev（TypeSafe System One）的类型化决策过滤 X…
- [Haslab-dev/pandu-jev](https://github.com/Haslab-dev/pandu-jev) - Pandu, is mini Jev model, inspired by Jev.
- [HuXioAn/jev-telegram-channel-router](https://github.com/HuXioAn/jev-telegram-channel-router) - Jev as a channel message router | Jev频道消息路由.
- [hyspacex/jev-router](https://github.com/hyspacex/jev-router) - Route OpenAI-style chat requests to a model and reasoning effort, using…
- [iseif/jev-model-router](https://github.com/iseif/jev-model-router) - Use Jev for structured judgments and Spring AI for model-routed answers.
- [jb-akp/jev-avatar](https://github.com/jb-akp/jev-avatar) - Jev decides, an LLM talks, an Akapulu avatar is the face.
- [kartikanand73/jev-decision-gateway](https://github.com/kartikanand73/jev-decision-gateway) - Governed decision-model gateway: TypeSafe Jev vs GPT-6 Sol vs rules on 100…
- [kellystuard/jev-gmail-classifier](https://github.com/kellystuard/jev-gmail-classifier) - Google Apps Script that classifies Gmail messages with Jev, TypeSafe.
- [Loule95450/jev-free-router](https://github.com/Loule95450/jev-free-router) - Dynamic per-turn model router on free OpenCode Zen + Go models.
- [Madheshvivekanandan/jev-vs-cosine](https://github.com/Madheshvivekanandan/jev-vs-cosine) - Do you need TypeSafe.
- [morler/pi-jev-core](https://github.com/morler/pi-jev-core) - Minimal Jev judgment core for Pi and other TypeScript apps: noul/choice/score…
- [petercr/jev-orchestrator](https://github.com/petercr/jev-orchestrator) - An mini node orchestrator that uses Jev to handle routing to different LLMs…
- [RichardoMrMu/jev-mini](https://github.com/RichardoMrMu/jev-mini) - Put Jev.
- [SENZO-NCEKANA/jev-triage-benchmark](https://github.com/SENZO-NCEKANA/jev-triage-benchmark) - Independent benchmark of TypeSafe.
- [Solizardking/jev-trader-solana](https://github.com/Solizardking/jev-trader-solana) - Dry-run multi-venue JEV trader on Solana: Jupiter + DFlow spot best-price…
- [tjkimcloud/jev-site-auditor](https://github.com/tjkimcloud/jev-site-auditor) - Score every page on your site against your own brand/AEO rules, combining free…
- [iammrduncan/typesafe-ai-benchmark](https://github.com/iammrduncan/typesafe-ai-benchmark) - Bu, tür güvenli yapısal çıktıyı taklit eden bir LLM Gateway.
- [Trystan-SA/laya-candle](https://github.com/Trystan-SA/laya-candle) - Laya kullanan JEV benzeri Rust bağlantı noktası; %100 yerel, Python yok.
- [joshhu/jevtest](https://github.com/joshhu/jevtest) - Duygu yalan makinesi: Ağızdan “iyi” deniyor, peki gerçekten içten iyi mi?
- [jagsan-cyber/reflex-gate](https://github.com/jagsan-cyber/reflex-gate) - Yerel Jev / System One uyumlu ağ geçidi — TypeSafe.
- [TheCoder30ec4/model_router_python](https://github.com/TheCoder30ec4/model_router_python) - Route every LLM call to the cheapest model that can actually do the job.
- [sumleo/RLCDAlignBench](https://github.com/sumleo/RLCDAlignBench) - RLCDAlignBench: 44 alignment-failure detection benchmarks and code for.

</details>

<a id="evaluation"></a>

## Değerlendirme, kalibrasyon ve kıyaslamalar

Kararların iyi olduğunu insan nereden bilsin? Kalibrasyon bu ekosistemde açık sorudur ve bunlar onu ölçen projelerdir.

<details>
<summary>🧪 <b><a href="https://github.com/OmniJev/awesome-jev-gallery">OmniJev/awesome-jev-gallery</a></b> · ⭐353 · JavaScript · 👁️ observed · 0 天 · ⭐+130</summary>

##### 📝 Özet

🔥🔥 System One modellerinin ve Jev'in arkasındaki makaleler, açık yeniden üretimler ve bağımsız değerlendirmeler.

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Değerlendirme, kalibrasyon ve kıyaslamalar` |
| Kanıt    | `observed`                                   |
| Dil      | JavaScript                                   |

##### 📊 Veri

| Ölçüt          | Değer          |
| -------------- | -------------- |
| Yıldız         | **353** (+130) |
| Son push       | 2026-09-25     |
| İlk listelenme | 2026-09-20     |

🏷 `awesome` · `awesome-list` · `calibrated-probabilities` · `calibration` · `decision-models` · `jev` · `llm` · `llm-evaluation`

</details>

<details>
<summary>🧪 <b><a href="https://github.com/ikermoel/open-alternative-jev">ikermoel/open-alternative-jev</a></b> · ⭐54 · Python · 👁️ observed · 0 天 · ⭐+3</summary>

##### 📝 Özet

TypeSafe's Jev için açık kaynaklı alternatif: tek ileri geçişte herhangi bir açık ağırlıklı LLM'ten yazım denetimli, kalibre edilmiş kararlar sağlayan System One tarzı bir model katmanı (HF + vLLM), dürüst kıyaslamalarla

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Değerlendirme, kalibrasyon ve kıyaslamalar` |
| Kanıt    | `observed`                                   |
| Dil      | Python                                       |

##### 📊 Veri

| Ölçüt          | Değer       |
| -------------- | ----------- |
| Yıldız         | **54** (+3) |
| Son push       | 2026-09-25  |
| İlk listelenme | 2026-09-18  |

🏷 `calibration` · `classification` · `jev` · `jev-alternative` · `llm` · `logprobs` · `open-jev` · `open-source-jev`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/ikermoel--open-alternative-jev/41dab050f73a168f.png" width="100%" alt="ikermoel/open-alternative-jev screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🧪 <b><a href="https://github.com/hev/reranker">hev/reranker</a></b> · ⭐13 · Python · 👁️ observed · 7 天 · ⭐+2</summary>

##### 📝 Özet

Jev'ı (TypeSafe'in System One modeli) kalibre edilmiş bir yeniden sıralayıcı olarak kullanın: tek çağrı, en fazla 30 belge ve belge başına bir olasılık. Apache-2.0.

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Değerlendirme, kalibrasyon ve kıyaslamalar` |
| Kanıt    | `observed`                                   |
| Dil      | Python                                       |

##### 📊 Veri

| Ölçüt          | Değer       |
| -------------- | ----------- |
| Yıldız         | **13** (+2) |
| Son push       | 2026-09-17  |
| İlk listelenme | 2026-09-18  |

</details>

<details>
<summary>🧪 <b><a href="https://github.com/akash-kamat/system-one-gemma">akash-kamat/system-one-gemma</a></b> · ⭐5 · Python · 👁️ observed · 7 天 · ⭐+1</summary>

##### 📝 Özet

Açık kaynak Jev tarzı System One karar modeli. Puanlama başlığına sahip Gemma 3 270M — tek bir ileri geçişte hızlı ve kalibre edilmiş kararlar. Metin üretimi yoktur. TypeSafe.ai'nin Jev modelinden esinlenilmiştir.

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Değerlendirme, kalibrasyon ve kıyaslamalar` |
| Kanıt    | `observed`                                   |
| Dil      | Python                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **5** (+1) |
| Son push       | 2026-09-18 |
| İlk listelenme | 2026-09-18 |

🏷 `calibration` · `decision-model` · `gemma` · `jev` · `lora` · `machine-learning` · `scoring-model` · `system-one`

</details>

<details>
<summary>🧪 <b><a href="https://github.com/brida-ai/reflexbench">brida-ai/reflexbench</a></b> · ⭐5 · Python · 👁️ observed · 1 天 · **NEW**</summary>

##### 📝 Özet

ReflexBench — open benchmark and evaluation harness for System One models and typed decision engines

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Değerlendirme, kalibrasyon ve kıyaslamalar` |
| Kanıt    | `observed`                                   |
| Dil      | Python                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **5**      |
| Son push       | 2026-09-24 |
| İlk listelenme | 2026-09-25 |

🏷 `ai-benchmark` · `ai-evaluation` · `benchmark` · `calibration` · `choice` · `decision-models` · `jev` · `laya`

</details>

<details>
<summary>🧪 <b><a href="https://github.com/bydeng01/scientific-decision-eval">bydeng01/scientific-decision-eval</a></b> · ⭐2 · Python · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

Code and data for evaluating Jev, a System One model, on scientific decisions and how its choices affect downstream results.

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Değerlendirme, kalibrasyon ve kıyaslamalar` |
| Kanıt    | `observed`                                   |
| Dil      | Python                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **2**      |
| Son push       | 2026-09-24 |
| İlk listelenme | 2026-09-25 |

🏷 `jev` · `jev-ai` · `jev-model` · `jevbench` · `system-one` · `system-one-model` · `system-one-models`

</details>

<details>
<summary>🧪 <b><a href="https://github.com/Barneyjm/circuit">Barneyjm/circuit</a></b> · ⭐1 · Python · 👁️ observed · 0 天</summary>

##### 📝 Özet

Open-weights System One models (text, images, audio) and the harness that trains and measures them: LoRA plus a pointer readout head, code-labeled data, calibration on the scoreboard.

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Değerlendirme, kalibrasyon ve kıyaslamalar` |
| Kanıt    | `observed`                                   |
| Dil      | Python                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **1**      |
| Son push       | 2026-09-24 |
| İlk listelenme | 2026-09-20 |

🏷 `calibration` · `decision-circuits` · `lora` · `multimodal` · `pytorch` · `qwen` · `system-one`

</details>

<details>
<summary>🧪 <b><a href="https://github.com/patryckalves/jev-no-enem">patryckalves/jev-no-enem</a></b> · ⭐1 · Python · 👁️ observed · 3 天</summary>

##### 📝 Özet

Reproducible benchmark evaluating TypeSafe AI's Jev (System One paradigm) on Brazil's ENEM 2025 standardized exam. Evaluates typed decision-making, domain-specific accuracy, and RLCD uncertainty calibration against open LLM baselines with an interactive GitHub Pages dashboard.

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Değerlendirme, kalibrasyon ve kıyaslamalar` |
| Kanıt    | `observed`                                   |
| Dil      | Python                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **1**      |
| Son push       | 2026-09-21 |
| İlk listelenme | 2026-09-21 |

🏷 `benchmark` · `brasil` · `enem` · `jev` · `llm` · `rlcd` · `system-one-models`

</details>

<details>
<summary>🧪 <b><a href="https://github.com/aarora79/jev-samples">aarora79/jev-samples</a></b> · 👁️ observed · 5 天</summary>

##### 📝 Özet

Runnable samples for Jev, TypeSafe AI's System One model. Send state and questions carrying their own answer options, then branch on the typed value that comes back.

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Değerlendirme, kalibrasyon ve kıyaslamalar` |
| Kanıt    | `observed`                                   |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-20 |
| İlk listelenme | 2026-09-19 |

🏷 `calibration` · `classification` · `code-samples` · `confidence-thresholds` · `decision-making` · `examples` · `jev` · `llm`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/aarora79/my-ai-assets/main/explainers/jev/poster/jev-poster-back.png" width="100%" alt="aarora79/jev-samples screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

<sub>Yeniden dağıtım lisansı belirtilmediği için varlık üst depodan doğrudan bağlanmıştır.</sub>

</details>

<details>
<summary>🧪 <b><a href="https://github.com/asp616848/better-jev-bench">asp616848/better-jev-bench</a></b> · Python · 👁️ observed · 0 天</summary>

##### 📝 Özet

A wide, multi-domain, multi-modal decision/classification benchmark corpus, built in the Jev typed-decision style — for training and evaluating System One models

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Değerlendirme, kalibrasyon ve kıyaslamalar` |
| Kanıt    | `observed`                                   |
| Dil      | Python                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-23 |

</details>

<details>
<summary>🧪 <b><a href="https://github.com/emretheus/jev-rag-benchmark">emretheus/jev-rag-benchmark</a></b> · Python · 👁️ observed · 0 天</summary>

##### 📝 Özet

Free English RAG benchmark for TypeSafe Jev 1.13 — frozen candidate pools, calibration, paired bootstrap CIs, $0 runs. Compared with OpenJev and NVIDIA cross-encoders.

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Değerlendirme, kalibrasyon ve kıyaslamalar` |
| Kanıt    | `observed`                                   |
| Dil      | Python                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-24 |
| İlk listelenme | 2026-09-21 |

🏷 `benchmark` · `calibration` · `information-retrieval` · `jev` · `llm-evaluation` · `nvidia` · `openjev` · `rag`

</details>

<details>
<summary>🧪 <b><a href="https://github.com/jburns24/jev-demo">jburns24/jev-demo</a></b> · 👁️ observed · 6 天</summary>

##### 📝 Özet

playing with jev-latest evaluation model

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Değerlendirme, kalibrasyon ve kıyaslamalar` |
| Kanıt    | `observed`                                   |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-19 |
| İlk listelenme | 2026-09-19 |

</details>

<details>
<summary>🧪 <b><a href="https://github.com/nokia-applied-research/AnyJev">nokia-applied-research/AnyJev</a></b> · ⭐537 · Python · 🔎 inferred · 0 天 · ⭐+255</summary>

##### 📝 Özet

Herhangi bir LLM'ı Jev tarzı bir karar modeline dönüştürün: türü belirlenmiş kararlar, gerçek olasılıklar, eğitim yok. (güncellenmeye devam ediyor)

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Değerlendirme, kalibrasyon ve kıyaslamalar` |
| Kanıt    | `inferred`                                   |
| Dil      | Python                                       |

##### 📊 Veri

| Ölçüt          | Değer          |
| -------------- | -------------- |
| Yıldız         | **537** (+255) |
| Son push       | 2026-09-25     |
| İlk listelenme | 2026-09-22     |

🏷 `calibration` · `decision-model` · `jev` · `jev-model` · `llm` · `system-one` · `transformers` · `vllm`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/nokia-applied-research--anyjev/aa858d6466499ce1.png" width="100%" alt="nokia-applied-research/AnyJev screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/nokia-applied-research--anyjev/93e61c3de618cc36.gif" width="100%" alt="nokia-applied-research/AnyJev animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

</details>

<details>
<summary>🧪 <b><a href="https://github.com/allebee/jevk5">allebee/jevk5</a></b> · ⭐105 · Python · 🔎 inferred · 0 天 · **NEW**</summary>

##### 📝 Özet

JevK5: TypeSafe Jev için açık ağırlıklı alternatif. Tek ileri geçişte olasılıklarla türlendirilmiş kararlar; Apache-2.0 ağırlıkları ve kodu.

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Değerlendirme, kalibrasyon ve kıyaslamalar` |
| Kanıt    | `inferred`                                   |
| Dil      | Python                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **105**    |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-25 |

🏷 `calibration` · `decision-model` · `jev` · `jev-alternative` · `jevbench` · `lora` · `qwen` · `system-one`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/allebee--jevk5/7bd683d100fd8dfc.png" width="100%" alt="allebee/jevk5 screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/allebee--jevk5/1f9cd6cb325d4bd4.gif" width="100%" alt="allebee/jevk5 animation"><br><sub>hareketli kayıt · <a href="https://raw.githubusercontent.com/allebee/jevk5/main/docs/demo/jevk5-nvidia-demo.mp4">Videoyu aç</a></sub></td>
</tr></table>

</details>

<details>
<summary>🧪 <b><a href="https://github.com/Heman10x-NGU/Verdict-open-jev">Heman10x-NGU/Verdict-open-jev</a></b> · ⭐101 · Python · 🔎 inferred · 4 天 · ⭐+33</summary>

##### 📝 Özet

Kalibre edilmiş belirsizlik (RLCD), TypeSafe AI Jev kıyaslama denetimi ve tarayıcı içi WebGPU oyun alanına sahip ModernBERT (151M) üzerinde otoregresif olmayan karar motoru

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Değerlendirme, kalibrasyon ve kıyaslamalar` |
| Kanıt    | `inferred`                                   |
| Dil      | Python                                       |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **101** (+33) |
| Son push       | 2026-09-20    |
| İlk listelenme | 2026-09-18    |

🏷 `brier-score` · `calibration` · `decision-engine` · `edge-ai` · `gliclass` · `jev` · `modernbert` · `onnx`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/Heman10x-NGU/Verdict-open-jev/main/assets/v1.4/benchmark-leaderboard-chart.png" width="100%" alt="Heman10x-NGU/Verdict-open-jev screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

<sub>Yeniden dağıtım lisansı belirtilmediği için varlık üst depodan doğrudan bağlanmıştır.</sub>

</details>

<details>
<summary>🧪 <b><a href="https://github.com/zwliJay/jev-forge">zwliJay/jev-forge</a></b> · ⭐48 · Python · 🔎 inferred · 2 天 · ⭐+25</summary>

##### 📝 Özet

Jev tarzı karar modelleri için açık bir eğitim ve çıkarım yığını. Modelleri, paylaşılan bir önekten dinamik aday dallarını puanlayacak şekilde eğitin; yüksek kardinaliteli seçim, kalibrasyon ve hızlı toplu çıkarım desteği sunar.

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Değerlendirme, kalibrasyon ve kıyaslamalar` |
| Kanıt    | `inferred`                                   |
| Dil      | Python                                       |

##### 📊 Veri

| Ölçüt          | Değer        |
| -------------- | ------------ |
| Yıldız         | **48** (+25) |
| Son push       | 2026-09-23   |
| İlk listelenme | 2026-09-23   |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/zwliJay/jev-forge/main/docs/assets/jevforge-demo.gif" width="100%" alt="zwliJay/jev-forge screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/zwliJay/jev-forge/main/docs/assets/jevforge-demo.gif" width="100%" alt="zwliJay/jev-forge animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

<sub>Yeniden dağıtım lisansı belirtilmediği için varlık üst depodan doğrudan bağlanmıştır.</sub>

</details>

<details>
<summary>🧪 <b><a href="https://github.com/weitianxin/JevAny">weitianxin/JevAny</a></b> · ⭐15 · Python · 🔎 inferred · 0 天 · ⭐+7</summary>

##### 📝 Özet

Uyarlanabilir karar sistemleri için kalibrasyon farkındalığına sahip pekiştirmeli öğrenme

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Değerlendirme, kalibrasyon ve kıyaslamalar` |
| Kanıt    | `inferred`                                   |
| Dil      | Python                                       |

##### 📊 Veri

| Ölçüt          | Değer       |
| -------------- | ----------- |
| Yıldız         | **15** (+7) |
| Son push       | 2026-09-25  |
| İlk listelenme | 2026-09-23  |

🏷 `decision-model` · `jev` · `jev-ai` · `jev-model` · `reinforcement-learning`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/weitianxin--jevany/5e0d1ca4d316392e.png" width="100%" alt="weitianxin/JevAny screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/weitianxin--jevany/711125367b6758ff.gif" width="100%" alt="weitianxin/JevAny animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

</details>

<details>
<summary>🧪 <b><a href="https://github.com/ankit-aglawe/tinyjev">ankit-aglawe/tinyjev</a></b> · ⭐11 · Python · 🔎 inferred · 2 天 · ⭐+8</summary>

##### 📝 Özet

Tek ileri geçişte Choice, Score ve Noul sorularını yanıtlayan ve kalibre edilmiş olasılıkları döndüren küçük, jev benzeri model. MLX veya PyTorch, tamamen çevrimdışı, System One uyumlu.

##### 📌 Temel bilgiler

| Alan     | Değer                                        |
| -------- | -------------------------------------------- |
| Kategori | `Değerlendirme, kalibrasyon ve kıyaslamalar` |
| Kanıt    | `inferred`                                   |
| Dil      | Python                                       |

##### 📊 Veri

| Ölçüt          | Değer       |
| -------------- | ----------- |
| Yıldız         | **11** (+8) |
| Son push       | 2026-09-22  |
| İlk listelenme | 2026-09-23  |

🏷 `decision-model` · `jev` · `jev-model` · `mlx` · `qwen3` · `small-language-models` · `system-one` · `typed-decisions`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/ankit-aglawe--tinyjev/bff87199840daa0f.gif" width="100%" alt="ankit-aglawe/tinyjev screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/ankit-aglawe--tinyjev/bff87199840daa0f.gif" width="100%" alt="ankit-aglawe/tinyjev animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

</details>

<details>
<summary><b>Bu kategoride daha fazla</b> <sub>· 63</sub></summary>

- [pozapas/awesome-system-one-models](https://github.com/pozapas/awesome-system-one-models) - Curated, evidence-graded list of System One models.
- [pozapas/system-one-models-survey](https://github.com/pozapas/system-one-models-survey) - Census, evidence ledger, benchmark harness and results for the survey of System…
- [ubermenchh/nanohunch](https://github.com/ubermenchh/nanohunch) - Minimal System One model: state + typed questions in, calibrated probabilities…
- [zhangcy122/OpenJev](https://github.com/zhangcy122/OpenJev) - OpenJev: TypeSafe Jev için açık kaynaklı alternatif.
- [buberlo/jev-trader](https://github.com/buberlo/jev-trader) - Jev (TypeSafe System One) kararları etrafında 7/24 piyasa yapıcılığı sistemi…
- [Eliot5566/JEV-Paper-Radar](https://github.com/Eliot5566/JEV-Paper-Radar) - Jev her sabah yeni arXiv makalelerini okuyup okumanız gereken birkaç tanesini…
- [cablehead/jev.nu](https://github.com/cablehead/jev.nu) - TypeSafe System One API için Nushell modülü: kalibre edilmiş olasılıklara sahip…
- [rupeshpoojary9/awesome-open-system-one](https://github.com/rupeshpoojary9/awesome-open-system-one) - Açık System One ekosisteminin derlenmiş listesi: açık modeller, bağımsız…
- [wondertwins/jev-benchmark](https://github.com/wondertwins/jev-benchmark) - TypeSafe.
- [y0usaf/jev-lm](https://github.com/y0usaf/jev-lm) - Çıktı katmanı Jev olan sözcük düzeyinde dil modeli: n-gram taslak oluşturucu…
- [us/jev-local](https://github.com/us/jev-local) - Yerel Jev uyumlu değerlendirme sunucusu: türü belirlenmiş noul/choice/score ile…
- [ARCJ137442/jev-2048](https://github.com/ARCJ137442/jev-2048) - Her hamlenin bir Jev (TypeSafe AI System One) Choice olduğu, enstrümante…
- [Yifan-Lan/awesome-jev-robustness](https://github.com/Yifan-Lan/awesome-jev-robustness) - Tests, calibration audits and failure-mode studies of Jev.
- [Eurekaleo/awesome-jev-survey](https://github.com/Eurekaleo/awesome-jev-survey) - Awesome Jev: an evidence survey of Jev and Jev-like typed decision models…
- [instax-dutta/sysone-bench](https://github.com/instax-dutta/sysone-bench) - Baytları özdeş girdiler üzerinde System One karar modellerinin (Laya ve Jev)…
- [KantaHayashiAI/jev-does-not-play-dice](https://github.com/KantaHayashiAI/jev-does-not-play-dice) - Jev.
- [dchristopoulos/jev-aita](https://github.com/dchristopoulos/jev-aita) - Benchmark of TypeSafe.
- [shivpratapsinghpanwar/edgefront_JEV](https://github.com/shivpratapsinghpanwar/edgefront_JEV) - Do you need a hosted decision model, or does a small local model match it?
- [teyhouse/jev-secret-detection](https://github.com/teyhouse/jev-secret-detection) - TypeSafe.
- [xienda/dsh-jev-verify](https://github.com/xienda/dsh-jev-verify) - Jev (TypeSafe System One) decision tools + live verification benchmark for…
- [adyoi/jev-system-one-playground](https://github.com/adyoi/jev-system-one-playground) - Windows Forms playground for Jev System One — visualize and evaluate TypeSafe…
- [dashbi1/jev-sim](https://github.com/dashbi1/jev-sim) - Jev-compatible /v1/systemone server reading typed decisions from LLM logits…
- [dnakhoa/jev-deferred-crispification](https://github.com/dnakhoa/jev-deferred-crispification) - Konum belgesi: TypeSafe AI.
- [EnesDemir143/jev-laya-benchmark](https://github.com/EnesDemir143/jev-laya-benchmark) - Local benchmark comparing TypeSafe Jev and Laya-MLX for structured issue…
- [etsabary/jev-deterministic-benchmark](https://github.com/etsabary/jev-deterministic-benchmark) - 1,000-decision behavioral benchmark of Jev across 25 deterministic reasoning…
- [jjd-lab/jev-synthetic-survey](https://github.com/jjd-lab/jev-synthetic-survey) - Jev vs GPT-4.1 as synthetic survey respondents on Twin-2K-500.
- [nanami-0713/jev-resume-screening](https://github.com/nanami-0713/jev-resume-screening) - TypeSafe Jev (System One) 简历-JD 匹配度初筛：判据模板 + 正/负/陷阱三类样本测试档案，判据 v1→v3 迭代全程可复现 /…
- [onlyoneaman/jev-eval](https://github.com/onlyoneaman/jev-eval) - TypeSafe.
- [vasanthsreeram/myNameisJevToo](https://github.com/vasanthsreeram/myNameisJevToo) - Convert any causal LM into a Jev-style typed decision model — no training, no…
- [bhushankinge/jev-laya-classification-bench](https://github.com/bhushankinge/jev-laya-classification-bench) - Typed-decision models (Jev API, Laya 421M) vs Qwen3.5-35B on 12,000 real U.S.
- [fooSynaptic/jev-any-llm](https://github.com/fooSynaptic/jev-any-llm) - Wrap any instruct LLM into Jev-mode prediction — and make those decisions SUPER…
- [Gabriel382/BioJev](https://github.com/Gabriel382/BioJev) - BioJev is a biomedical decision-model benchmark and adaptation framework built…
- [HCTDIP/jev-calib](https://github.com/HCTDIP/jev-calib) - Calibration observability for TypeSafe Jev — repeatability, drift, flip rate…
- [heyimMarc/jev-calibration-probe](https://github.com/heyimMarc/jev-calibration-probe) - Measures whether the Jev model.
- [hyeoki0831-stack/jev_disaster_tweets](https://github.com/hyeoki0831-stack/jev_disaster_tweets) - JEV를 이용한 Kaggle 재난 트윗 zero-shot 분류 실험 및 threshold calibration, batch inference…
- [JawzoD3TH/JevApi](https://github.com/JawzoD3TH/JevApi) - JevApi — .NET tooling for the TypeSafe AI Jev model, which evaluates typed…
- [jevsaidit/jev-said-it](https://github.com/jevsaidit/jev-said-it) - A model commits a probability on-chain before anyone answers.
- [lytang63/OmniMed-Jev](https://github.com/lytang63/OmniMed-Jev) - Code for.
- [Mahad-007/jev-why](https://github.com/Mahad-007/jev-why) - Causal attribution and calibration for TypeSafe Jev decisions.
- [model-collapse/jev-bench](https://github.com/model-collapse/jev-bench) - JevBench: an open, model-agnostic benchmark for typed decision models…
- [MohitSV/jev-calibration-audit](https://github.com/MohitSV/jev-calibration-audit) - Exact-target audit of the probabilities returned by calibration-trained…
- [musman550/musfira-ai-made-the-horizontal-open-source-model-for-jev-with-rlcd-and](https://github.com/musman550/musfira-ai-made-the-horizontal-open-source-model-for-jev-with-rlcd-and) - Jev için RLCD ile yatay açık kaynak modelini oluşturdu ve tüm Jev…
- [NathanHB/open-jev](https://github.com/NathanHB/open-jev) - Four minimal decision models (System One style): typed questions in, calibrated…
- [OniReimu/Edge-Computing-JEV](https://github.com/OniReimu/Edge-Computing-JEV) - Code, EdgeIntent v1 benchmark, and results for.
- [open-factoryai/myJEV](https://github.com/open-factoryai/myJEV) - Turn any state into structured, calibrated decisions.
- [opper-ai/jev-vs-kev](https://github.com/opper-ai/jev-vs-kev) - Jev vs Kev 4B on the same System One endpoint: benchmark code, a post-cutoff…
- [patelvishwa112/jev-system-one-rlcd](https://github.com/patelvishwa112/jev-system-one-rlcd) - Jev System One AI &amp; RLCD Reproduction Engine: Sub-70ms Calibrated Decisions…
- [PavelRavvich/jev-bench](https://github.com/PavelRavvich/jev-bench) - Independent benchmark of TypeSafe Jev (System One) vs cheap and frontier LLMs…
- [pratikgorji/jev-guide](https://github.com/pratikgorji/jev-guide) - An illustrated guide to TypeSafe AI.
- [RiverHe2000/jev-scout](https://github.com/RiverHe2000/jev-scout) - An evidence-linked research inbox with Jev structured decisions, local Qwen…
- [sureshbujji/jev-eval-lab](https://github.com/sureshbujji/jev-eval-lab) - Eval harness for TypeSafe Jev decision models: golden-dataset evaluation…
- [UpHash-Network/mini-jev](https://github.com/UpHash-Network/mini-jev) - Typed decisions from frozen local LLMs, with reproducible evaluation and…
- [uspraveen/Jev-Reranker](https://github.com/uspraveen/Jev-Reranker) - A System-1 model based memory retrieval reranked using caliberated decision…
- [viko-max/jev-decision-bench](https://github.com/viko-max/jev-decision-bench) - Jev vs DeepSeek closed-world decision benchmark: calibration, abstention…
- [wdlctc/jev-recommend](https://github.com/wdlctc/jev-recommend) - Jev-style typed decision models as recommenders: pointwise vs isolated-mask vs…
- [xyzzzh/GroundingJev](https://github.com/xyzzzh/GroundingJev) - Visual grounding with Qwen3.5-0.8B: predict bounding boxes in one forward pass…
- [kyotofin/tax-doc-classifier](https://github.com/kyotofin/tax-doc-classifier) - Jev kararları üzerine kurulmuş vergi belgesi sayfası sınıflandırıcısı.
- [Mapika/decider](https://github.com/Mapika/decider) - Qwen3.5.
- [fstandhartinger/jevbench](https://github.com/fstandhartinger/jevbench) - JevBench v1 - Jev sınıfı türlendirilmiş karar modelleri için bir kıyaslama…
- [mithalouni/system-one-open](https://github.com/mithalouni/system-one-open) - TypeSafe&#x27;s Jev&#x27;in açık kopyası: Gemma 4 E2B / Gemma 3 270M (Modal) üzerinde tek…
- [kotoba-lang/typed-decisions](https://github.com/kotoba-lang/typed-decisions) - ModernBERT / DeBERTa / LLaDA-MoE üzerinde, ölçülmüş gecikme, doğruluk…
- [arnabgho/rlcd-lite](https://github.com/arnabgho/rlcd-lite) - Simplified RL for Calibrated Decisions: parallel constrained JSON decoding +…
- [shamazharikh/qwen-rlcd](https://github.com/shamazharikh/qwen-rlcd) - Qwen3.5-0.8B üzerinde Jev tarzı kalibre edilmiş karar modeli (Choice/Score/Noul).

</details>

<a id="research-models"></a>

## Açık yeniden üretimler, ağırlıklar ve mimari araştırması

Açık ağırlıklar, küçük kopyalar ve mimari çalışmaları. Bunlardan birkaçı, kalibrasyon davranışının yalnızca kamuya açık materyalden yeniden üretilememesi yüzünden var.

<details>
<summary>🔬 <b><a href="https://github.com/kshetrajna12/reflex">kshetrajna12/reflex</a></b> · ⭐145 · Python · 👁️ observed · 2 天 · ⭐+19</summary>

##### 📝 Özet

Küçük ve açık bir karar modeli: durum + tür belirtilmiş sorular -> kalibre edilmiş olasılıklar. Qwen3.5 üzerinde bir Jev / System One yeniden oluşturması.

> 💡 Aynı durum-artı-tür-belirtilmiş-soru arayüzüne sahip açık bir karar modeli. Hiç çalıştırmasanız bile biçim referansı olarak okumaya değerdir.

##### 📌 Temel bilgiler

| Alan     | Değer                                                      |
| -------- | ---------------------------------------------------------- |
| Kategori | `Açık yeniden üretimler, ağırlıklar ve mimari araştırması` |
| Kanıt    | `observed`                                                 |
| Dil      | Python                                                     |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **145** (+19) |
| Son push       | 2026-09-23    |
| İlk listelenme | 2026-09-18    |

</details>

<details>
<summary>🔬 <b><a href="https://github.com/TianyuCodings/NanoJev">TianyuCodings/NanoJev</a></b> · ⭐2220 · Python · 🔎 inferred · 4 天 · ⭐+153</summary>

##### 📝 Özet

Jev'ın nano kopyası: paralel kararlar, dinamik adaylar ve uçtan uca eğitim işlem hattı.

> 💡 Paralel karar biçiminin küçük bir kopyası. Tedarikçi yığını olmadan mimariyi anlamak için kullanışlıdır ve arayüzle ilgili çeşitli iddiaların ilk kez doğrulanabilir hâle gelmesini sağlamıştır.

##### 📌 Temel bilgiler

| Alan     | Değer                                                      |
| -------- | ---------------------------------------------------------- |
| Kategori | `Açık yeniden üretimler, ağırlıklar ve mimari araştırması` |
| Kanıt    | `inferred`                                                 |
| Dil      | Python                                                     |

##### 📊 Veri

| Ölçüt          | Değer           |
| -------------- | --------------- |
| Yıldız         | **2220** (+153) |
| Son push       | 2026-09-21      |
| İlk listelenme | 2026-09-18      |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/tianyucodings--nanojev/da3528699bc28423.gif" width="100%" alt="TianyuCodings/NanoJev screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/tianyucodings--nanojev/da3528699bc28423.gif" width="100%" alt="TianyuCodings/NanoJev animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

</details>

<details>
<summary>🔬 <b><a href="https://github.com/r-ms/mini-jev">r-ms/mini-jev</a></b> · ⭐55 · Python · 🔎 inferred · 6 天 · ⭐+7</summary>

##### 📝 Özet

mini-Jev: dondurulmuş bir Qwen3-4B üzerinde Jev tarzı tür belirtilmiş karar arayüzünün nasıl göründüğü — JSON üretmek yerine seçenek harfinin logit'lerini okuyun. Önceden kaydedilmiş deney, sonuçlar, öğretim tezgâhı.

> 💡 Okunabilecek en yararlı bağımsız yeniden üretim: logit'leri okuma mekanizmasının çalıştığını gösteriyor ve ayrıca okuduğu payın kalibre edilmiş bir olasılık olmadığı konusunda açıkça uyarıyor. Bu uyarı, bu ekosistemdeki en önemli tek çekincedir.

##### 📌 Temel bilgiler

| Alan     | Değer                                                      |
| -------- | ---------------------------------------------------------- |
| Kategori | `Açık yeniden üretimler, ağırlıklar ve mimari araştırması` |
| Kanıt    | `inferred`                                                 |
| Dil      | Python                                                     |

##### 📊 Veri

| Ölçüt          | Değer       |
| -------------- | ----------- |
| Yıldız         | **55** (+7) |
| Son push       | 2026-09-18  |
| İlk listelenme | 2026-09-18  |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/r-ms--mini-jev/fe789cc568b74976.png" width="100%" alt="r-ms/mini-jev screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🔬 <b><a href="https://github.com/PsiACE/dohnuts">PsiACE/dohnuts</a></b> · ⭐27 · Python · 👁️ observed · 3 天 · ⭐+2</summary>

##### 📝 Özet

Dohnuts, doğrudan kararlar için küçük multimodal modeller oluşturur. -> System One modeli

##### 📌 Temel bilgiler

| Alan     | Değer                                                      |
| -------- | ---------------------------------------------------------- |
| Kategori | `Açık yeniden üretimler, ağırlıklar ve mimari araştırması` |
| Kanıt    | `observed`                                                 |
| Dil      | Python                                                     |

##### 📊 Veri

| Ölçüt          | Değer       |
| -------------- | ----------- |
| Yıldız         | **27** (+2) |
| Son push       | 2026-09-21  |
| İlk listelenme | 2026-09-21  |

🏷 `decision-model` · `jev` · `laya` · `llm` · `lora` · `rlcd` · `system-one` · `typesafe`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/psiace--dohnuts/146654189a75a90b.png" width="100%" alt="PsiACE/dohnuts screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🔬 <b><a href="https://github.com/emnlmn/snap">emnlmn/snap</a></b> · ⭐20 · Rust · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

Yapılandırılmamış durumdan türlendirilmiş kararlar: tek ileri geçiş, üretilen metin yok. Yerel, deterministik, Jev uyumlu. typesafe.ai ile bağlantılı değildir.

##### 📌 Temel bilgiler

| Alan     | Değer                                                      |
| -------- | ---------------------------------------------------------- |
| Kategori | `Açık yeniden üretimler, ağırlıklar ve mimari araştırması` |
| Kanıt    | `observed`                                                 |
| Dil      | Rust                                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **20**     |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-25 |

🏷 `decision-engine` · `gguf` · `llamacpp` · `llm` · `llm-inference` · `local-llm` · `rust` · `structured-outputs`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/emnlmn/snap/main/assets/logo.png" width="100%" alt="emnlmn/snap screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

<sub>Yeniden dağıtım lisansı belirtilmediği için varlık üst depodan doğrudan bağlanmıştır.</sub>

</details>

<details>
<summary>🔬 <b><a href="https://github.com/antonellof/laya-vs-dijkstra">antonellof/laya-vs-dijkstra</a></b> · ⭐1 · HTML · 👁️ observed · 1 天 · **NEW**</summary>

##### 📝 Özet

Laya MLX typed decisions vs Dijkstra on seeded weighted mazes, side-by-side replay on Apple Silicon

##### 📌 Temel bilgiler

| Alan     | Değer                                                      |
| -------- | ---------------------------------------------------------- |
| Kategori | `Açık yeniden üretimler, ağırlıklar ve mimari araştırması` |
| Kanıt    | `observed`                                                 |
| Dil      | HTML                                                       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **1**      |
| Son push       | 2026-09-24 |
| İlk listelenme | 2026-09-25 |

🏷 `ai` · `dijkstra-algorithm` · `laya` · `system-one` · `system-one-models`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/antonellof--laya-vs-dijkstra/b61bc3a8e3d368be.gif" width="100%" alt="antonellof/laya-vs-dijkstra screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/antonellof--laya-vs-dijkstra/b61bc3a8e3d368be.gif" width="100%" alt="antonellof/laya-vs-dijkstra animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

</details>

<details>
<summary>🔬 <b><a href="https://huggingface.co/autotrust/JEV">autotrust/JEV</a></b> · model · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

Üst kaynakta açıklama yayımlanmamış.

##### 📌 Temel bilgiler

| Alan     | Değer                                                      |
| -------- | ---------------------------------------------------------- |
| Kategori | `Açık yeniden üretimler, ağırlıklar ve mimari araştırması` |
| Kanıt    | `observed`                                                 |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| İlk listelenme | 2026-09-25 |

🏷 `transformers` · `safetensors` · `qwen3_5_text` · `text-generation` · `system-one` · `typed-decisions` · `decision-model` · `calibrated-probabilities`

</details>

<details>
<summary>🔬 <b><a href="https://github.com/BestNathan/system-one-code-explore">BestNathan/system-one-code-explore</a></b> · Python · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

Research runtime for System One models to explore and localize code through progressively disclosed state and action spaces.

##### 📌 Temel bilgiler

| Alan     | Değer                                                      |
| -------- | ---------------------------------------------------------- |
| Kategori | `Açık yeniden üretimler, ağırlıklar ve mimari araştırması` |
| Kanıt    | `observed`                                                 |
| Dil      | Python                                                     |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-25 |

🏷 `code-exploration` · `code-intelligence` · `code-localization` · `code-search` · `harness-engineering` · `jev` · `jev-ai` · `system-one`

</details>

<details>
<summary>🔬 <b><a href="https://github.com/BipinRajC/Jev-api-experiments">BipinRajC/Jev-api-experiments</a></b> · Python · 👁️ observed · 0 天</summary>

##### 📝 Özet

Empirical experiments and API research for TypeSafe's Jev System One model trained using RLCD

##### 📌 Temel bilgiler

| Alan     | Değer                                                      |
| -------- | ---------------------------------------------------------- |
| Kategori | `Açık yeniden üretimler, ağırlıklar ve mimari araştırması` |
| Kanıt    | `observed`                                                 |
| Dil      | Python                                                     |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-24 |
| İlk listelenme | 2026-09-22 |

🏷 `api` · `experimental` · `jev` · `rlcd` · `typesafe-ai`

</details>

<details>
<summary>🔬 <b><a href="https://github.com/kiroclawai/system-one-blueprint">kiroclawai/system-one-blueprint</a></b> · Python · 👁️ observed · 4 天</summary>

##### 📝 Özet

Jev-class System One model blueprint — built from open components (50M encoder + parallel decision heads, Needle3 distillation, RLCD path)

##### 📌 Temel bilgiler

| Alan     | Değer                                                      |
| -------- | ---------------------------------------------------------- |
| Kategori | `Açık yeniden üretimler, ağırlıklar ve mimari araştırması` |
| Kanıt    | `observed`                                                 |
| Dil      | Python                                                     |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-20 |
| İlk listelenme | 2026-09-21 |

</details>

<details>
<summary>🔬 <b><a href="https://huggingface.co/lafalce/system-one-model">lafalce/system-one-model</a></b> · model · 👁️ observed · 5 天</summary>

##### 📝 Özet

Üst kaynakta açıklama yayımlanmamış.

##### 📌 Temel bilgiler

| Alan     | Değer                                                      |
| -------- | ---------------------------------------------------------- |
| Kategori | `Açık yeniden üretimler, ağırlıklar ve mimari araştırması` |
| Kanıt    | `observed`                                                 |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| İlk listelenme | 2026-09-20 |

🏷 `transformers` · `safetensors` · `modernbert` · `lora` · `text-classification` · `decision-model` · `en` · `base_model:answerdotai/ModernBERT-base`

</details>

<details>
<summary>🔬 <b><a href="https://huggingface.co/shreyanbr/nev-lite-systemone">shreyanbr/nev-lite-systemone</a></b> · model · 👁️ observed · 2 天</summary>

##### 📝 Özet

Üst kaynakta açıklama yayımlanmamış.

##### 📌 Temel bilgiler

| Alan     | Değer                                                      |
| -------- | ---------------------------------------------------------- |
| Kategori | `Açık yeniden üretimler, ağırlıklar ve mimari araştırması` |
| Kanıt    | `observed`                                                 |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| İlk listelenme | 2026-09-22 |

🏷 `model2vec` · `text-classification` · `static-embeddings` · `no-attention` · `efficient-inference` · `decision-engine` · `en` · `dataset:DeepPavlov/clinc_oos`

</details>

<details>
<summary>🔬 <b><a href="https://github.com/TheoLeeCJ/SemIf-OpenJev">TheoLeeCJ/SemIf-OpenJev</a></b> · ⭐4268 · Python · 🔎 inferred · 1 天 · **NEW**</summary>

##### 📝 Özet

Evdeki 3090 üzerinde açık modellerden anlamsal if ifadeleri. Bağımsızdır; Jev veya TypeSafe ile bağlantılı değildir.

##### 📌 Temel bilgiler

| Alan     | Değer                                                      |
| -------- | ---------------------------------------------------------- |
| Kategori | `Açık yeniden üretimler, ağırlıklar ve mimari araştırması` |
| Kanıt    | `inferred`                                                 |
| Dil      | Python                                                     |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **4268**   |
| Son push       | 2026-09-23 |
| İlk listelenme | 2026-09-25 |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/theoleecj--semif-openjev/0504149cd9ef3cad.gif" width="100%" alt="TheoLeeCJ/SemIf-OpenJev screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/theoleecj--semif-openjev/0504149cd9ef3cad.gif" width="100%" alt="TheoLeeCJ/SemIf-OpenJev animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

</details>

<details>
<summary>🔬 <b><a href="https://github.com/featherless-ai/simple-jev">featherless-ai/simple-jev</a></b> · ⭐530 · Python · 🔎 inferred · 0 天 · ⭐+38</summary>

##### 📝 Özet

Herhangi bir açık modeli bir sınıflandırıcı/jev uç noktasına dönüştürün

##### 📌 Temel bilgiler

| Alan     | Değer                                                      |
| -------- | ---------------------------------------------------------- |
| Kategori | `Açık yeniden üretimler, ağırlıklar ve mimari araştırması` |
| Kanıt    | `inferred`                                                 |
| Dil      | Python                                                     |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **530** (+38) |
| Son push       | 2026-09-24    |
| İlk listelenme | 2026-09-19    |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/featherless-ai--simple-jev/46e66fbf4a92a3e9.png" width="100%" alt="featherless-ai/simple-jev screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🔬 <b><a href="https://github.com/ekzhang/openjev-sglang">ekzhang/openjev-sglang</a></b> · ⭐323 · Python · 🔎 inferred · 0 天 · ⭐+35</summary>

##### 📝 Özet

Açık modellere dayalı, Jev uyumlu API uç noktası (yalnızca prefill)

> 💡 Açık modellerden sunulan Jev uyumlu bir uç noktadır; böylece arayüz, barındırılan API olmadan kullanılabilir.

##### 📌 Temel bilgiler

| Alan     | Değer                                                      |
| -------- | ---------------------------------------------------------- |
| Kategori | `Açık yeniden üretimler, ağırlıklar ve mimari araştırması` |
| Kanıt    | `inferred`                                                 |
| Dil      | Python                                                     |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **323** (+35) |
| Son push       | 2026-09-25    |
| İlk listelenme | 2026-09-18    |

🏷 `jev` · `llm` · `structured-generation` · `systemone`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://i.imgur.com/wHM3jxV.gif" width="100%" alt="ekzhang/openjev-sglang screenshot"></td>
<td align="center" valign="top"><img src="https://i.imgur.com/wHM3jxV.gif" width="100%" alt="ekzhang/openjev-sglang animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

<sub>Yeniden dağıtım lisansı belirtilmediği için varlık üst depodan doğrudan bağlanmıştır.</sub>

</details>

<details>
<summary>🔬 <b><a href="https://github.com/FBddcz/embodied-jev">FBddcz/embodied-jev</a></b> · ⭐225 · Python · 🔎 inferred · 2 天 · ⭐+15</summary>

##### 📝 Özet

EmbodiedJev: MiniCPM5-2B, Jev ve uyumlu model APIs ile MuJoCo robot karar çalışma tezgâhı

##### 📌 Temel bilgiler

| Alan     | Değer                                                      |
| -------- | ---------------------------------------------------------- |
| Kategori | `Açık yeniden üretimler, ağırlıklar ve mimari araştırması` |
| Kanıt    | `inferred`                                                 |
| Dil      | Python                                                     |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **225** (+15) |
| Son push       | 2026-09-22    |
| İlk listelenme | 2026-09-20    |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/fbddcz--embodied-jev/51cd011e2d8e50f2.png" width="100%" alt="FBddcz/embodied-jev screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/fbddcz--embodied-jev/297ef7c24f9a67a8.gif" width="100%" alt="FBddcz/embodied-jev animation"><br><sub>hareketli kayıt · <a href="https://raw.githubusercontent.com/FBddcz/embodied-jev/main/docs/media/libero-drawer-comparison.mp4">Videoyu aç</a></sub></td>
</tr></table>

</details>

<details>
<summary>🔬 <b><a href="https://github.com/choxos/jev-reviewer">choxos/jev-reviewer</a></b> · ⭐34 · JavaScript · 🔎 inferred · 5 天 · ⭐+2</summary>

##### 📝 Özet

Sistematik incelemeler için makalelerden alıntılanmış veri çıkarma. Bir araştırma raporuna ve eklerine çıkarma formunuzu veya RoB 2, ROBINS-I, QUADAS-2 ya da TIDieR şablonunuzu sorun; Jev satırları gösterir, her yanıt sayfasıyla birlikte kelimesi kelimesine bir alıntıdır, siz kontrol eder ve tabloyu dışa aktarırsınız. Dosyalar tarayıcınızda kalır.

##### 📌 Temel bilgiler

| Alan     | Değer                                                      |
| -------- | ---------------------------------------------------------- |
| Kategori | `Açık yeniden üretimler, ağırlıklar ve mimari araştırması` |
| Kanıt    | `inferred`                                                 |
| Dil      | JavaScript                                                 |

##### 📊 Veri

| Ölçüt          | Değer       |
| -------------- | ----------- |
| Yıldız         | **34** (+2) |
| Son push       | 2026-09-19  |
| İlk listelenme | 2026-09-18  |

🏷 `clinical-trials` · `cochrane` · `data-extraction` · `endnote` · `evidence-based-medicine` · `evidence-synthesis` · `jev` · `literature-review`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/choxos--jev-reviewer/4a551dbca9d0c41e.jpg" width="100%" alt="choxos/jev-reviewer screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/choxos--jev-reviewer/71abea319063ce6d.gif" width="100%" alt="choxos/jev-reviewer animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

</details>

<details>
<summary>🔬 <b><a href="https://github.com/amithgc/local-jev">amithgc/local-jev</a></b> · ⭐8 · Python · 🔎 inferred · 3 天 · ⭐+3</summary>

##### 📝 Özet

TypeSafe'ın Jev API ile uyumlu, yerel ve çevrimdışı bir System One sunucusu. Küçük açık modellerle tür belirtilmiş evet/hayır, kategori ve puan sorularını yanıtlar.

##### 📌 Temel bilgiler

| Alan     | Değer                                                      |
| -------- | ---------------------------------------------------------- |
| Kategori | `Açık yeniden üretimler, ağırlıklar ve mimari araştırması` |
| Kanıt    | `inferred`                                                 |
| Dil      | Python                                                     |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **8** (+3) |
| Son push       | 2026-09-21 |
| İlk listelenme | 2026-09-23 |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/amithgc--local-jev/ba35847ffadbc4f9.png" width="100%" alt="amithgc/local-jev screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary><b>Bu kategoride daha fazla</b> <sub>· 20</sub></summary>

- [wfzyx/von](https://github.com/wfzyx/von) - Açık kaynaklı System One karar modeli.
- [hyperspaceai/jevcache](https://github.com/hyperspaceai/jevcache) - TypeSafe Jev-class modelleri için karar önbelleği — kararları memoize ederek…
- [hunkim/solar-mini4-jev](https://github.com/hunkim/solar-mini4-jev)
- [klauswg/jev-suite](https://github.com/klauswg/jev-suite) - Jev üzerinde dört karar kalitesi aracı (TypeSafe System One): Jev…
- [whyashthakker/awesome-jev-use-cases](https://github.com/whyashthakker/awesome-jev-use-cases) - Jev kullanım alanlarının harika listesi.
- [ljwwwiop/JEV-mini](https://github.com/ljwwwiop/JEV-mini) - JEV-mini, hızlı bir karar verme modelini eğitmek için hazırlanmış minimal bir…
- [ShuhanSun/jev-oas-sentinel](https://github.com/ShuhanSun/jev-oas-sentinel) - OpenAPI düzyazısında gizlenmiş, bozulmaya yol açan API davranışlarını…
- [AgenticAPP-Web/Jev-Research-Index](https://github.com/AgenticAPP-Web/Jev-Research-Index) - Jev Research Index is a bilingual catalogue of papers, software projects…
- [kw2828/OpenJev](https://github.com/kw2828/OpenJev) - Browser decision playground and reproducible experiments on memory…
- [AbdelStark/reachy-jev](https://github.com/AbdelStark/reachy-jev) - Typed Jev decision primitives for Reachy Mini applications.
- [AnilDeshpande/jev-youtube-demo](https://github.com/AnilDeshpande/jev-youtube-demo) - Jev (TypeSafe) vs GPT-4o-mini: classifying YouTube comments with a Judgement…
- [bkarak/jev-mac](https://github.com/bkarak/jev-mac) - Typed-decision engine (choice / score / noul) on Apple foundation models, after…
- [emilesilvis/jev-experiment](https://github.com/emilesilvis/jev-experiment) - Reproducible Jev and GPT-4o mini classification experiments, findings, and…
- [JevResearch/Jev-Research](https://github.com/JevResearch/Jev-Research) - Performance and architectural analysis of the new &quot;Jev&quot; model by TypeSafe AI.
- [luckberonne/mini-jev](https://github.com/luckberonne/mini-jev) - Clasificador de comandos de shell de una sola pasada.
- [simonmesmith/jev-probability-experiment](https://github.com/simonmesmith/jev-probability-experiment) - Testing TypeSafe Jev against exact probabilities: 68 scenarios, Choice and…
- [zhihz/openjev](https://github.com/zhihz/openjev) - Bağlam, sorular ve aday yanıtlardan yerel iki dilli olasılık kararları.
- [apiplant/laya-rs](https://github.com/apiplant/laya-rs) - Laya.
- [Astro-Han/decision-head-rlcd](https://github.com/Astro-Han/decision-head-rlcd) - Bir karar modelinin genellemesi nereden gelir? Qwen3.5-4B üzerinde RLCD, eğitim…
- [vicksiyi/qwen-2.5-1b-rlcd](https://github.com/vicksiyi/qwen-2.5-1b-rlcd) - Local MLX decision probability demo derived from harshatheg/Qwen-2.5-1B-RLCD on…

</details>

<a id="apps-demos"></a>

## Uygulamalar, oyunlar, robotik ve etkileşimli demolar

Oyunlar, robotlar, tarayıcılar ve panolar. Gecikme ile maliyet iddialarını okunur kılan şey demolardır.

<details>
<summary>🎮 <b><a href="https://github.com/v-modal/awesome-jev-tools">v-modal/awesome-jev-tools</a></b> · ⭐716 · 👁️ observed · 0 天 · ⭐+40</summary>

##### 📝 Özet

Jev için oluşturulmuş seçkili araç listesi — TypeSafe AI's System One typed decisions modeli.

##### 📌 Temel bilgiler

| Alan     | Değer                                                  |
| -------- | ------------------------------------------------------ |
| Kategori | `Uygulamalar, oyunlar, robotik ve etkileşimli demolar` |
| Kanıt    | `observed`                                             |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **716** (+40) |
| Son push       | 2026-09-24    |
| İlk listelenme | 2026-09-20    |

🏷 `awesome` · `awesome-list` · `awesome-lists` · `jev` · `llm` · `robotics` · `robotics-algorithms` · `robotics-control`

</details>

<details>
<summary>🎮 <b><a href="https://github.com/zadescoxp/Jev-Trades">zadescoxp/Jev-Trades</a></b> · ⭐32 · Python · 👁️ observed · 0 天 · ⭐+5</summary>

##### 📝 Özet

TypeSafe AI'ın Jev olarak adlandırılan yepyeni ilk sistem one modeline sahip alım satım botu

##### 📌 Temel bilgiler

| Alan     | Değer                                                  |
| -------- | ------------------------------------------------------ |
| Kategori | `Uygulamalar, oyunlar, robotik ve etkileşimli demolar` |
| Kanıt    | `observed`                                             |
| Dil      | Python                                                 |

##### 📊 Veri

| Ölçüt          | Değer       |
| -------------- | ----------- |
| Yıldız         | **32** (+5) |
| Son push       | 2026-09-24  |
| İlk listelenme | 2026-09-18  |

🏷 `automated-testing` · `backtesting` · `crypto-trading-bot-latest` · `trading-bot-ai-automated`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/zadescoxp--jev-trades/d74708c101b60531.png" width="100%" alt="zadescoxp/Jev-Trades screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/zadescoxp--jev-trades/2abea899d779de55.gif" width="100%" alt="zadescoxp/Jev-Trades animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

</details>

<details>
<summary>🎮 <b><a href="https://github.com/Li-Evan/awesome-jev">Li-Evan/awesome-jev</a></b> · ⭐5 · HTML · 👁️ observed · 1 天</summary>

##### 📝 Özet

İnsanların Jev, TypeSafe'ın System One modeliyle oluşturduklarının en kapsamlı galerisi: senaryolara göre düzenlenmiş 3.400'den fazla proje, demo ve yazı; her birinde özgün bağlantısı, görseli ve açıklaması bulunur.

##### 📌 Temel bilgiler

| Alan     | Değer                                                  |
| -------- | ------------------------------------------------------ |
| Kategori | `Uygulamalar, oyunlar, robotik ve etkileşimli demolar` |
| Kanıt    | `observed`                                             |
| Dil      | HTML                                                   |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **5**      |
| Son push       | 2026-09-24 |
| İlk listelenme | 2026-09-21 |

🏷 `ai` · `awesome` · `awesome-list` · `classification` · `gallery` · `jev` · `llm` · `structured-output`

</details>

<details>
<summary>🎮 <b><a href="https://github.com/markjaquith/typesafe-ai-playground">markjaquith/typesafe-ai-playground</a></b> · ⭐5 · Rust · 👁️ observed · 2 天 · ⭐+3</summary>

##### 📝 Özet

A playground for experiments around Jev, TypeSafe's System One model.

##### 📌 Temel bilgiler

| Alan     | Değer                                                  |
| -------- | ------------------------------------------------------ |
| Kategori | `Uygulamalar, oyunlar, robotik ve etkileşimli demolar` |
| Kanıt    | `observed`                                             |
| Dil      | Rust                                                   |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **5** (+3) |
| Son push       | 2026-09-22 |
| İlk listelenme | 2026-09-23 |

</details>

<details>
<summary>🎮 <b><a href="https://github.com/wustep/jev-playground">wustep/jev-playground</a></b> · ⭐2 · TypeScript · 👁️ observed · 0 天 · ⭐+1</summary>

##### 📝 Özet

Can a System One model steer music? Jev picks the plan (enums only); code renders sheet, audio and MIDI.

##### 📌 Temel bilgiler

| Alan     | Değer                                                  |
| -------- | ------------------------------------------------------ |
| Kategori | `Uygulamalar, oyunlar, robotik ve etkileşimli demolar` |
| Kanıt    | `observed`                                             |
| Dil      | TypeScript                                             |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **2** (+1) |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-23 |

</details>

<details>
<summary>🎮 <b><a href="https://github.com/Ashadeepa/typesafe-jev-model-use-cases">Ashadeepa/typesafe-jev-model-use-cases</a></b> · ⭐1 · Python · 👁️ observed · 6 天</summary>

##### 📝 Özet

TypeSafe'nin System One modeli (Jev) için çalıştırılabilir demolar — paralel Noul hükümleri ve Choice tabanlı alıntı/iddia denetleyicisi

##### 📌 Temel bilgiler

| Alan     | Değer                                                  |
| -------- | ------------------------------------------------------ |
| Kategori | `Uygulamalar, oyunlar, robotik ve etkileşimli demolar` |
| Kanıt    | `observed`                                             |
| Dil      | Python                                                 |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **1**      |
| Son push       | 2026-09-19 |
| İlk listelenme | 2026-09-19 |

</details>

<details>
<summary>🎮 <b><a href="https://github.com/Ashadeepa/typesafe-showcase">Ashadeepa/typesafe-showcase</a></b> · ⭐1 · TypeScript · 👁️ observed · 3 天</summary>

##### 📝 Özet

TypeSafe'ın System One modelini (Jev) gösteren Next.js arayüzü — paralel Noul değerlendirmeleri ve Choice tabanlı bir alıntı denetleyicisi, Vercel'e dağıtılabilir

##### 📌 Temel bilgiler

| Alan     | Değer                                                  |
| -------- | ------------------------------------------------------ |
| Kategori | `Uygulamalar, oyunlar, robotik ve etkileşimli demolar` |
| Kanıt    | `observed`                                             |
| Dil      | TypeScript                                             |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **1**      |
| Son push       | 2026-09-22 |
| İlk listelenme | 2026-09-19 |

</details>

<details>
<summary>🎮 <b><a href="https://github.com/dfinke/Jev">dfinke/Jev</a></b> · ⭐1 · PowerShell · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

PowerShell decisions with TypeSafe AI's Jev model: https://typesafe.ai/blog/introducing-system-one-models-and-jev

##### 📌 Temel bilgiler

| Alan     | Değer                                                  |
| -------- | ------------------------------------------------------ |
| Kategori | `Uygulamalar, oyunlar, robotik ve etkileşimli demolar` |
| Kanıt    | `observed`                                             |
| Dil      | PowerShell                                             |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **1**      |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-25 |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/dfinke--jev/25271c0538eb4644.png" width="100%" alt="dfinke/Jev screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🎮 <b><a href="https://github.com/karozi/awesome-jev-resources">karozi/awesome-jev-resources</a></b> · ⭐1 · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

A curated list of Jev guides, official documentation, and community projects for AI practitioners, product managers, and builders.

##### 📌 Temel bilgiler

| Alan     | Değer                                                  |
| -------- | ------------------------------------------------------ |
| Kategori | `Uygulamalar, oyunlar, robotik ve etkileşimli demolar` |
| Kanıt    | `observed`                                             |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **1**      |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-25 |

🏷 `community-project` · `jev` · `system-one-models` · `typesafe` · `typesafe-ai`

</details>

<details>
<summary>🎮 <b><a href="https://github.com/Omrigotlieb/awesome-jev">Omrigotlieb/awesome-jev</a></b> · ⭐1 · Python · 👁️ observed · 0 天</summary>

##### 📝 Özet

Verified, screenshot-rich awesome list of projects built on Jev — TypeSafe AI's System One model for typed decisions

##### 📌 Temel bilgiler

| Alan     | Değer                                                  |
| -------- | ------------------------------------------------------ |
| Kategori | `Uygulamalar, oyunlar, robotik ve etkileşimli demolar` |
| Kanıt    | `observed`                                             |
| Dil      | Python                                                 |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **1**      |
| Son push       | 2026-09-24 |
| İlk listelenme | 2026-09-21 |

</details>

<details>
<summary>🎮 <b><a href="https://github.com/Shashank-H/jev-trader">Shashank-H/jev-trader</a></b> · ⭐1 · JavaScript · 👁️ observed · 4 天</summary>

##### 📝 Özet

SystemOne modeli kullanan otomatik bir alım satım sistemi - TypesafeAI Jev

##### 📌 Temel bilgiler

| Alan     | Değer                                                  |
| -------- | ------------------------------------------------------ |
| Kategori | `Uygulamalar, oyunlar, robotik ve etkileşimli demolar` |
| Kanıt    | `observed`                                             |
| Dil      | JavaScript                                             |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **1**      |
| Son push       | 2026-09-21 |
| İlk listelenme | 2026-09-19 |

</details>

<details>
<summary>🎮 <b><a href="https://github.com/dannyowelch/jev-noul-vs-choice">dannyowelch/jev-noul-vs-choice</a></b> · TypeScript · 👁️ observed · 1 天 · **NEW**</summary>

##### 📝 Özet

Interactive Jev demo: Noul vs yes/no Choice disagree (TypeSafe jev-1.13 jaggedness)

##### 📌 Temel bilgiler

| Alan     | Değer                                                  |
| -------- | ------------------------------------------------------ |
| Kategori | `Uygulamalar, oyunlar, robotik ve etkileşimli demolar` |
| Kanıt    | `observed`                                             |
| Dil      | TypeScript                                             |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-24 |
| İlk listelenme | 2026-09-25 |

</details>

<details>
<summary>🎮 <b><a href="https://x.com/tspy/status/2100864234523685146">Xtags — X intent labeller</a></b> · @tspy · 👁️ observed · 7 天</summary>

##### 📝 Özet

Xtags: a Chrome extension that labels each post in an X timeline with what it is trying to get from you — inducement, provocation, promotion, machine-generated, persuasion, entertainment, information — drawn as a tag plus a probability right after the timestamp. A side panel reports session counts (seen, judged, correct) and cumulative token cost. The author reports near-instant responses and usable accuracy before any tuning.

➡️ **Original project (manifoldor/xtags)** — [https://github.com/manifoldor/xtags](https://github.com/manifoldor/xtags)

> 💡 Worth reading as a latency argument rather than an accuracy one: labelling a timeline only works if the decision costs less than the scroll, which is the constraint a generative model cannot meet.

##### 📌 Temel bilgiler

| Alan     | Değer                                                  |
| -------- | ------------------------------------------------------ |
| Kategori | `Uygulamalar, oyunlar, robotik ve etkileşimli demolar` |
| Kanıt    | `observed`                                             |
| Sahip    | [yishan](https://x.com/tspy) · @tspy                   |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Görüntüleme    | **9873**   |
| Beğeni         | 24         |
| Yorum          | 11         |
| Yayınlandı     | 2026-09-18 |
| İlk listelenme | 2026-09-18 |

🏷 `x`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/x--tspy--2100864234523685146/0641644f12a25a45.jpg" width="100%" alt="Xtags — X intent labeller screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/x--tspy--2100864234523685146/b80cf3173f63bdd7.gif" width="100%" alt="Xtags — X intent labeller animation"><br><sub>hareketli kayıt · <a href="https://video.twimg.com/amplify_video/2100858340331200512/vid/avc1/1242x720/ex2FF5-TerVxo9xX.mp4">Videoyu aç</a></sub></td>
</tr></table>

</details>

<details>
<summary>🎮 <b><a href="https://github.com/superagents-lab/jev-search">superagents-lab/jev-search</a></b> · ⭐454 · TypeScript · 🔎 inferred · 4 天 · ⭐+28</summary>

##### 📝 Özet

TypeSafe'ın Jev'iyle web'de arama yapın: kaynak seçimi, sorgu anlama ve alaka sıralaması. Search1API ile oluşturulmuştur.

##### 📌 Temel bilgiler

| Alan     | Değer                                                  |
| -------- | ------------------------------------------------------ |
| Kategori | `Uygulamalar, oyunlar, robotik ve etkileşimli demolar` |
| Kanıt    | `inferred`                                             |
| Dil      | TypeScript                                             |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **454** (+28) |
| Son push       | 2026-09-20    |
| İlk listelenme | 2026-09-18    |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/superagents-lab--jev-search/5089f9de953e4d67.png" width="100%" alt="superagents-lab/jev-search screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🎮 <b><a href="https://github.com/hr98w/jev-visual">hr98w/jev-visual</a></b> · ⭐282 · Python · 🔎 inferred · 3 天 · ⭐+30</summary>

##### 📝 Özet

Apple Silicon üzerinde eğitim amaçlı Jev benzeri görsel çıkarım deneyi: paylaşılan bağlam, doğrudan aday puanlama ve yerel görsel demolar.

##### 📌 Temel bilgiler

| Alan     | Değer                                                  |
| -------- | ------------------------------------------------------ |
| Kategori | `Uygulamalar, oyunlar, robotik ve etkileşimli demolar` |
| Kanıt    | `inferred`                                             |
| Dil      | Python                                                 |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **282** (+30) |
| Son push       | 2026-09-21    |
| İlk listelenme | 2026-09-18    |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/hr98w--jev-visual/10390ced72c89223.png" width="100%" alt="hr98w/jev-visual screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/hr98w--jev-visual/e05a42d934ea69ce.gif" width="100%" alt="hr98w/jev-visual animation"><br><sub>hareketli kayıt · <a href="https://github.com/user-attachments/assets/c87c09d8-30d2-4392-b981-d0b5cf1879ae">Videoyu aç</a></sub></td>
</tr></table>

</details>

<details>
<summary>🎮 <b><a href="https://github.com/jkudish/jev-browser">jkudish/jev-browser</a></b> · ⭐265 · JavaScript · 🔎 inferred · 0 天 · ⭐+20</summary>

##### 📝 Özet

Typesafe'in Jev modelini kullanarak tarayıcı kullanımı

##### 📌 Temel bilgiler

| Alan     | Değer                                                  |
| -------- | ------------------------------------------------------ |
| Kategori | `Uygulamalar, oyunlar, robotik ve etkileşimli demolar` |
| Kanıt    | `inferred`                                             |
| Dil      | JavaScript                                             |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **265** (+20) |
| Son push       | 2026-09-24    |
| İlk listelenme | 2026-09-18    |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/jkudish--jev-browser/9712e94d8402c3ec.gif" width="100%" alt="jkudish/jev-browser screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/jkudish--jev-browser/d9b7631c25d7db06.gif" width="100%" alt="jkudish/jev-browser animation"><br><sub>hareketli kayıt · <a href="https://raw.githubusercontent.com/jkudish/jev-browser/main/assets/github-demo.mp4">Videoyu aç</a></sub></td>
</tr></table>

</details>

<details>
<summary>🎮 <b><a href="https://github.com/aowang-ai/jev-trade">aowang-ai/jev-trade</a></b> · ⭐140 · TypeScript · 🔎 inferred · 3 天 · ⭐+19</summary>

##### 📝 Özet

Hyperliquid üzerinde canlı Jev trader

##### 📌 Temel bilgiler

| Alan     | Değer                                                  |
| -------- | ------------------------------------------------------ |
| Kategori | `Uygulamalar, oyunlar, robotik ve etkileşimli demolar` |
| Kanıt    | `inferred`                                             |
| Dil      | TypeScript                                             |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **140** (+19) |
| Son push       | 2026-09-21    |
| İlk listelenme | 2026-09-19    |

🏷 `hyperliquid` · `jev` · `quantitative-finance` · `typesafe`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/aowang-ai/jev-trade/main/assets/desk.png" width="100%" alt="aowang-ai/jev-trade screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

<sub>Yeniden dağıtım lisansı belirtilmediği için varlık üst depodan doğrudan bağlanmıştır.</sub>

</details>

<details>
<summary>🎮 <b><a href="https://github.com/mrnugget/jev-shell-history">mrnugget/jev-shell-history</a></b> · ⭐112 · TypeScript · 🔎 inferred · 7 天 · ⭐+5</summary>

##### 📝 Özet

Jev (TypeSafe) tarafından sıralanan Fish tarzı zsh geçmişi otomatik önerileri

##### 📌 Temel bilgiler

| Alan     | Değer                                                  |
| -------- | ------------------------------------------------------ |
| Kategori | `Uygulamalar, oyunlar, robotik ve etkileşimli demolar` |
| Kanıt    | `inferred`                                             |
| Dil      | TypeScript                                             |

##### 📊 Veri

| Ölçüt          | Değer        |
| -------------- | ------------ |
| Yıldız         | **112** (+5) |
| Son push       | 2026-09-18   |
| İlk listelenme | 2026-09-18   |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/mrnugget/jev-shell-history/main/demo/demo.gif" width="100%" alt="mrnugget/jev-shell-history screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/mrnugget/jev-shell-history/main/demo/demo.gif" width="100%" alt="mrnugget/jev-shell-history animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

<sub>Yeniden dağıtım lisansı belirtilmediği için varlık üst depodan doğrudan bağlanmıştır.</sub>

</details>

<details>
<summary><b>Bu kategoride daha fazla</b> <sub>· 36</sub></summary>

- [Expeed-Software/smart-home-assistant](https://github.com/Expeed-Software/smart-home-assistant) - Demo app to showcase the System One models functionality.
- [OuchengLiu/Jev-Game-Theory-Arena](https://github.com/OuchengLiu/Jev-Game-Theory-Arena) - Play poker, liar&#x27;s dice, prisoner&#x27;s dilemma and more against Jev, TypeSafe.
- [jarrodwatts/jev-trader](https://github.com/jarrodwatts/jev-trader) - Her Monad bloğunda bir yapay zekâ işlem kararı. Kuru MON-USDC üzerinde Jev.
- [heyjunpenn/awesome-jev](https://github.com/heyjunpenn/awesome-jev) - Jev ile oluşturulmuş 916 açık kaynaklı projenin doğrulanmış, topluluk…
- [droidrun/mobile-jev](https://github.com/droidrun/mobile-jev)
- [moritzkremb/jev-voice-browser](https://github.com/moritzkremb/jev-voice-browser) - Gerçek bir tarayıcıyı sesle kontrol edin.
- [NanmiCoder/jev-arena](https://github.com/NanmiCoder/jev-arena) - Jev modeli tanıtımı ve gerçek test sonuçları: Choice / Score / Noul…
- [kevinbadi/jev-voice](https://github.com/kevinbadi/jev-voice) - Mac.
- [bilune/jev-design](https://github.com/bilune/jev-design) - Bir model bir gösterge paneli tasarlayabilir mi? Tüm tasarım sistemi, tek…
- [emrickgarrett/OneVOneJev](https://github.com/emrickgarrett/OneVOneJev) - 1.
- [guanxuyu-sv/Visual-Jev](https://github.com/guanxuyu-sv/Visual-Jev)
- [paulsmith/computer-use-jev](https://github.com/paulsmith/computer-use-jev) - macOS computer use driven by Jev (TypeSafe System One) as the decision maker.
- [AMMIROSOH/jev-2048-selenium](https://github.com/AMMIROSOH/jev-2048-selenium) - Selenium 2048 player powered by expectimax search and TypeSafe Jev, with…
- [PistachioAIHQ/jev-synergy-screening](https://github.com/PistachioAIHQ/jev-synergy-screening) - Jev (TypeSafe System One) × ASReview SYNERGY özet tarama demosu — Choice/Noul…
- [Eliot5566/jev-arena](https://github.com/Eliot5566/jev-arena) - Write a fighter in plain English. Jev pilots it in real time.
- [RileyCarney/JevTools](https://github.com/RileyCarney/JevTools) - A lightweight collection of developer utilities and scripts designed to…
- [codaaiteam/jev-companion-arena](https://github.com/codaaiteam/jev-companion-arena) - Fight beside a Jev-controlled AI teammate that decides its own move…
- [codaaiteam/jev-pacman](https://github.com/codaaiteam/jev-pacman) - You drive Pac-Man; Jev — TypeSafe AI.
- [codaaiteam/jev-semantic-racer](https://github.com/codaaiteam/jev-semantic-racer) - Type words that fit a clue; Jev scores each and your car races ahead.
- [codaaiteam/jev-wikiracer](https://github.com/codaaiteam/jev-wikiracer) - You vs Jev: race across Wikipedia by clicking links;
- [dgr8akki/jev-voice](https://github.com/dgr8akki/jev-voice) - Control Chrome by voice: open sites, search, click links and fill in forms.
- [hazlema/jev-riffs](https://github.com/hazlema/jev-riffs) - Music pattern ripper: MIDI → interval tokens → code mines candidate motifs →…
- [itsadrianxv/jev-quant](https://github.com/itsadrianxv/jev-quant) - C++ trading system leveraging TypeSafe Jev.
- [itsatgupta/Jev](https://github.com/itsatgupta/Jev) - JevDemo to compare which llm can do the job better.
- [kevinwindisch/trex-jev-bot](https://github.com/kevinwindisch/trex-jev-bot) - A bot that plays the Chrome T-Rex Runner to 1000 points using TypeSafe AI.
- [mikespins/doom-jev](https://github.com/mikespins/doom-jev) - Jev (TypeSafe AI) plays Doom live: ViZDoom + real-time Jev decisions + retro UI.
- [prestonjarnagin/jev-playground](https://github.com/prestonjarnagin/jev-playground) - Playground for TypeSafe.
- [Rosequan/jev-robotics-radar](https://github.com/Rosequan/jev-robotics-radar) - Curated robotics-company tracker, scored on 4 lenses by TypeSafe.
- [schalkneethling/jev-3000](https://github.com/schalkneethling/jev-3000) - Demo of TypeSafe.
- [SungHoonKim-Ski/jev-trading-arena](https://github.com/SungHoonKim-Ski/jev-trading-arena) - TypeSafe Jev 판단으로 한국·미국 주식을 모의 매매하고 전략별 수익률을 겨루는 백테스트·랭킹 서비스.
- [wwwyo/jev-playground](https://github.com/wwwyo/jev-playground) - Playground for TypeSafe AI.
- [yAntPower/jev-decision-lab](https://github.com/yAntPower/jev-decision-lab) - A self-hosted, bilingual playground for Jev.
- [yasuhito/jev-computer-use](https://github.com/yasuhito/jev-computer-use) - Safe computer-use automation guided by typed Jev decisions.
- [YuraDunaev/JevAiProject](https://github.com/YuraDunaev/JevAiProject) - Jev AI Studio — десктопный клиент для Jev AI API на .NET 8 (WinForms).
- [realZachi/typesafe-adblock](https://github.com/realZachi/typesafe-adblock) - 🧹 Eğlenceli proje: Küçük bir yapay zekâ karar modeline (TypeSafe Jev) “bu DOM…
- [sorrycc/typesafe-snake](https://github.com/sorrycc/typesafe-snake) - TypeSafe.

</details>

<a id="media-discussions"></a>

## Yazılar, tartışmalar ve kardeş listeler

Lansman başlıkları, bağımsız yazılar ve bu alandaki diğer derlenmiş listeler. Bu depo tek değil ve bunu söylemek aksini varsaymaktan daha yararlı.

<details>
<summary>📰 <b><a href="https://docs.typesafe.ai/model-jaggedness/jev-1.13">9 类已知缺陷（jaggedness）</a></b> · ✅ official · 7 天</summary>

##### 📝 Özet

字面理解、**计数不可靠**、日期比较、多级间接、无关细节大 state、对抗内容、指令与 criteria 矛盾、结构性不变量、生成。官方建议"拆成原子问题在代码里组合"。

> 💡 **能力边界**——这是厂商自认的边界清单，已完整收录

##### 📌 Temel bilgiler

| Alan     | Değer                                     |
| -------- | ----------------------------------------- |
| Kategori | `Yazılar, tartışmalar ve kardeş listeler` |
| Kanıt    | `official`                                |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| İlk listelenme | 2026-09-20 |

🏷 `collection` · `官方`

</details>

<details>
<summary>📰 <b><a href="https://docs.typesafe.ai/cookbooks/consistency_choice_cookbook">`consistency_choice` cookbook 正式回答了&quot;重复调用结果会变&quot;</a></b> · ✅ official · 7 天</summary>

##### 📝 Özet

同一审核 rubric、**8 个 Choice × 每条件重复 15 次**。TypeSafe **raw agree 90.8%**（8 题有 2 题翻转，但 **conflicts=0** —— 翻转只发生在「具体标签 ↔ uncertain」之间）。**加一条 abstention 规则（用 `probabilities` 要求 top probability ≥ 0.60，否则 `uncertain` 转人工）后一致率升到 99.2%**，automatic 74.2%，**不增加模型调用**。对照：Haiku t=0 100%、Opus 4.8 reasoning 92.5%、gpt-5.4-mini t=0 99.2%。官方声明：这不使模型确定；这些数字是**重复性不是正确性**；0.60 是示例策略，生产阈值要用带标签样本与代价定。

> 💡 **\[DEFINITION-CHANGE\]** 回写了 B5、关键数字表、SKILL.md 铁律 3 与上线第 6 项、适用性边界第 6 项——**本库此前把"非确定性"记为风险，现改为官方的正解：用 abstention 管理它**

##### 📌 Temel bilgiler

| Alan     | Değer                                     |
| -------- | ----------------------------------------- |
| Kategori | `Yazılar, tartışmalar ve kardeş listeler` |
| Kanıt    | `official`                                |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| İlk listelenme | 2026-09-20 |

🏷 `collection` · `官方`

</details>

<details>
<summary>📰 <b><a href="https://docs.typesafe.ai/confidence">confidence 的官方语义确认</a></b> · ✅ official · 7 天</summary>

##### 📝 Özet

官方明说 `confidence` 是**从 `probabilities` 分布算出的统计量**，分布越集中越确定；**Noul 不带 confidence**；官方明说"你没有被锁死在我们的定义上"；阈值应"从保守值开始、在你自己数据上测"。

> 💡 与 Archer 的逆向公式一致，写入定义文件

##### 📌 Temel bilgiler

| Alan     | Değer                                     |
| -------- | ----------------------------------------- |
| Kategori | `Yazılar, tartışmalar ve kardeş listeler` |
| Kanıt    | `official`                                |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| İlk listelenme | 2026-09-20 |

🏷 `collection` · `官方`

</details>

<details>
<summary>📰 <b><a href="https://www.langchain.com/blog/building-a-harness-with-jev（2026-09-17">LangChain 官方博文解释了&quot;为什么 agent 护栏品类爆发&quot; — ，Sydney Runkle &amp; Hunter Lovell）</a></b> · ✅ official</summary>

##### 📝 Özet

*"Coding harnesses like claude, codex, cursor have shipped some kind of way to classify dangerous actions… **Up until now, this classifier step has been locked away in the closed source parts of the harness. Now that a cheap and performant classifier model exists, we can take the same pattern and adopt it to all agents!**"*；并给出 router 示例（用 Jev 按自定义标准选模型，概率保留在 agent state）与 *"Jev isn't a drop-in replacement for an LLM"*。

> 💡 **解释了本库观察到的"护栏目用品化"现象**——致因为闭源 harness 特性被开源化；已写入片段库 §2.5

##### 📌 Temel bilgiler

| Alan     | Değer                                     |
| -------- | ----------------------------------------- |
| Kategori | `Yazılar, tartışmalar ve kardeş listeler` |
| Kanıt    | `official`                                |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| İlk listelenme | 2026-09-20 |

🏷 `collection` · `官方/生态`

</details>

<details>
<summary>📰 <b><a href="https://typesafe.ai/team">TypeSafe 新增 Team 页 + 导航新增 `Enterprise` 入口（产品化/商业化信号）</a></b> · ✅ official · 6 天</summary>

##### 📝 Özet

TypeSafe 新增 Team 页 + 导航新增 `Enterprise` 入口（产品化/商业化信号）

> 💡 **\[DEFINITION-CHANGE\]**（团队事实 + 产品化信号）；`01-系统定义/jev-系统定义.md` 新增 **§6b/6c**；`references/README.md` 快速事实卡更新

##### 📌 Temel bilgiler

| Alan     | Değer                                     |
| -------- | ----------------------------------------- |
| Kategori | `Yazılar, tartışmalar ve kardeş listeler` |
| Kanıt    | `official`                                |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| İlk listelenme | 2026-09-20 |

🏷 `collection` · `官方·定义级`

</details>

<details>
<summary>📰 <b><a href="https://vercel.com/changelog/typesafe-ai-jev-now-available-on-ai-gateway">Vercel 官方 changelog 宣布 Jev 上架 AI Gateway</a></b> · ✅ official</summary>

##### 📝 Özet

Vercel 官方 changelog 宣布 Jev 上架 AI Gateway

> 💡 片段库 Vercel 行补 **AI SDK 用法**（真正的增量）；**关键数字表无需改动**（数字已有）→ **本轮定义级变更降级为"否"**

##### 📌 Temel bilgiler

| Alan     | Değer                                     |
| -------- | ----------------------------------------- |
| Kategori | `Yazılar, tartışmalar ve kardeş listeler` |
| Kanıt    | `official`                                |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| İlk listelenme | 2026-09-20 |

🏷 `collection` · `官方·定义级`

</details>

<details>
<summary>📰 <b><a href="https://docs.typesafe.ai/llms.txt">发现并抓取 18 篇官方 cookbook 实战配方（此前完全缺失）</a></b> · ✅ official · 7 天</summary>

##### 📝 Özet

`llms.txt` 全站 111 页；本地原有 30 页，diff 后发现 `cookbooks/` 下 **18 篇配方**（含可跑代码、真实数据集、实测数字）。逐篇拉取并加 `type: official-cookbook` frontmatter。

> 💡 新建 `03-能力边界/jev-官方cookbook配方.md` 索引页；**这是"从原理走到生产"的那一层**

##### 📌 Temel bilgiler

| Alan     | Değer                                     |
| -------- | ----------------------------------------- |
| Kategori | `Yazılar, tartışmalar ve kardeş listeler` |
| Kanıt    | `official`                                |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| İlk listelenme | 2026-09-20 |

🏷 `collection` · `官方`

</details>

<details>
<summary>📰 <b><a href="https://github.com/typesafe-ai/skills（⭐128">安装官方 agent skill `typesafe-ai` — ，MIT）</a></b> · ✅ official · 7 天</summary>

##### 📝 Özet

`npx -y skills@latest add typesafe-ai/skills -s typesafe-ai -g -a '*' --copy -y` → 安装到 `~/.agents/skills/typesafe-ai/`（单文件 SKILL.md，149 行 / 10 KB，覆盖 79 个 agent）。安全评估：Gen=Safe、Socket=0 alerts、Snyk=Low Risk。唯一失败项是 Eve（不支持全局安装，无影响）。

> 💡 与我们的 `jev` skill **并存互补**

##### 📌 Temel bilgiler

| Alan     | Değer                                     |
| -------- | ----------------------------------------- |
| Kategori | `Yazılar, tartışmalar ve kardeş listeler` |
| Kanıt    | `official`                                |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| İlk listelenme | 2026-09-20 |

🏷 `collection` · `官方`

</details>

<details>
<summary>📰 <b><a href="https://docs.typesafe.ai/llms.txt`">官方文档 diff 再次验证有效 — `</a></b> · ✅ official</summary>

##### 📝 Özet

本轮 diff 未发现新增页（30 篇 + 18 cookbook 已齐）；说明采集器的 diff 逻辑正确。

> 💡 保留为每轮固定步骤

##### 📌 Temel bilgiler

| Alan     | Değer                                     |
| -------- | ----------------------------------------- |
| Kategori | `Yazılar, tartışmalar ve kardeş listeler` |
| Kanıt    | `official`                                |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| İlk listelenme | 2026-09-20 |

🏷 `collection` · `官方`

</details>

<details>
<summary>📰 <b><a href="https://docs.typesafe.ai/models">官方文档索引与限流/上下文/定制政策全部核实</a></b> · ✅ official · 7 天</summary>

##### 📝 Özet

端点 `POST https://api.typesafe.ai/v1/systemone`；`jev-1.13.0`，别名 `jev-latest`/`jev-preview`；64k/请求、state+最长问题 32k；限流 250k tok/s、1200 req/min 且**动态调整**；仅文本输入；Choice≤255、Score 2–10 档；**不 fine-tune、所有账号共用同一套权重**；不用客户数据训练；英语最准。

> 💡 **架构/能力边界**——写入 `01-系统定义/jev-系统定义.md` 与 `jev-能力与边界-准确定义.md`

##### 📌 Temel bilgiler

| Alan     | Değer                                     |
| -------- | ----------------------------------------- |
| Kategori | `Yazılar, tartışmalar ve kardeş listeler` |
| Kanıt    | `official`                                |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| İlk listelenme | 2026-09-20 |

🏷 `collection` · `官方`

</details>

<details>
<summary>📰 <b><a href="https://docs.typesafe.ai/migrating-to-v1">补齐官方缺失页：迁移到 v1 API</a></b> · ✅ official · 7 天</summary>

##### 📝 Özet

preview 端点 `/preview/evaluation` → 稳定版 **`POST /v1/systemone`**；`prompts` 数组 → `questions` map；`responses` 数组 → `answers` map；字段改名 **`probability`→`noul`、`chosen`→`choice`、`expectation`→`score`**；choice 概率由数组变 map；**score 概率为 v1 新增**；认证不变。

> 💡 入档 `官方文档快照/migrating-to-v1.md`；这也给出了**第二种官方页发现渠道**（官方 skill 的文档路由表）

##### 📌 Temel bilgiler

| Alan     | Değer                                     |
| -------- | ----------------------------------------- |
| Kategori | `Yazılar, tartışmalar ve kardeş listeler` |
| Kanıt    | `official`                                |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| İlk listelenme | 2026-09-20 |

🏷 `collection` · `官方`

</details>

<details>
<summary>📰 <b><a href="https://news.ycombinator.com/item?id=49717558">Introducing System One Models and Jev</a></b> · ⭐1979 · 👁️ observed · 9 天 · ⭐+6</summary>

##### 📝 Özet

Üst kaynakta açıklama yayımlanmamış.

##### 📌 Temel bilgiler

| Alan     | Değer                                     |
| -------- | ----------------------------------------- |
| Kategori | `Yazılar, tartışmalar ve kardeş listeler` |
| Kanıt    | `observed`                                |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| İlk listelenme | 2026-09-18 |

</details>

<details>
<summary>📰 <b><a href="https://github.com/ayyazzafar/needle-jev">ayyazzafar/needle-jev</a></b> · JavaScript · 🔎 inferred · 0 天 · **NEW**</summary>

##### 📝 Özet

Semantic find-in-page using TypeSafe Jev with your own API key. Modified from Needle in Shubhamsaboo/awesome-llm-apps (Apache-2.0).

##### 📌 Temel bilgiler

| Alan     | Değer                                     |
| -------- | ----------------------------------------- |
| Kategori | `Yazılar, tartışmalar ve kardeş listeler` |
| Kanıt    | `inferred`                                |
| Dil      | JavaScript                                |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-25 |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/ayyazzafar--needle-jev/8b925374f5727b3e.png" width="100%" alt="ayyazzafar/needle-jev screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary><b>Bu kategoride daha fazla</b> <sub>· 89</sub></summary>

- [yibie/awesome-jev](https://github.com/yibie/awesome-jev) - Jev üzerine kurulmuş herkese açık projeler, entegrasyonlar ve tartışmalardan…
- [Anil-matcha/awesome-jev-by-typesafe](https://github.com/Anil-matcha/awesome-jev-by-typesafe) - TypeSafe Jev için kanıta dayalı kullanım alanları, kalıplar, istemler ve…
- [cobanov/awesome-jev](https://github.com/cobanov/awesome-jev) - Jev ile oluşturulmuş, TypeSafe AI.
- [hellogumbo/awesome-jev](https://github.com/hellogumbo/awesome-jev) - Jev üzerinde oluşturulmuş projelerden oluşan, TypeSafe AI.
- [Open-sourced jev architecture last year with model,paper and dataset](https://news.ycombinator.com/item?id=49736660) - Herkes artık &amp;#x27;otomatik regresif olmayan ve JSON şemasıyla ışık hızında…
- [Show HN: CUA-S1 – A System One Model for Computer Use](https://news.ycombinator.com/item?id=49767564) - Merhaba HN! Cua&#x27;dan Dillon ve Francesco&#x27;yuz.
- [What Is RLCD? The Secret Behind Jev](https://news.ycombinator.com/item?id=49829625)
- [Jev Based Code Review](https://news.ycombinator.com/item?id=49840300)
- [aliaihub/awesome-jev-usecases](https://github.com/aliaihub/awesome-jev-usecases) - Jev ile, TypeSafe AI.
- [Jev and System One Models: Calibration Beats Accuracy](https://news.ycombinator.com/item?id=49839510)
- [Inventor of ChatGPT and RLHF Launches Typesafe.ai](https://news.ycombinator.com/item?id=49763045)
- [Show HN: Agent Chaperone – Screen AI agent tool calls and results with Jev](https://news.ycombinator.com/item?id=49789538) - Herkese merhaba. TypeSafe geçen hafta Jev yayınladı ve bu beni…
- [Sub-15ms, non-autoregressive, local drop-in alternative to TypeSafe Jev](https://news.ycombinator.com/item?id=49781612)
- [I Couldn't Build Jev at OpenAI – Diogo Almeida, TypeSafe Co-Founder and CEO \[video\]](https://news.ycombinator.com/item?id=49817923)
- [Jev: System One Models for Prod, Not God – With Diogo Almeida, CEO, TypeSafe AI](https://news.ycombinator.com/item?id=49794590)
- [jtnkminimal/awesome-jev](https://github.com/jtnkminimal/awesome-jev) - Jev, TypeSafe.
- [TypeSafe AI Jev vs. GPT-6 Astra](https://news.ycombinator.com/item?id=49798734)
- [Cua S1- A family of System One models](https://news.ycombinator.com/item?id=49758885)
- [I Built a System One Model on a RTX 3070](https://news.ycombinator.com/item?id=49770066)
- [Jev 1.13 Jaggedness](https://news.ycombinator.com/item?id=49784926)
- [Jev is now available to everyone. No waitlist.](https://news.ycombinator.com/item?id=49780343)
- [Show HN: Jevopt: Making intelligent compiler optimisation decisions with Jev](https://news.ycombinator.com/item?id=49795171) - Can Jev optimise the size of compiled binaries better than clang -Oz? Turns out…
- [Show HN: Knowledge Signal – A JEV-powered rubric assessment tool for study notes](https://news.ycombinator.com/item?id=49840561) - I built Knowledge Signal, an open-source prototype that turns study notes into…
- [The Jagged Frontier of Jev 1.13](https://news.ycombinator.com/item?id=49809509)
- [Two techniques for working with System One models](https://news.ycombinator.com/item?id=49755005)
- [TypeSafe / Jev latency-focused demos built by Devin](https://news.ycombinator.com/item?id=49757995)
- [TypeSafe AI's Jev Is Not an LLM – and That May Be the Point](https://news.ycombinator.com/item?id=49761730)
- [TypeSafe's Jev Can't See. I Made It Guess What I Drew Anyway](https://news.ycombinator.com/item?id=49768633)
- [Using system-one models inside high-throughput data pipelines](https://news.ycombinator.com/item?id=49771931)
- [What is a System One model and why we need it?](https://news.ycombinator.com/item?id=49760138)
- [A deep dive into Jev, TypeSafe's System One model](https://news.ycombinator.com/item?id=49774157)
- [A MySQL plugin that filters rows by meaning (built on TypeSafe Jev)](https://news.ycombinator.com/item?id=49774592)
- [Calibrating Jev as a Code Reviewer](https://news.ycombinator.com/item?id=49803758)
- [Contrastive Language Model (CLM): An Ultra-Fast System One Model](https://news.ycombinator.com/item?id=49827145)
- [How to classify, route, and score with Jev and AI SDK](https://news.ycombinator.com/item?id=49765748)
- [Jev as a Primitive Feature of Ruby](https://news.ycombinator.com/item?id=49757734)
- [Jevify skill – Gets your existing agents running on Jev](https://news.ycombinator.com/item?id=49812519) - Tried Jev when it came out last week and it&amp;#x27;s crazy fast.
- [Perch: Semantic Code Linting with Jev](https://news.ycombinator.com/item?id=49840204)
- [Replacing an agentic classification loop with Jev: 7x faster](https://news.ycombinator.com/item?id=49783806)
- [rhc98/awesome-jev](https://github.com/rhc98/awesome-jev) - Projects built on Jev.
- [Show HN: I Built a GIF Decider with Jev](https://news.ycombinator.com/item?id=49817371) - Hey everyone! I wanted to try out a fun use case for Jev and had this idea…
- [Show HN: Jev, Fly Me to the Moon](https://news.ycombinator.com/item?id=49769916) - I built Odyssey, a browser based journey from Earth to lunar orbit.
- [Show HN: Jev-CLI – CLI wrapper for JEV typesafe AI model](https://news.ycombinator.com/item?id=49786725)
- [Show HN: Jevper – an LLM API client constrained to the Jev wire format](https://news.ycombinator.com/item?id=49840721)
- [Show HN: Sokit – a LangChain like harness for Jev (or other System 1 models)](https://news.ycombinator.com/item?id=49744527) - Açık konuşmak gerekirse, AI ile kodlandı; aksini iddia etmiyorum.
- [Show HN: Typesafe Java SDK (Unofficial)](https://news.ycombinator.com/item?id=49762324)
- [The first (public) System One Model; Jev gives AI the properties of code](https://news.ycombinator.com/item?id=49729945)
- [Typesafe.ai Jev Open Source Alternative Qwen-2.5-1B-RLCD](https://news.ycombinator.com/item?id=49734345)
- [What you can build with TypeSafe AI Jev](https://news.ycombinator.com/item?id=49797360)
- [ozers/jevsome-projects](https://github.com/ozers/jevsome-projects) - Open-source projects that provably call Jev, TypeSafe AI.
- [108 条声明对照：Jev 的校准并不独特（含作者自我修正）](https://truestandard.ai/blog/jev-accuracy-tested)
- [\[cellcog.ai 追踪页\]() 自称分层记录 *"claims / concedes / measured / found, dated"* → 与本库锚页设计意图一致，值得长期对照](https://cellcog.ai/blog/jev-typesafe-decision-model)
- [\`calibre\`：独立标定实验证明"阈值参数不跨数据集迁移"](https://github.com/FirasSX914/calibre) - 在两个带标签数据集（**Banking77** 与 **Web of Science**）上独立测量 Jev 校准，**协议在出结果前冻结**，原始…
- [\`classifier.dev\`：建在 Jev 上的免费零样本分类服务（含 agent skill）](https://classifier.dev) - **无 API key、无账号**的零样本分类 HTTP 服务；提供 `openapi.json`(OpenAPI 3.1) 与…
- [\`clownware/bouncer\` 提供目前最硬的第三方独立标定数据](https://github.com/clownware/bouncer) - 6 个问题的准确率/Brier/分桶可靠度：Brier **0.036–0.149**；高置信桶（0.9–1.0）多为 100%.
- [\`Jev Judge vs Dimension Scores\`：分解不总是更好（含日语多语证据）](https://agentjournal.dev/blog/llm-judge-vs-feature-extraction/) - 三个分类任务上对比&quot;每题一个直接 Jev 问题&quot; vs &quot;12–14 个打分维度 + 本地拟合权重&quot;；**5,477 条测试行、3,410 万输入…
- [\`jevcal\`：最小样本经验值 \< 约 100 条不可信；并多做两件我们没做的事](https://github.com/abhixhek/jevcal) - 在自有带标签数据上拟合每问题置信度阈值 → 留出集验证 → **估算回退流量** → **在 CI 里复查已锁定阈值**.
- [\`kaijia323/dsh-plugin-jev\`：DeepSeek Harness 的 Jev 插件（与本库同环境）](https://github.com/kaijia323/dsh-plugin-jev) - 把 Jev 接进 DSH，**注册原生工具 `jev_decide`**.
- [New：\[InfoWorld（2026-09-17）\]() —— 企业媒体视角，用例清单：*"which tool to invoke, which action to take next, whether a request should be approved, when a task…](https://www.infoworld.com/article/4223468/typesafe-ais-new-models-work-with-machines-not-humans.html)
- [OOD 校准审计："Jev 知道自己不知道吗？"](https://github.com/scienthoon/jev-ood-calibration)
- [X 上一条新描述口径](https://x.com/testingcatalog/status/2099968075861008781) - 把 Jev 描述为 *&quot;a small and customizable System One decision model&quot;*，并明确 *.
- [X： \[@omarsar0\]()（Elvis Saravia, DAIR.AI）独立表述了本库的「铁律 1」](https://x.com/omarsar0/status/2099933100440494105) - *.
- [X：\[@heyDhavall\]() 的简洁概括（*"why make software wait for an LLM to generate paragraphs when it really just needs decisions?"*）](https://x.com/heyDhavall/status/2099926342095798783)
- [X：拿到本库最具体的单次成本数字 — \[@moritzkremb\]()](https://x.com/moritzkremb) - *.
- [先例清单：\[ibl.ai\]() 指出 CriticGPT 与 Prometheus 2 都在 2024 年就做"判断模型" → 写进优先权争议页 §4d，与 Laya 的指称分开记；本库判断 Jev 与它们的真实差异是"把判断做成带类型与概率的一等 API"](https://ibl.ai/blog/jev-judge-model-evaluation-not-generation-enterprise)
- [创始人第二段"原始初衷"（比第一段更锋利）：Diogo Almeida 经 \[Pat McGuinness Substack\]() 转引](https://patmcguinness.substack.com/p/jev-makes-fast-and-cheap-decisions)
- [又一份"本地 vs 托管"选型实录：\[astgl.com\]()（James Cruce，09-22）—— 421M 开源模型在其 frozen replay 上胜过自制路由；他最终不选 Jev 是因为约束（数据不出本机、可控回退），并主动限定*"whether Laya beats Jev…](https://astgl.com/p/local-laya-vs-hosted-jev-typed-decisions)
- [发现与本库做同一件事的 agent skill：\`decision-first\`](https://github.com/harrymunro/decision-first) - *.
- [发现社区维护的完整索引 \`AbdelStark/awesome-typesafe\`（ 103）](https://github.com/AbdelStark/awesome-typesafe) - 把我们此前零散收集的生态补成一张完整的图，结构为「官方资源 / 客户端与集成 / agent 与开发者工具 / 浏览器 agent / 游戏机器人与交互…
- [发现迄今最成熟的开源 Jev 实现：\`genai-craft/openvons\`（原 open-Jev， 7）](https://github.com/genai-craft/openvons) - **共享层 `openvons.core`** 含 Noul/Choice/Score 表示、决策策略…
- [四条精准告诫（已写入 IO 契约 §S8） —— \[Wavect 独立评测\]()：](https://wavect.io/blog/jev-ai-decision-model-review)
- [新用例：上下文压缩（LiteLLM 的 TypeSafe 护栏）](https://github.com/BerriAI/litellm) - `litellm/types/proxy/guardrails/guardrail_hooks/typesafe.py` 实现了 **compaction…
- [独立第三方与我们的术语纪律收敛：\[HarnessRouter（2026-09-20）\]()](https://harnessrouter.ai/blog/what-is-a-system-one-model)
- [生态跨过一条线：出现 Jev-HTTP-API 兼容的开源服务端](https://github.com/ekzhang/openjev-sglang（⭐**123**）) - *.
- [第 11 份独立评测：克罗地亚语零样本 vs 自己微调的 BERT（\[Senko Rašić\]()，09-22）](https://blog.senko.net/analyzing-jev-a-new-ai-model)
- [第 12 份独立评测：注入鲁棒性系统测试《Can Jev be jailbroken?》（\[tanh.xyz\]()，14,495 次记录调用，原始记录公开）](https://tanh.xyz/research-notes/jev-prompt-manipulation)
- [第 9 份独立评测：\[@juanmacias《Testing TypeSafe's Jev - Legal Corpus》\]()](https://x.com/i/article/2100463318209048850)
- [第三方深度解读补充大量可操作实测](https://flaviocopes.com/jev) - ① **Noul 的 0.5 不等于&quot;中等&quot;**，意味模型分不出来；② **Score 不是测量**，不要在档位间插值.
- [第三方独立实测：Classmethod / DevelopersIO](https://dev.classmethod.jp/en/articles/jev-for-llm-model-routing/) - 40 次调用全成功、10/10 分类正确；median **0.643–0.674…
- [第三方独立实测：Vercel 生产集成（最重要产业证据）](https://x.com/rauchg/status/2100307962262872105) - fx 默认 auto 模式的 safety reviewer 原跑 GPT Luna，换 Jev 后 **p95 快 18× 且更准**；.
- [财经媒体的独立表态：\[TechStock²《…Its 445× Cost Claim Is Still Self-Tested》\]()（2026-09-17）：*"…leaves the biggest performance numbers dependent on TypeSafe's…](https://ts2.tech/en/typesafe-ai-raises-40-million-for-jev-but-its-445x-cost-claim-is-still-self-tested/)
- [闭环信号：Archer 那篇文章上了 HN —— \`Jev's Architecture Unmasked\`（\[item 49766351\]()，1pt）](https://news.ycombinator.com/item?id=49766351)
- [首个上市公司级集成：\[Aurora Mobile（NASDAQ: JG）的 GPTBots.ai 集成 Jev\]()（09-22）→ *"two-layer AI architecture: one layer that thinks, one layer that…](https://markets.businessinsider.com/news/stocks/aurora-mobile-s-gptbots-ai-integrates-jev-two-layers-of-ai-one-enterprise-platform-1036563338)
- [AnotiaWang/awesome-jev](https://github.com/AnotiaWang/awesome-jev) - Harika Jev / TypeSafe System One uygulamaları, kitaplıkları ve kaynaklarından…
- [AppitStudio/awesome-jev](https://github.com/AppitStudio/awesome-jev) - Türlendirilmiş AI kararları için derlenmiş Jev kaynakları ve çalıştırılabilir…
- [samTime101/Jev-emoji-reactor](https://github.com/samTime101/Jev-emoji-reactor) - integrated latest decision making model jev to classify emoji reading the…
- [syedabbasshaheer-art/jev-atlas](https://github.com/syedabbasshaheer-art/jev-atlas) - A category-anchored catalogue of 1,442 shipped projects — what each one is for…
- [新的优先权主张（与 Laya 无关）：HN \`I built the Jev architecture one year ago and open-sourced it\`（2pts）→ \[r/LocalLLaMA 帖\]()；本轮只登记 URL 与主张，未读原文、不下判断，下一轮核实](https://www.reddit.com/r/LocalLLaMA/comments/1wijo3e/i_literally_built_the_jev_architecture_one_year/)
- [X 社区：发布日官方账号被钓鱼](https://commondefense.ai/blog/typesafe-x-account-phished) - @typesafeai 在发布日 11:17am 发布后，5:43pm 被 CEO 报告被盗，5:54pm 确认钓鱼，11:49pm 夺回.

</details>

<a id="other"></a>

## Diğer projeler

<details>
<summary>🧩 <b><a href="https://github.com/jaredpalmer/kev">jaredpalmer/kev</a></b> · ⭐6834 · Python · 👁️ observed · 0 天 · ⭐+1647</summary>

##### 📝 Özet

Kendi başınıza eğitip çalıştırabileceğiniz, Qwen3.5/3.8 üzerine kurulmuş Jev benzeri karar modelleri ailesi

<sub>🔧 Kodda kullanıldığı tespit edildi: `README.md`</sub>

##### 📌 Temel bilgiler

| Alan     | Değer            |
| -------- | ---------------- |
| Kategori | `Diğer projeler` |
| Kanıt    | `observed`       |
| Dil      | Python           |

##### 📊 Veri

| Ölçüt          | Değer            |
| -------------- | ---------------- |
| Yıldız         | **6834** (+1647) |
| Son push       | 2026-09-25       |
| İlk listelenme | 2026-09-21       |

🏷 `decision-model` · `jev` · `qwen3`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/jaredpalmer--kev/d4ced44cfa2a6f4e.png" width="100%" alt="jaredpalmer/kev screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🧩 <b><a href="https://github.com/HarnessRouter/SystemOneHarness">HarnessRouter/SystemOneHarness</a></b> · ⭐162 · Python · 👁️ observed · 3 天 · ⭐+29</summary>

##### 📝 Özet

System one modelleri için System one Harness.

##### 📌 Temel bilgiler

| Alan     | Değer            |
| -------- | ---------------- |
| Kategori | `Diğer projeler` |
| Kanıt    | `observed`       |
| Dil      | Python           |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **162** (+29) |
| Son push       | 2026-09-21    |
| İlk listelenme | 2026-09-20    |

</details>

<details>
<summary>🧩 <b><a href="https://github.com/Futureppo/typesafe_register">Futureppo/typesafe_register</a></b> · ⭐124 · Python · 👁️ observed · 3 天 · ⭐+8</summary>

##### 📝 Özet

typesafe.ai kayıt oluşturucu, en üst düzey optimizasyon, sınırsız jev

##### 📌 Temel bilgiler

| Alan     | Değer            |
| -------- | ---------------- |
| Kategori | `Diğer projeler` |
| Kanıt    | `observed`       |
| Dil      | Python           |

##### 📊 Veri

| Ölçüt          | Değer        |
| -------------- | ------------ |
| Yıldız         | **124** (+8) |
| Son push       | 2026-09-21   |
| İlk listelenme | 2026-09-22   |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/futureppo--typesafe_register/f058f3b3a08736ec.png" width="100%" alt="Futureppo/typesafe_register screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🧩 <b><a href="https://github.com/Liuziyu77/Valen">Liuziyu77/Valen</a></b> · ⭐113 · Python · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

Jev benzeri çok modlu bir modeli kendi başınıza eğitin. System One Model, artık görme yeteneğiyle.

##### 📌 Temel bilgiler

| Alan     | Değer            |
| -------- | ---------------- |
| Kategori | `Diğer projeler` |
| Kanıt    | `observed`       |
| Dil      | Python           |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **113**    |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-25 |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/liuziyu77--valen/67f00b2e5b787edc.png" width="100%" alt="Liuziyu77/Valen screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/liuziyu77--valen/d4fa3bf5c399a28d.gif" width="100%" alt="Liuziyu77/Valen animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

</details>

<details>
<summary>🧩 <b><a href="https://github.com/deepanwadhwa/OpenDecision">deepanwadhwa/OpenDecision</a></b> · ⭐57 · Python · 👁️ observed · 3 天 · ⭐+2</summary>

##### 📝 Özet

OpenDecision, typesafe'in jev'ine benzer açık kaynaklı bir anlamsal karar motorudur.

<sub>🔧 Kodda kullanıldığı tespit edildi: `examples/m3_typesafe_sdk_demo.py`</sub>

##### 📌 Temel bilgiler

| Alan     | Değer            |
| -------- | ---------------- |
| Kategori | `Diğer projeler` |
| Kanıt    | `observed`       |
| Dil      | Python           |

##### 📊 Veri

| Ölçüt          | Değer       |
| -------------- | ----------- |
| Yıldız         | **57** (+2) |
| Son push       | 2026-09-21  |
| İlk listelenme | 2026-09-19  |

🏷 `automation` · `jev` · `nli` · `nli-tasks` · `systemone` · `typesafe-ai`

</details>

<details>
<summary>🧩 <b><a href="https://github.com/parth-kp/jev-mail-classifier">parth-kp/jev-mail-classifier</a></b> · ⭐18 · Python · 👁️ observed · 4 天 · ⭐+3</summary>

##### 📝 Özet

Gelen kutunuzu Jev (TypeSafe's System One modeli) ile sınıflandırın — etiketleyin, taşıyın, işaretleyin ve bildirim gönderin; tümü yapılandırma odaklıdır.

##### 📌 Temel bilgiler

| Alan     | Değer            |
| -------- | ---------------- |
| Kategori | `Diğer projeler` |
| Kanıt    | `observed`       |
| Dil      | Python           |

##### 📊 Veri

| Ölçüt          | Değer       |
| -------------- | ----------- |
| Yıldız         | **18** (+3) |
| Son push       | 2026-09-20  |
| İlk listelenme | 2026-09-19  |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/parth-kp--jev-mail-classifier/08aebc25ef0891d7.png" width="100%" alt="parth-kp/jev-mail-classifier screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🧩 <b><a href="https://github.com/sqliteai/blink">sqliteai/blink</a></b> · ⭐17 · C · 👁️ observed · 2 天 · ⭐+10</summary>

##### 📝 Özet

Tek geçişli türlendirilmiş kararlar için, gömülebilir bir C çalışma zamanı ve WebAssembly desteğine sahip açık kaynaklı, yüksek performanslı bir System One Model.

##### 📌 Temel bilgiler

| Alan     | Değer            |
| -------- | ---------------- |
| Kategori | `Diğer projeler` |
| Kanıt    | `observed`       |
| Dil      | C                |

##### 📊 Veri

| Ölçüt          | Değer        |
| -------------- | ------------ |
| Yıldız         | **17** (+10) |
| Son push       | 2026-09-23   |
| İlk listelenme | 2026-09-23   |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/sqliteai--blink/2b387280ffd7c92b.png" width="100%" alt="sqliteai/blink screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🧩 <b><a href="https://github.com/Mushroom-Systems/lichen">Mushroom-Systems/lichen</a></b> · ⭐9 · Python · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

Jev için yerel, API uyumlu alternatif; TypeSafe'ın System One modeli

##### 📌 Temel bilgiler

| Alan     | Değer            |
| -------- | ---------------- |
| Kategori | `Diğer projeler` |
| Kanıt    | `observed`       |
| Dil      | Python           |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **9**      |
| Son push       | 2026-09-24 |
| İlk listelenme | 2026-09-25 |

</details>

<details>
<summary>🧩 <b><a href="https://github.com/Bald0Wang/jev-docs-zh">Bald0Wang/jev-docs-zh</a></b> · ⭐5 · Jupyter · 👁️ observed · 0 天 · ⭐+1</summary>

##### 📝 Özet

Jev 模型（TypeSafe AI）官方使用文档的中文翻译 | Unofficial Chinese translation of the official Jev (TypeSafe AI) docs — https://docs.typesafe.ai

##### 📌 Temel bilgiler

| Alan     | Değer            |
| -------- | ---------------- |
| Kategori | `Diğer projeler` |
| Kanıt    | `observed`       |
| Dil      | Jupyter          |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **5** (+1) |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-23 |

</details>

<details>
<summary>🧩 <b><a href="https://github.com/soderlind/ai-provider-for-jev">soderlind/ai-provider-for-jev</a></b> · ⭐4 · PHP · 👁️ observed · 6 天</summary>

##### 📝 Özet

Yapılandırılmış kararlar (choice, score, noul) için WordPress'i TypeSafe'ın Jev System One modeline bağlayın.

##### 📌 Temel bilgiler

| Alan     | Değer            |
| -------- | ---------------- |
| Kategori | `Diğer projeler` |
| Kanıt    | `observed`       |
| Dil      | PHP              |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **4**      |
| Son push       | 2026-09-18 |
| İlk listelenme | 2026-09-18 |

</details>

<details>
<summary>🧩 <b><a href="https://github.com/DDnim/jev-tweet-radar">DDnim/jev-tweet-radar</a></b> · ⭐1 · JavaScript · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

Chrome extension: score every X post with one Jev (System One Model) call — worth engaging, buzz, misread, repost/bookmark-worthy, AI-ish

##### 📌 Temel bilgiler

| Alan     | Değer            |
| -------- | ---------------- |
| Kategori | `Diğer projeler` |
| Kanıt    | `observed`       |
| Dil      | JavaScript       |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **1**      |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-25 |

</details>

<details>
<summary>🧩 <b><a href="https://github.com/aaditagrawal/laya-inference-engine">aaditagrawal/laya-inference-engine</a></b> · Python · 👁️ observed · 0 天 · **NEW**</summary>

##### 📝 Özet

an experimental blackwell optimised inference engine for laya, an open source system one model

##### 📌 Temel bilgiler

| Alan     | Değer            |
| -------- | ---------------- |
| Kategori | `Diğer projeler` |
| Kanıt    | `observed`       |
| Dil      | Python           |

##### 📊 Veri

| Ölçüt          | Değer      |
| -------------- | ---------- |
| Yıldız         | **0**      |
| Son push       | 2026-09-25 |
| İlk listelenme | 2026-09-25 |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/aaditagrawal--laya-inference-engine/600ade2965782ea2.png" width="100%" alt="aaditagrawal/laya-inference-engine screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary>🧩 <b><a href="https://github.com/jev-chat/jev-chat-windows">jev-chat/jev-chat-windows</a></b> · ⭐543 · Python · 🔎 inferred · 0 天 · ⭐+145</summary>

##### 📝 Özet

JevChat-Windows: sohbet penceresinin yanında duran yanıt yardımcısı. Pencere ekran görüntüsü + yerel çevrimdışı OCR karşı tarafın mesajını okur → Jev amacı belirler → 3 yanıt adayı tek tıklamayla doldurulur; gönderim her zaman manueldir

##### 📌 Temel bilgiler

| Alan     | Değer            |
| -------- | ---------------- |
| Kategori | `Diğer projeler` |
| Kanıt    | `inferred`       |
| Dil      | Python           |

##### 📊 Veri

| Ölçüt          | Değer          |
| -------------- | -------------- |
| Yıldız         | **543** (+145) |
| Son push       | 2026-09-24     |
| İlk listelenme | 2026-09-22     |

🏷 `chat-assistant` · `jev` · `llm` · `local-first` · `ocr` · `privacy` · `pyqt` · `python`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/jev-chat/jev-chat-windows/main/docs/ui_home.png" width="100%" alt="jev-chat/jev-chat-windows screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

<sub>Yeniden dağıtım lisansı belirtilmediği için varlık üst depodan doğrudan bağlanmıştır.</sub>

</details>

<details>
<summary>🧩 <b><a href="https://github.com/jev-chat/jev-chat-jarvis-mac">jev-chat/jev-chat-jarvis-mac</a></b> · ⭐384 · Python · 🔎 inferred · 0 天 · ⭐+94</summary>

##### 📝 Özet

Sohbet yüzen pencere yardımcısı (macOS): ekran algılama + amacı ve riski yerel küçük modelle belirleme, konuşma kalıplarına göre yanıt adayları oluşturma. Tamamen salt okunurdur.

##### 📌 Temel bilgiler

| Alan     | Değer            |
| -------- | ---------------- |
| Kategori | `Diğer projeler` |
| Kanıt    | `inferred`       |
| Dil      | Python           |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **384** (+94) |
| Son push       | 2026-09-24    |
| İlk listelenme | 2026-09-22    |

🏷 `llm` · `local-first` · `macos` · `ocr` · `privacy` · `pyobjc` · `python`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/jev-chat--jev-chat-jarvis-mac/daa9ab5c1715d764.gif" width="100%" alt="jev-chat/jev-chat-jarvis-mac screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/jev-chat--jev-chat-jarvis-mac/daa9ab5c1715d764.gif" width="100%" alt="jev-chat/jev-chat-jarvis-mac animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

</details>

<details>
<summary>🧩 <b><a href="https://github.com/Yinsongxu/LLM2Jev">Yinsongxu/LLM2Jev</a></b> · ⭐326 · Python · 🔎 inferred · 1 天 · ⭐+115</summary>

##### 📝 Özet

Yerel dil modellerini Jev tarzı yapılandırılmış karar modellerine dönüştürün. Yalnızca prefill ile metin ve görsellerden sonuç alın; belirteçleri tek tek kodlamaya gerek yoktur.

##### 📌 Temel bilgiler

| Alan     | Değer            |
| -------- | ---------------- |
| Kategori | `Diğer projeler` |
| Kanıt    | `inferred`       |
| Dil      | Python           |

##### 📊 Veri

| Ölçüt          | Değer          |
| -------------- | -------------- |
| Yıldız         | **326** (+115) |
| Son push       | 2026-09-23     |
| İlk listelenme | 2026-09-20     |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/yinsongxu--llm2jev/437593c7d81d05ee.jpg" width="100%" alt="Yinsongxu/LLM2Jev screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/yinsongxu--llm2jev/f1121682db48993a.gif" width="100%" alt="Yinsongxu/LLM2Jev animation"><br><sub>hareketli kayıt</sub></td>
</tr></table>

</details>

<details>
<summary>🧩 <b><a href="https://github.com/Zefan-Cai/Open-Jev">Zefan-Cai/Open-Jev</a></b> · ⭐324 · Python · 🔎 inferred · 2 天 · ⭐+67</summary>

##### 📝 Özet

Üst kaynakta açıklama yayımlanmamış.

##### 📌 Temel bilgiler

| Alan     | Değer            |
| -------- | ---------------- |
| Kategori | `Diğer projeler` |
| Kanıt    | `inferred`       |
| Dil      | Python           |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **324** (+67) |
| Son push       | 2026-09-23    |
| İlk listelenme | 2026-09-22    |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/zefan-cai--open-jev/44285bf2a2384cdd.jpg" width="100%" alt="Zefan-Cai/Open-Jev screenshot"></td>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/zefan-cai--open-jev/f6f2903828782e64.gif" width="100%" alt="Zefan-Cai/Open-Jev animation"><br><sub>hareketli kayıt · <a href="https://zefan-cai.github.io/open-jev/v1-1/replay.mp4">Videoyu aç</a></sub></td>
</tr></table>

</details>

<details>
<summary>🧩 <b><a href="https://github.com/TianyuCodings/JevHarness">TianyuCodings/JevHarness</a></b> · ⭐237 · Python · 🔎 inferred · 3 天 · ⭐+81</summary>

##### 📝 Özet

LLM tarafından yazılmış, göreve özgü Jev koşumları; isteğe bağlı tam yörünge ödül yansıtması ve GEPA evrimiyle.

##### 📌 Temel bilgiler

| Alan     | Değer            |
| -------- | ---------------- |
| Kategori | `Diğer projeler` |
| Kanıt    | `inferred`       |
| Dil      | Python           |

##### 📊 Veri

| Ölçüt          | Değer         |
| -------------- | ------------- |
| Yıldız         | **237** (+81) |
| Son push       | 2026-09-21    |
| İlk listelenme | 2026-09-22    |

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/TianyuCodings/JevHarness/main/docs/screenshots/pokemon-evolution.png" width="100%" alt="TianyuCodings/JevHarness screenshot"></td>
<td align="center" valign="top"><a href="https://raw.githubusercontent.com/TianyuCodings/JevHarness/main/docs/media/pokemon-comparison.mp4"><img src="https://raw.githubusercontent.com/TianyuCodings/JevHarness/main/docs/screenshots/pokemon-evolution.png" width="100%" alt="TianyuCodings/JevHarness video"></a><br><sub><a href="https://raw.githubusercontent.com/TianyuCodings/JevHarness/main/docs/media/pokemon-comparison.mp4">Videoyu aç</a></sub></td>
</tr></table>

<sub>Yeniden dağıtım lisansı belirtilmediği için varlık üst depodan doğrudan bağlanmıştır.</sub>

</details>

<details>
<summary>🧩 <b><a href="https://github.com/TypeLLM/TypeLLM">TypeLLM/TypeLLM</a></b> · ⭐217 · Python · 🔎 inferred · 0 天 · ⭐+179</summary>

##### 📝 Özet

TypeLLM: tür güvenli üretim ile LLMs

##### 📌 Temel bilgiler

| Alan     | Değer            |
| -------- | ---------------- |
| Kategori | `Diğer projeler` |
| Kanıt    | `inferred`       |
| Dil      | Python           |

##### 📊 Veri

| Ölçüt          | Değer          |
| -------------- | -------------- |
| Yıldız         | **217** (+179) |
| Son push       | 2026-09-25     |
| İlk listelenme | 2026-09-23     |

🏷 `ai` · `jev` · `llms` · `types`

---

<table><tr><th align="center" width="50%">🖼 Görsel</th><th align="center" width="50%">🎬 Video</th></tr><tr>
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/typellm--typellm/abc23488e639f57c.png" width="100%" alt="TypeLLM/TypeLLM screenshot"></td>
<td align="center" valign="top"><sub>medya yayımlanmamış</sub></td>
</tr></table>

</details>

<details>
<summary><b>Bu kategoride daha fazla</b> <sub>· 64</sub></summary>

- [AntonioGr7/Jeff](https://github.com/AntonioGr7/Jeff) - Local System one Model inspired by Jev.
- [justintout/systemone](https://github.com/justintout/systemone) - An unofficial Go module for interacting with Jev and other System One models.
- [kenmori/jev-dojo](https://github.com/kenmori/jev-dojo) - Jev（TypeSafe AI の System One モデル）を級・段で学ぶハンズオン教材.
- [Lancer59/SystemOneModels](https://github.com/Lancer59/SystemOneModels) - Testing out System One Models.
- [lnuxe/typesafe-docs-zh](https://github.com/lnuxe/typesafe-docs-zh) - TypeSafe AI 中文文档站（docs.typesafe.ai 中文复刻，Mintlify 构建）.
- [Macasacker/design-review](https://github.com/Macasacker/design-review) - design review tool for use by a systemone model.
- [nicothollot/system_one_model_testing](https://github.com/nicothollot/system_one_model_testing)
- [pranay-tecnomi/jev-2048](https://github.com/pranay-tecnomi/jev-2048) - Watch the Jev decision model (typesafe.ai) play 2048 live in your terminal.
- [tobegit3hub/systemone-open-service](https://github.com/tobegit3hub/systemone-open-service) - The open service for system one models.
- [yorshstudent-a11y/jevlike](https://github.com/yorshstudent-a11y/jevlike) - Train a compact model to pick one option from a changing list in a single pass…
- [Sac-Y/Jev-cu](https://github.com/Sac-Y/Jev-cu)
- [dabit3/jev-experiments](https://github.com/dabit3/jev-experiments)
- [logan-markewich/jeff](https://github.com/logan-markewich/jeff) - TypeSafe.
- [uehaj/jev-semgrep](https://github.com/uehaj/jev-semgrep) - Diller arasında anlama göre grep. TypeSafe Jev, her satırı bir anlama göre…
- [f/jev-leftpad](https://github.com/f/jev-leftpad) - Dizgileri TypeSafe AI&#x27;ın Jev&#x27;i ile soldan doldurun. Sebeplerden dolayı.
- [1Panel-dev/laya-server](https://github.com/1Panel-dev/laya-server) - Laya.
- [peterfriese/jev-foundation-models](https://github.com/peterfriese/jev-foundation-models) - TypeSafe AI&#x27;ın Jev System One karar modelini Apple&#x27;ın Foundation Models…
- [alvarobartt/sys1](https://github.com/alvarobartt/sys1) - Açık karar modelleri için System One uyumlu API, Rust ile yazılmıştır.
- [2951461586/Jev-Register-Tool](https://github.com/2951461586/Jev-Register-Tool) - TypeSafe（Jev / System One）başvuru → onay e-postası → onay alma → kayıt → API…
- [HITsz-TMG/JevEmbed](https://github.com/HITsz-TMG/JevEmbed) - JevEmbed ile tanışın — gömme vektörlerini kararlara dönüştürün.
- [Kevthetech143/super-jev](https://github.com/Kevthetech143/super-jev) - TypeSafe Jev için küçük, genişletilebilir bir karardan eyleme geçiş düzeneği.
- [manifoldor/xtags](https://github.com/manifoldor/xtags) - X zaman akışında her gönderinin senden ne yapmanı istediğini etiketler.
- [yijunyu/jev-rs](https://github.com/yijunyu/jev-rs) - Tek bir prefill içinde herhangi bir LLM kaynağından System One yargıları…
- [comoc/jev-minesweeper](https://github.com/comoc/jev-minesweeper) - TypeSafe Jev (System One) öğesine tarayıcıdaki Mayın Tarlası.
- [rawwerks/one-system](https://github.com/rawwerks/one-system) - Yerel ve barındırılan sınıflandırıcıları, yani karar modellerini, yani Jev…
- [Abhinavexists/lev](https://github.com/Abhinavexists/lev) - An open System One decision model.
- [dagfinndybvig/Jev_Ontology](https://github.com/dagfinndybvig/Jev_Ontology) - Trying to combine Jev with ontology.
- [haseeb-heaven/jev-system-one](https://github.com/haseeb-heaven/jev-system-one) - Şeffaf karar raporlarıyla yanıtlar sunan, özenle hazırlanmış bir OpenAI +…
- [lbotinelly/jev-little-airways](https://github.com/lbotinelly/jev-little-airways) - TypeSafe.
- [Charlyhno-eng/jev-document-classification](https://github.com/Charlyhno-eng/jev-document-classification) - JEV Document Classification enables the rapid and cost-effective classification…
- [exfly/laya-jev-compatible-server](https://github.com/exfly/laya-jev-compatible-server) - TypeSafe Jev uyumlu bir HTTP sunucusu (POST /v1/systemone).
- [jinlio/mjbrain](https://github.com/jinlio/mjbrain) - 基于 LAYA 微调的立直麻将实时决策推荐系统 — real-time mahjong decision advisor.
- [TanayPadar/gpt-vs-jev](https://github.com/TanayPadar/gpt-vs-jev) - Aynı girdi üzerinde GPT tarafından üretilen dili JEV yapılandırılmış Noul…
- [XingQiPan/rwkv-jev](https://github.com/XingQiPan/rwkv-jev) - 基于 RWKV-7 (g1k-1b) 的 Jev 式零生成决策引擎：受限 softmax 直读概率分布，支持 noul/choice/score…
- [asp616848/better-jev-for-all](https://github.com/asp616848/better-jev-for-all) - Open, self-hostable, faster System One decision model — API-compatible…
- [fukukei23/jev-harness-guide](https://github.com/fukukei23/jev-harness-guide) - Jevハーネスガイド — 判断を外注する超軽量モデルSystem Oneの実践ガイド.
- [hazlema/jev-patterns](https://github.com/hazlema/jev-patterns) - Extracting repeating patterns from noisy sequences with Jev.
- [iAziz786/omp-jev-decision](https://github.com/iAziz786/omp-jev-decision) - omp extension: a read-tier `decision` tool backed by TypeSafe System One (Jev)…
- [katya4oyu/jev-systemone-local](https://github.com/katya4oyu/jev-systemone-local) - Local System One-compatible decision server for Laya backends.
- [krisitown/jev-loop-control](https://github.com/krisitown/jev-loop-control) - Bounded Jev supervision for Pi: observe decisions, block tool batches, and…
- [Lasimeri/Intel-Phi-Jev](https://github.com/Lasimeri/Intel-Phi-Jev) - XKEYSCORE for Jev: TypeSafe System One judgments (noul/choice/score) served…
- [LiLittleCat/jev-playlist-classify](https://github.com/LiLittleCat/jev-playlist-classify) - Classify large playlists by song language with TypeSafe AI.
- [mattheworiordan/jev-landscape](https://github.com/mattheworiordan/jev-landscape) - Labels, tables and code for &quot;A week of Jev, sorted&quot;: what X posts built with…
- [meddle-connect/jev-fault-diagnosis-poc](https://github.com/meddle-connect/jev-fault-diagnosis-poc) - Fault Diagnosis PoC replays real compressor telemetry through an emulated…
- [Mihir-Rabari/jev-one-click](https://github.com/Mihir-Rabari/jev-one-click) - Deploy a Jev-compatible decision API locally in one click.
- [ORiONx888/JevSentinel](https://github.com/ORiONx888/JevSentinel) - Real-time token risk intelligence and early-warning protection engine for…
- [rocstack/jev-profanity](https://github.com/rocstack/jev-profanity) - Context-aware profanity detection for JavaScript and TypeScript, powered by Jev…
- [rupeshs/flappybird-jev-test](https://github.com/rupeshs/flappybird-jev-test) - Testing flappy bird with SystemOne Jev like model locally.
- [ShivaniKumar1/Jev-Chess](https://github.com/ShivaniKumar1/Jev-Chess) - Jev Plays Chess Using DigitalOcean /v1/systemone Endpoint.
- [SoMaCoSF/jev-minesweeper-harness](https://github.com/SoMaCoSF/jev-minesweeper-harness) - Jev System One tester: Minesweeper as a typed decision world, plus a generic…
- [sprah/jev-monid-poc](https://github.com/sprah/jev-monid-poc) - Proof-of-concept combining TypeSafe AI&#x27;s Jev with Monid&#x27;s pay-per-call API…
- [taylorbayouth/jev-infinite-ctx](https://github.com/taylorbayouth/jev-infinite-ctx) - A 32k model, a 250k document, one decision back.
- [vamsikrishna2421/jev-usecases](https://github.com/vamsikrishna2421/jev-usecases) - Jev.
- [WEP-56/IF_JEV](https://github.com/WEP-56/IF_JEV) - jev与LLLM共同施工的世界线引擎，不同于酒馆，在 IF 中，用户只是一个事件发生器.
- [yukurash/jev-playwright-repair-lab](https://github.com/yukurash/jev-playwright-repair-lab) - Locator-only Playwright repair: rules vs Azure GPT-5.5 vs Jev, with independent…
- [receptron/laya](https://github.com/receptron/laya) - Açık kaynaklı Jev uyumlu System-1 karar modeli Laya.
- [jerryjliu/docjev](https://github.com/jerryjliu/docjev) - Jev kullanan çok hızlı bir belge sınıflandırıcısı/bölücüsü.
- [Rizzo-AI-Academy/rizzo-flow](https://github.com/Rizzo-AI-Academy/rizzo-flow) - Jev&#x27;in açık, yerel yorumu: bir LLM&#x27;den tek bir token üretmeden tiplendirilmiş…
- [razorback16/openjev](https://github.com/razorback16/openjev) - DiffusionGemma üzerinde açık, Jev uyumlu System One karar sunucusu.
- [iapp-technology/openthai-systemone](https://github.com/iapp-technology/openthai-systemone) - OpenThai-SystemOne: açık kaynaklı Tayca + İngilizce System One karar modeli…
- [hawkymisc/typed-decision-bert](https://github.com/hawkymisc/typed-decision-bert) - Resmî olmayan PoC: tipli-karar (noul / choice / score) HTTP API arkasında BERT…
- [bokuweb/omg](https://github.com/bokuweb/omg) - A System One style decision model runtime in Rust.
- [fritzprix/systemone-lite](https://github.com/fritzprix/systemone-lite) - Toy local System One–style decision API (Jev-shaped).
- [newfull5/malkuth](https://github.com/newfull5/malkuth) - Jev like System One decision models for multilingual.

</details>

<a id="projects-by-implementation-language"></a>

## Uygulama diline göre projeler

Ekosistem Python ve TypeScript'te yoğunlaşıyor, ancak tipli istemciler başka dillerde de görünmeye devam ediyor. Bu tablo kayıtların kendisinden üretilir.

| Dil        | Kayıt | Örnekler                                                                                                                  |
| ---------- | ----- | ------------------------------------------------------------------------------------------------------------------------- |
| Python     | 295   | `typesafe-ai/system-one-adapter-python`, `typesafe-ai/typesafe-sdk-python`, `bodepudimuneendra-netizen/laya-jev-GraphRAG` |
| TypeScript | 154   | `typesafe-ai/typesafe-sdk-js`, `TypeSafeAI/jev-harness`, `agencyenterprise/jev-recipes`                                   |
| JavaScript | 95    | `ChenYCL/jev-browser-skill`, `clouatre-labs/decisions-judge-mcp`, `hemanth/jevql`                                         |
| HTML       | 32    | `typesafe-ai/typesafe-ai.github.io`, `yzfly/awesome-jev-zh`, `AkashPriyadarshii/jev-superpowers`                          |
| Rust       | 26    | `AkashPriyadarshii/jev-curate`, `AkashPriyadarshii/jev-seo`, `shaharia-lab/jev-cli`                                       |
| Go         | 17    | `kisshan13/typesafe-ai-go`, `juangchuank-ops/jev2api`, `olivere/systemone`                                                |
| Java       | 12    | `Premo-Cloud/typesafe-sdk-java`, `danvega/jev-spring-boot-starter`, `Olti1947/jev-java`                                   |
| C#         | 6     | `ryancole/jev-dotnet`, `saibimajdi/typesafeai-dotnet-sdk`, `kkdev92/jev-dotnet`                                           |
| Jupyter    | 5     | `jexp/neo4jev`, `GitHub30/OpenJev`, `atliq/jev-ai-use-cases`                                                              |
| PHP        | 4     | `basi/typesafe-ai-php-client`, `Butochnikov/laravel-typesafe-jev`, `dirnbauer/typo3-webcon-jev`                           |
| C          | 3     | `CheshiAI/Cheshi`, `giuliosmall/pg_typesafe`, `sqliteai/blink`                                                            |
| PowerShell | 3     | `BFLabsAI/bf-jev-deep-research`, `omni-/ask-jev`, `dfinke/Jev`                                                            |
| Swift      | 3     | `john-rocky/coreai-kit`, `bkarak/jev-mac`, `peterfriese/jev-foundation-models`                                            |
| C++        | 2     | `pewriebontal/typesafe-sdk-cpp`, `itsadrianxv/jev-quant`                                                                  |
| Elixir     | 2     | `nshkrdotcom/typesafe_sdk`, `dannote/jev`                                                                                 |
| Kotlin     | 2     | `snevadalabs/jev-kmp`, `jev-chat/jev-chat-jarvis`                                                                         |
| Ruby       | 2     | `Ray-Hughes/jevalyn`, `obie/ruby_decision_model`                                                                          |
| Shell      | 2     | `realZachi/pg-jev`, `slashdaemon/jev-expert`                                                                              |
| Astro      | 1     | `heyjunpenn/awesome-jev`                                                                                                  |
| Lua        | 1     | `kiwi0719/jev-edge`                                                                                                       |
| Nushell    | 1     | `cablehead/jev.nu`                                                                                                        |
| Perl       | 1     | `usualoma/mt-plugin-jev`                                                                                                  |
| R          | 1     | `mountainMath/JevR`                                                                                                       |
| TeX        | 1     | `dnakhoa/jev-deferred-crispification`                                                                                     |

<sub>Yalnızca dil belirten kayıtlar sayılır. Altyapı, dokümantasyon ve tartışma kayıtları bu tablonun dışında tutulur.</sub>

## Bu liste nasıl güncel kalıyor

Bu README'nin gövdesini hiçbir insan düzenlemez. Depo, zamanlanmış biçimde beş aşamalı bir iş akışı çalıştırır ve yalnızca gerçekten bir şey değiştiğinde commit eder.

<img src="../assets/readme/pipeline.svg" width="100%" alt="Bu liste nasıl güncel kalıyor">

|             |                                                                                                                                                                                                                                                                                              |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **collect** | bir sorgu matrisi, resmî organizasyon, GitHub kod araması, Hacker News ve Hugging Face hub üzerinden GitHub araması.                                                                                                                                                                         |
| **curate**  | deterministik ve LLM'siz, bu yüzden aynı girdiyle art arda iki çalıştırma bayt düzeyinde aynı çıktıyı üretir. Alaka düzeyine iki sinyalli bir kural karar verir; ad çakışmaları (JeVois, JEvents, Jevil, jEveAssets, ESP32-RLCD ve benzerleri) açık ve denetlenebilir bir listeyle dışlanır. |
| **media**   | her projenin kendi ekran görüntülerini ve ekran kayıtlarını toplar. Varlıklar bu depoya yalnızca proje yeniden dağıtıma uygun bir lisans beyan ettiğinde kopyalanır; aksi halde üst kaynağın URL'si doğrudan bağlanır ve kart bunu belirtir.                                                 |
| **render**  | her dil sürümünü tek bir şablondan üretir, böylece yirmi README yapı açısından asla birbirinden ayrışamaz.                                                                                                                                                                                   |
| **audit**   | bir kaydın URL'si yoksa, bir bağlantı ölüyse, iki kayıt aynı URL'yi yineliyorsa veya bir README üretilen biçiminden sapıyorsa derlemeyi başarısız kılar.                                                                                                                                     |

## Katkıda bulunma

Düzeltmeler memnuniyetle karşılanır ve bu listeyi iyileştirmenin en hızlı yoludur. Bir kayıt yanlış sınıflandırılmış, yanlış derecelendirilmiş ya da bir proje ad çakışması diye hatalı biçimde dışlanmışsa bir issue veya pull request açın — otomatik filtrelerin en çok yanılma olasılığı son kategoridedir. Ekleme yapmanın en iyi yolu README'yi düzenlemek değil, `scripts/collect.py` dosyasına bir kaynak eklemektir; çünkü README her turda yeniden üretilir.

---

<sub>Bağımsız topluluk projesi. TypeSafe AI ile bağlantılı değildir; onun tarafından onaylanmamış veya incelenmemiştir. Ürün davranışı, fiyatlandırma, sınırlar ve model takma adları bildirimsiz değişebilir; kritik her şeyi resmî belgelerle doğrulayın. Varlıklar üst projelerinin mülkiyetinde kalır ve yalnızca lisansın izin verdiği yerlerde yeniden üretilir.</sub>

<sub>Oluşturan · `render.py` · 2026-09-25T18:10:37+08:00</sub>
