...
> [!NOTE]
> **라이브 인덱스** · 최근 동기화: `2026-09-25T18:10:37+08:00` (UTC+8)
> · 항목: **799** · 이번 회차 신규: **298** · 구현 언어: **24**
...
<sub>🔧 코드에서 사용 확인: `src/typesafe_sdk/__init__.py`</sub>
...
<sub>🔧 코드에서 사용 확인: `Sources/CoreAIOps/CoreAI+SystemOne.swift`</sub>
...
교체 가능한 System One 모델(로컬 Laya / 클라우드 Jev)을 사용하는 데이터베이스 불가지론적 Agentic GraphRAG 프레임워크. 모든 그래프 데이터베이스를 위한 완전한 4단계 파이프라인과 맞춤형 A* 탐색을 특징으로 하는 플러그 앤 플레이 지능 계층.
...
200개 이상의 플러그 앤 플레이 Jev 레시피: 에이전트, RAG, 지원, 코드 리뷰 및 음악을 위해 텍스트를 라우팅, 채점, 게이트, 비교, 라벨링하는 작고 보정된 AI 결정. JavaScript 또는 TypeScript에서 가져오거나, 어떤 언어에서든 JSON로 CLI를 호출하세요.
...
Jev, TypeSafe의 System One 모델을 위한 커뮤니티 Java SDK: 타입 지정 질문을 입력하면 보정된 확률이 포함된 타입 지정 답변이 출력됩니다. Java 17+, Spring Boot starter(비공식)
...
모든 로봇을 위한 하나의 CLI. 로봇들을 연결하고, 명령하고, 함께 일하게 하세요. 각 로봇은 두뇌로 LLM(Claude, OpenAI, Gemini, Grok, 또는 Ollama나 vLLM를 통한 로컬)를 사용하고, 다중 선택을 위한 결정 LLM(Jev, Laya, Kev)를 사용합니다. 노트북에서 오프보드로 Microduck, Open Duck Mini, LeRobot, XLeRobot, AlohaMini, ToddlerBot 또는 모든 ROS 2 베이스를 구동합니다.
...
작업에 대해 Cursor, Claude Code, Codex 또는 OpenCode + 모델/노력을 선택한 뒤 실행하는 CLI. Jev와 Herdr로 구동됩니다
...
<details>
<summary><b>이 분류의 나머지</b> <sub>· 111</sub></summary>
...
- [chengyongru/fastjev](https://github.com/chengyongru/fastjev) - 빠른 셀프 호스팅 시맨틱 결정을 위한 SDK 우선, 독립 유지 관리되는 SemIf 포크.
...
- [utk2103/jev-studio](https://github.com/utk2103/jev-studio) - jev를 실험하고 있다면 여기서부터 더 쉬울 것입니다.
...
TREC 컬렉션에서의 검색 엔진 실험. 현재 typesafe.ai의 JEV 모델을 사용한 제로샷 재순위화 구현에 집중
...
로컬 오픈 소스 Jev 대안: 자체 컴퓨터에서 Laya(System One 모델)를 사용한 브라우저 에이전트 의사결정. 클라우드와 API 키가 필요 없습니다. Playwright/CDP 및 MCP 친화적입니다.
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/mansicer--jev-plays/c4f1ec41afcb4ae9.gif" width="100%" alt="mansicer/jev-plays animation"><br><sub>움직이는 화면 녹화</sub></td>
</tr></table>
...
- [angel291592/Intent-Router](https://github.com/angel291592/Intent-Router) - AI 에이전트를 위한 의도 컴파일러 — 모호한 요청을 타입 지정 IntentSpec 계약으로 수렴시킵니다(라우팅 전 탐색, 질문 또는 중단).
- [socai-io/jev-social](https://github.com/socai-io/jev-social) - 오픈 소스 Agent Skill 및 로컬 데모: Instagram, TikTok, LinkedIn 리서치를 위한 Jev × socai—타입…
...
- [PyModel/jev-judge-mcp](https://github.com/PyModel/jev-judge-mcp) - MCP 에이전트를 위한 타입 지정 판단 도구. TypeSafe의 Jev 모델을 verify, screen, find, classify…
...
- [khordoo/jev-reflex-autonomy-lab](https://github.com/khordoo/jev-reflex-autonomy-lab) - 빠른 System 1 반사적 결정과 충돌 회피를 위해 Jev를 사용하고, 전략적 안내를 위해 선택적 System 2 추론을 사용하는 실시간…
...
- [AskTheWay/dsh-jev-interceptor](https://github.com/AskTheWay/dsh-jev-interceptor) - ⚡ DeepSeek Harness의 모든 도구 호출을 위한 밀리초 System-1 판단 — Jev 기반 위험 분류 및 증거 게이트 자동 승인.
...
- [itsmostafa/typesafe-mcp](https://github.com/itsmostafa/typesafe-mcp) - 무엇이든 빠르고 저렴하게 평가하는 mcp 커넥터. AI 에이전트에 typesafe ai의 jev 모델과 laya 같은 오픈 가중치 모델에 대한…
...
트래픽 엣지에서의 타입 지정 판단 접근 제어: nginx/OpenResty를 위한 3계층 프롬프트 인젝션 및 악용 필터, TypeSafe Jev로 구동. Fail-open, 캐시됨, 핫 리로드 가능.
...
음성과 가리키는 손가락으로 tldraw 캔버스에 그리세요. Jev (TypeSafe System One)가 말한 단어당 약 350ms 만에 동작, 대상 및 위치를 결정합니다.
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/weitianxin--jevany/711125367b6758ff.gif" width="100%" alt="weitianxin/JevAny animation"><br><sub>움직이는 화면 녹화</sub></td>
</tr></table>
...
- [Eliot5566/JEV-Paper-Radar](https://github.com/Eliot5566/JEV-Paper-Radar) - 매일 아침 Jev가 모든 새 arXiv paper를 읽고, 읽어야 할 몇 편을 드러내게 하세요.
...
비정형 상태에서 타입 지정 결정: 한 번의 순전파, 생성 텍스트 없음. 로컬, 결정론적, Jev 호환. typesafe.ai와 제휴하지 않았습니다.
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/antonellof--laya-vs-dijkstra/b61bc3a8e3d368be.gif" width="100%" alt="antonellof/laya-vs-dijkstra animation"><br><sub>움직이는 화면 녹화</sub></td>
</tr></table>
...
집의 3090에서, 오픈 모델로 구현한 Semantic ifs. 독립적이며, Jev 또는 TypeSafe와 제휴하지 않았습니다.
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/theoleecj--semif-openjev/0504149cd9ef3cad.gif" width="100%" alt="TheoLeeCJ/SemIf-OpenJev animation"><br><sub>움직이는 화면 녹화</sub></td>
</tr></table>
...
Jev 같은 멀티모달 모델을 직접 훈련하세요. System One Model, 이제 비전 포함.
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/liuziyu77--valen/d4fa3bf5c399a28d.gif" width="100%" alt="Liuziyu77/Valen animation"><br><sub>움직이는 화면 녹화</sub></td>
</tr></table>
...
Jev, TypeSafe의 System One 모델을 위한 로컬 API 호환 대체품
...
- [HITsz-TMG/JevEmbed](https://github.com/HITsz-TMG/JevEmbed) - JevEmbed를 만나보세요 — 임베딩을 결정으로 전환합니다. 원하는 임베딩 모델로 선택하고, 점수화하고, 판단하세요.
...
<sub>생성 도구 · `render.py` · 2026-09-25T18:10:37+08:00</sub>
