...
> [!NOTE]
> **라이브 인덱스** · 최근 동기화: `2026-09-23T17:45:55+08:00` (UTC+8)
> · 항목: **778** · 이번 회차 신규: **111** · 구현 언어: **23**
...
<sub>패턴으로 작성할 수 없는 질문을 grep합니다. 일부 함수만 샘플링하는 대신 모든 함수를 읽습니다. Jev, TypeSafe AI의 System One 모델로 구동됩니다.</sub>
</td>
</tr>
<tr>
<td width="50%" valign="top">
...
- [에이전트 도구: MCP, 훅, 게이트, 코딩 에이전트](#에이전트-도구-mcp-훅-게이트-코딩-에이전트) — **239**
- [라우팅, 가드레일, 승인](#라우팅-가드레일-승인) — **55**
- [평가, 캘리브레이션, 벤치마크](#평가-캘리브레이션-벤치마크) — **78**
- [공개 재현, 가중치, 아키텍처 연구](#공개-재현-가중치-아키텍처-연구) — **41**
...
<sub>🔧 코드에서 사용 확인: `src/typesafe_sdk/__init__.py`, `src/typesafe_sdk/_core/retry.py`, `src/typesafe_sdk/_core/config.py`</sub>
...
TypeSafe AI의 Jev을(를) 위한 맞춤형 코딩 하네스입니다. LLM가 제안하고, Jev가 좁은 질문에 답하며, 코드가 결정하고, 모든 단계에 기록이 남습니다.
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/clouatre-labs--decisions-judge-mcp/c8c00f2b58774857.gif" width="100%" alt="clouatre-labs/decisions-judge-mcp animation"><br><sub>움직이는 화면 녹화</sub></td>
</tr></table>
...
모든 로봇을 위한 단일 오프보드 CLI입니다. 로봇을 연결하고 명령을 내리며 서로 협력하게 하세요. 각 로봇은 두뇌로 LLM(Claude, OpenAI, Gemini, Grok 또는 Ollama나 vLLM를 통한 로컬 모델)를 사용하고, 더 저렴하고 빠른 단계를 위한 의사 결정 LLM(Jev, Laya, Kev)를 사용합니다. Microduck, Open Duck Mini, LeRobot, XLeRobot, AlohaMini, ToddlerBot, Jetson 또는 모든 ROS를 지원합니다.
...
Apple의 Core AI를 사용해 iPhone 및 Mac에서 채팅, 비전 및 음성 모델을 실행하기 위한 Swift SDK. 모델 다운로드 및 캐싱, FoundationModels 통합, 문서화된 OS, SDK 및 모델 요구 사항을 갖춘 실행 가능한 예제.
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/john-rocky--coreai-kit/c98c7bb8cb04e76e.gif" width="100%" alt="john-rocky/coreai-kit animation"><br><sub>움직이는 화면 녹화</sub></td>
</tr></table>
...
jev-seo: 코딩 에이전트를 위한 Rust SEO 및 GEO CLI와 MCP 서버: 50개 규칙 감사, 실시간 크롤링, GEO 점수, 순위 변동, CI 게이트. MIT, 구독료 없음.
...
읽기 전용 트레이딩 저널 및 검토 하니스: Jev 유형이 지정된 판단, 에이전트 통합 및 재현 가능한 금융 벤치마크를 제공합니다. 주문 및 조언은 제공하지 않습니다.
...
텍스트 파일용 Jev 기반 규칙 기반 채점기. 모든 줄에 대해 모든 규칙을 병렬로 실행합니다. 대충 훑어보지 않고, 놓치는 줄도 없습니다.
...
빠른 셀프 호스팅 semantic decisions를 위한 SDK 우선, 독립적으로 유지 관리되는 SemIf 포크.
...
- [bladedevoff/stuntd](https://github.com/bladedevoff/stuntd) - 앱의 타입화된 LLM 결정을 학습하고 Laya head로 답변하는 로컬 프록시. Jev 및 OpenAI와 호환됩니다.
- [tumf/jev-cli](https://github.com/tumf/jev-cli) - TypeSafe Jev용 작은 의존성 없는 CLI.
- [talmago/fast_gliner](https://github.com/talmago/fast_gliner) - Rust로 작성된 GLiNER 모델용 추론 엔진에 대한 Python 바인딩입니다.
- [frostney/clean-code-review](https://github.com/frostney/clean-code-review) - 풀 리퀘스트의 모든 코드 파일을 TypeSafe의 Jev을(를) 사용해 Uncle Bob의 Clean Code를 기준으로 평가한 다음…
...
- [vercel-labs/jev-ai-sdk-form-router](https://github.com/vercel-labs/jev-ai-sdk-form-router) - Jev와 AI SDK로 양식 제출을 적절한 사람들에게 라우팅하세요.
- [ansidium/jev-codex-bridge](https://github.com/ansidium/jev-codex-bridge) - Codex Desktop 및 CLI를 위한 모델 및 reasoning 라우팅, Windows 서비스와 검증된 업데이트 포함.
- [mountainMath/JevR](https://github.com/mountainMath/JevR) - TypeSafe Jev System One API용 R 클라이언트.
- [shinpr/jev-reranker](https://github.com/shinpr/jev-reranker) - JSON 검색 결과를 TypeSafe AI의 Jev로 재순위 지정하고, 필터링하고, 압축합니다.
...
- [nylon-memory/NylonME](https://github.com/nylon-memory/NylonME) - AI 에이전트를 위한 메모리 엔진입니다. Jev와 함께 작업하면 의사결정 정확도를 높일 수 있습니다.
...
<td align="center" valign="top"><a href="https://youtu.be/ZC4oge4WsCQ"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/24601--augustus/d2593af47d538d33.jpg" width="100%" alt="video"></a><br><sub><a href="https://youtu.be/ZC4oge4WsCQ">시청 위치 youtu.be</a> · 재생은 호스트 사이트에서 열립니다. GitHub에서는 인라인으로 삽입할 수 없습니다</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/storm-software--mindctl/69298c40a77b7be5.gif" width="100%" alt="storm-software/mindctl animation"><br><sub>움직이는 화면 녹화</sub></td>
</tr></table>
...
<details>
<summary><b>이 분류의 나머지</b> <sub>· 217</sub></summary>
...
- [kydlikebtc/awesome-jev](https://github.com/kydlikebtc/awesome-jev) - Jev의 검증된 예제 805개 — TypeSafe AI의 System One decision 모델 — 언급한 블로그가 아니라 각 예제가 내리는…
- [Amal-David/awesome-jev](https://github.com/Amal-David/awesome-jev) - Jev 데모, 프로젝트, SDKs 및 기술, 소스 링크와 큐레이션된 X 갤러리 포함.
...
- [Alex314618-create/JevRev](https://github.com/Alex314618-create/JevRev) - LLM + Jev 워크플로. 내부의 spine으로 척추동물의 뇌를 강화하세요.
...
- [socai-io/jev-social](https://github.com/socai-io/jev-social) - Jev 기반 Instagram, TikTok 및 LinkedIn 리서치: 유형이 지정된 라우팅, 실제 브라우저 증거, 스트리밍 게시물 카드…
...
- [Bodila51/muse-jev-playbook](https://github.com/Bodila51/muse-jev-playbook) - Muse를 위한 Jev 의사 결정 계층: 비용이 많이 드는 에이전트 작업 전에 거치는 빠르고 저렴한 TypeSafe AI 게이트 — 신뢰도…
...
- [rashedInt32/jev-mcp](https://github.com/rashedInt32/jev-mcp) - TypeSafe Jev를 타입화되고 보정된 판단 도구로 노출하는 MCP 서버: 분류, 점수화, 확인, 배치 ask.
...
- [green-dalii/pi-shift-router](https://github.com/green-dalii/pi-shift-router) - Pi 코딩 에이전트를 위한 턴별 모델 라우팅: 작은 judge가 각 메시지마다 저렴한 티어 또는 강력한 티어를 선택하며, 다중 모델 장애…
...
- [itsmostafa/typesafe-mcp](https://github.com/itsmostafa/typesafe-mcp) - 무엇이든 빠르고 저렴하게 평가하는 mcp 커넥터입니다. AI 에이전트가 typesafe ai의 jev 모델에 직접 접근할 수 있게 합니다.
...
비자기회귀형 System 1 의사결정 엔진. 단일 순전파로 모든 텍스트에 대해 100개 이상의 언어에서 유형이 지정된 선택, 점수 및 예/아니요 결정을 수행하며, 요청마다 적합한 체크포인트를 선택하는 라우터를 제공합니다.

<sub>🔧 코드에서 사용 확인: `research/benchmarks/feishu_zh/run.py`</sub>
...
Jev, TypeSafe의 System One 모델이 모든 OpenRouter LLM, Stockfish 및 사용자와 체스를 둡니다. 실시간 수, Jev의 수별 확률, 저장된 게임 및 승률을 제공하는 한 페이지 웹 앱입니다.
...
훈련된 정책이 없는 Clash Royale 봇입니다. Jev(TypeSafe System One)가 실시간 게임 상태만으로 모든 결정을 내립니다.
...
AI 평가를 위한 검증 레이어. 점수뿐 아니라 도구를 확인합니다: 데이터, 채점기, 실행, 숫자, 주장, 그리고 그 자체.
...
어떤 LLM든 Jev 스타일의 의사결정 모델로 변환합니다: 유형이 지정된 의사결정, 실제 확률, 학습 불필요. (계속 업데이트 중)
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/nokia-applied-research--anyjev/93e61c3de618cc36.gif" width="100%" alt="nokia-applied-research/AnyJev animation"><br><sub>움직이는 화면 녹화</sub></td>
</tr></table>
...
Jev 스타일 의사 결정 모델을 위한 오픈 학습 및 추론 스택입니다. 공유 접두사에서 동적인 후보 분기를 평가하도록 모델을 학습시키며, 높은 카디널리티 선택, 보정 및 빠른 배치 추론을 지원합니다.
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/weitianxin--jevany/05825e369317a429.gif" width="100%" alt="weitianxin/JevAny animation"><br><sub>움직이는 화면 녹화</sub></td>
</tr></table>
...
- [RenaGao/jev-dataops](https://github.com/RenaGao/jev-dataops) - 스트리밍 데이터 선택, 품질 평가, 자동 LoRA 학습 및 홀드아웃 모델 평가를 위한 오픈 소스 JEV 기반 워크벤치입니다.
...
- [ARCJ137442/jev-2048](https://github.com/ARCJ137442/jev-2048) - 모든 수를 Jev(TypeSafe AI System One) Choice로 기록하는 계측형 2048 웹 실험실이며, 휴리스틱 대체 로직은…
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/wh000wh000/awesome-jev-live/main/media/fbddcz--embodied-jev/297ef7c24f9a67a8.gif" width="100%" alt="FBddcz/embodied-jev animation"><br><sub>움직이는 화면 녹화 · <a href="https://raw.githubusercontent.com/FBddcz/embodied-jev/main/docs/media/libero-drawer-comparison.mp4">동영상 열기</a></sub></td>
</tr></table>
...
<details>
<summary><b>이 분류의 나머지</b> <sub>· 19</sub></summary>
...
사람들이 Jev, TypeSafe의 System One 모델로 구축한 것들을 가장 완벽하게 모아 놓은 갤러리입니다. 시나리오별로 3,400개 이상의 프로젝트, 데모 및 작성 글을 원본 링크, 이미지, 설명과 함께 제공합니다.
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/darthblanc/tictacjev/main/assets/screenrecording.gif" width="100%" alt="darthblanc/tictacjev animation"><br><sub>움직이는 화면 녹화</sub></td>
</tr></table>
...
<td align="center" valign="top"><img src="https://raw.githubusercontent.com/surbhit20/jev-plays-snake/main/jev-plays-snake.gif" width="100%" alt="surbhit20/jev-plays-snake animation"><br><sub>움직이는 화면 녹화</sub></td>
</tr></table>
...
- [heyjunpenn/awesome-jev](https://github.com/heyjunpenn/awesome-jev) - Jev로 구축된 832개의 오픈 소스 프로젝트를 검증하고 커뮤니티가 관리하는 카탈로그입니다.
...
- [jaibhasin/jev-yt-time-saver](https://github.com/jaibhasin/jev-yt-time-saver) - 방해되는 YouTube 동영상을 Jev로 가리는 Chrome 확장 프로그램입니다. 원할 때 언제든 표시할 수 있습니다.
...
- [Show HN: Will Jev pull the lever in the trolley problem?](https://news.ycombinator.com/item?id=49773756) - 여러분 모두가 기다려 온 Jev 벤치마크: Jev는 트롤리 문제에서 레버를 당길까요? 너무 깊이 생각하지 마세요 :) 선로 양쪽에 원하는…
...
微信(Windows 4.x)에 곁붙여 사용하는 답장 보조 도구: 창 스크린샷 + 로컬 오프라인 OCR로 상대방 메시지 읽기 → Jev로 의도 판단 → 후보 3개를 한 번에 입력, 전송은 항상 수동
...
LLM가 작성한 작업별 Jev 하네스로, 선택적 전체 궤적 보상 반영 및 GEPA 진화를 지원합니다.
...
DeepSeek + TypeSafe JEV 微信 답변 도우미: Windows 정식 버전, Apple Silicon macOS 프리뷰 버전; 로컬 OCR 및 사람이 제어할 수 있는 답변
...
<sub>생성 도구 · `render.py` · 2026-09-23T17:45:55+08:00</sub>
